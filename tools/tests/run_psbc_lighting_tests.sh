...
"${CXX:-clang++-18}" -std=c++17 -O2 -I"$ROOT/ps4/third_party/ps4_vulkan/include" \
    "$ROOT/tools/tests/psbc_lighting_test.cpp" "$ROOT/tools/tests/psbc_host_compat.cpp" \
    "$ROOT/ps4/third_party/ps4_vulkan/lib/libpsbc.orbis.a" \
    "$ROOT/ps4/third_party/ps4_vulkan/lib/libopengnm.a" \
    "${LINK_FLAGS[@]}" -lpthread -ldl -lm -lz -o "$TEST_DIR/psbc_lighting_test"
"$TEST_DIR/psbc_lighting_test" "$ROOT/assets/shaders/celestial.vert.spv" celestial
"$TEST_DIR/psbc_lighting_test" "$ROOT/assets/shaders/wmo.frag.spv" wmo
rm -rf "$TEST_DIR"
