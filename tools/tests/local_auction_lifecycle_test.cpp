...
int main(){
 ...
 // The bought item is mailed like the proceeds, so a buyout from a virtual
 // seller still needs one free entry; a full queue blocks it without charging.
 assert(!d.buyout(1,buyer,world,error) && buyer.money==9895 && d.deliveries().size()==1024 && d.auctions().size()==1);
 full.pop_back();assert(d.restoreDeliveries(full));
 assert(d.buyout(1,buyer,world,error));assert(buyer.money==9500 && buyer.inventory.empty() && d.deliveries().size()==1024);
 assert(d.deliveries().back().recipient==1 && d.deliveries().back().itemId==800001 && d.deliveries().back().count==1 && !d.deliveries().back().money);
 full.push_back({99,0,1,0});
 ...
 // Human buyout with exactly enough room for the item and the seller's
 // proceeds; one free entry is one short of what the pair needs.
 assert(d.restoreAuctions({listing(5,2)},error));full.push_back({99,0,1,0});assert(d.restoreDeliveries(full));
 assert(!d.buyout(5,buyer,world,error) && buyer.money==9500 && d.deliveries().size()==1023 && d.auctions().size()==1);
 full.pop_back();assert(d.restoreDeliveries(full));
 assert(d.buyout(5,buyer,world,error) && buyer.money==9000 && d.deliveries().size()==1024);full.push_back({99,0,1,0});
 ...
 std::cout<<"PASS auction queue boundaries: zero/one/two deliveries, full-queue self bids and virtual buyouts, blocked outbid/expiry, exact item and proceeds space\n";
 ...
 // A simulated board already at MaxMarketAuctions leaves the bot nothing to
 // add, and the 32 places that difference reserves stay open for the player.
 constexpr unsigned Simulated=unsigned(LocalBotDirector::MaxMarketAuctions),Reserved=unsigned(LocalBotDirector::MaxAuctions-LocalBotDirector::MaxMarketAuctions);
 static_assert(Reserved==32);
 std::vector<LocalAuction> board;for(unsigned i=1;i<=Simulated;++i){auto v=listing(i,i%2?virtualSeller:p.guid);v.remainingSeconds=170000;board.push_back(v);}assert(bots.restoreAuctions(board,error));p.inventory={{800001,20}};bots.tick(300,world,players);assert(bots.auctions().size()==Simulated && p.inventory[0].count==20);
 buyer.inventory.assign(2,{800001,20});assert(bots.listStacksPriced(buyer,800001,1,uint16_t(Reserved/2),100,0,720,world,error));assert(bots.listStacksPriced(buyer,800001,1,uint16_t(Reserved/2),100,0,720,world,error));assert(bots.auctions().size()==LocalBotDirector::MaxAuctions);
 bots.setEnabled(false);bots.clear(players);assert(players.empty() && bots.auctions().size()==LocalBotDirector::MaxAuctions);
 std::cout<<"PASS walking bots: no dungeon spawn, no movement/listing during dead/combat/cast/flight/instance states, a full simulated board preserves the 32 reserved human slots, disable retains market\n";
}
