...
static void statAuraTest(){
 ...
 // Name the columns through the shipping layout: the literals here were the
 // pre-correction Spell.dbc mapping and had drifted off the proc fields. Aura
 // stacking is implemented now, so it is bounded by the local aura capacity
 // rather than refused outright; the proc guard is unchanged.
 row[spell335::StackAmount]=2;sd=dbc({row});tables.spells=sd.get();decoded={};assert(detail::decodeClientSpell(tables,0,decoded)&&decoded.maxAuraStacks==2);
 row[spell335::StackAmount]=256;sd=dbc({row});tables.spells=sd.get();decoded={};assert(!detail::decodeClientSpell(tables,0,decoded));
 row[spell335::StackAmount]=0;row[spell335::ProcFlags]=1;sd=dbc({row});tables.spells=sd.get();decoded={};assert(!detail::decodeClientSpell(tables,0,decoded));
 row[spell335::ProcFlags]=0;row[86]=21;sd=dbc({row});tables.spells=sd.get();decoded={};assert(detail::decodeClientSpell(tables,0,decoded) && !decoded.buffSelfOnly);
...
static void healingViewTest(){
 ...
 std::vector<uint32_t> row(234),range(40);row[0]=990003;row[28]=row[40]=row[46]=1;row[68]=UINT32_MAX;row[71]=6;row[86]=21;row[80]=6;row[95]=8;row[98]=250;row[spell335::ProcChance]=100;range[0]=1;range[3]=range[4]=bits(30);
 ...
 // Name the columns through the shipping layout. The literals this line used
 // to carry were the pre-correction Spell.dbc mapping and had drifted onto the
 // interruption and channel flags, so the guards below were never exercised.
 constexpr uint32_t TriggeredEffect=116;
 for(auto column:{spell335::ProcFlags,spell335::ProcCharges,TriggeredEffect}){auto invalid=row;invalid[column]=2;sd=dbc({invalid});tables.spells=sd.get();decoded={};assert(!detail::decodeClientSpell(tables,0,decoded));}
 // Aura stacking is implemented now, so it is bounded rather than refused:
 // a depth the local aura record can hold imports, one past it does not.
 {auto stacked=row;stacked[spell335::StackAmount]=255;sd=dbc({stacked});tables.spells=sd.get();decoded={};
  assert(detail::decodeClientSpell(tables,0,decoded)&&decoded.maxAuraStacks==255);
  stacked[spell335::StackAmount]=256;sd=dbc({stacked});tables.spells=sd.get();decoded={};
  assert(!detail::decodeClientSpell(tables,0,decoded));}
 std::cout<<"PASS periodic import guards: plain healing with unused proc chance accepted; proc flags, charges and triggered effects blocked; aura stacking bounded by the local capacity\n";
}
int main(){healingViewTest();shieldTest();statAuraTest();
 importTest();
