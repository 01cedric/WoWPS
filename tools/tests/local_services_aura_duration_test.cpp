...
static void statAuraTest(){
 ...
 // Friendly-target aura importing landed in the 0188 checkpoint, so the old
 // "blocked" expectation no longer describes the shipping decoder. Assert what
 // it does now rather than a boundary the product has already moved past.
...
int main(){statAuraTest();
 ...
 std::cout<<"PASS UDP/save18 talents: failed-save rollback, confirmed owner state, altered duplicate ignored, production reload and reset rollback\n";
 ...
 for(uint32_t i=0;i<192;++i){LocalSpellDefinition s;s.id=1000+i;s.name="Spell";c->spells.push_back(s);}member.knownSpells.clear();for(uint32_t i=0;i<192;++i)member.knownSpells.push_back(1000+i);member.knownTaxiNodes.clear();for(uint32_t i=1;i<=512;++i)member.knownTaxiNodes.push_back(i);member.statAuras={{70001,1000,member.mapId,member.instanceId}};h.progress(h.peers[0]);receive();assert(g.self.statAuras==member.statAuras);assert(h.saveRealm());LocalRealm::Impl buffReload;assert(buffReload.parseSave(good+"/realm.wprs")&&buffReload.findSaved(2)->player.statAuras==member.statAuras);assert(g.self.knownSpells.size()==192 && g.self.knownTaxiNodes.size()==512);
