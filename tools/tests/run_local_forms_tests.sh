...
if [ "$#" -ne 1 ]; then
    echo "Usage: run_local_forms_tests.sh DBC_DIRECTORY" >&2
    exit 2
fi
TEST_DIR="$(mktemp -d)"
...
# src/game/local_realm.cpp directly for the codec constants, so that translation
# unit must not also be compiled here.
"${CXX:-c++}" -std=c++20 -O1 -g -ffunction-sections -fdata-sections "${SAN_FLAGS[@]}" \
    ...
    "$ROOT/tools/tests/local_forms_test.cpp" \
