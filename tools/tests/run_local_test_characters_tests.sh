...
if [ "$#" -ne 1 ]; then
    echo "Usage: run_local_test_characters_tests.sh DBC_DIRECTORY" >&2
    exit 2
fi
TEST_DIR="$(mktemp -d)"
...
"${CXX:-c++}" -std=c++20 -O1 -g -ffunction-sections -fdata-sections "${SAN_FLAGS[@]}" \
    ...
    "$ROOT/tools/tests/local_test_characters_test.cpp" \
    ...
    -o "$TEST_DIR/local_test_characters_test"
ASAN_OPTIONS="${ASAN_OPTIONS:-detect_leaks=0}" UBSAN_OPTIONS="${UBSAN_OPTIONS:-halt_on_error=1}" \
    "$TEST_DIR/local_test_characters_test" "$1" "$ROOT/assets/local_realm/world.json"
