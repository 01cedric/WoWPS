...
"${CXX:-clang++-18}" -std=c++17 -O2 -I"$ROOT/ps4/third_party/ps4_vulkan/include" \
    "$ROOT/tools/tests/psbc_volumetric_test.cpp" "$ROOT/tools/tests/psbc_host_compat.cpp" \
    "$ROOT/ps4/third_party/ps4_vulkan/lib/libpsbc.orbis.a" \
    "$ROOT/ps4/third_party/ps4_vulkan/lib/libopengnm.a" \
    "${LINK_FLAGS[@]}" -lpthread -ldl -lm -lz -o "$TEST_DIR/psbc_volumetric_test"
for shader in volumetric.frag volumetric_resolve.frag volumetric_composite.frag; do
    if command -v spirv-val >/dev/null; then
        spirv-val --target-env vulkan1.0 "$ROOT/assets/shaders/$shader.spv"
    fi
    "$TEST_DIR/psbc_volumetric_test" "$ROOT/assets/shaders/$shader.spv"
done
rm -rf "$TEST_DIR"
