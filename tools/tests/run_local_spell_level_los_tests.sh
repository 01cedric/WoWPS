...
if [ "$#" -ne 1 ]; then
    echo "Usage: run_local_spell_level_los_tests.sh DBC_DIRECTORY" >&2
    exit 2
fi
TEST_DIR="$(mktemp -d)"
...
# The suite drives the real authority tick with the real client importer and
...
"${CXX:-c++}" -std=c++20 -O1 -g -ffunction-sections -fdata-sections "${SAN_FLAGS[@]}" \
    ...
    "$ROOT/tools/tests/local_spell_level_los_test.cpp" \
