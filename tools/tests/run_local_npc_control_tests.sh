...
if [ "$#" -ne 1 ]; then
    echo "Usage: run_local_npc_control_tests.sh DBC_DIRECTORY" >&2
    exit 2
fi
TEST_DIR="$(mktemp -d)"
...
# The suite drives the real authority tick for every suppression and lifecycle
# group and the real client importer for the admission census, and it includes
# src/game/local_realm.cpp directly for writeNpc/readNpc and the save/LAN
# version constants - so that translation unit must not also be compiled here.
# Everything the suite does not call is removed by linker garbage collection.
"${CXX:-c++}" -std=c++20 -O1 -g -ffunction-sections -fdata-sections "${SAN_FLAGS[@]}" \
    ...
    "$ROOT/tools/tests/local_npc_control_test.cpp" \
