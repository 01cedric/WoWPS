...
"${CXX:-c++}" -std=c++20 -O1 -g -ffunction-sections -fdata-sections "${SAN_FLAGS[@]}" \
    ...
    "$ROOT/tools/tests/local_script_sequence_test.cpp" "$ROOT/src/game/local_gameplay.cpp" "$ROOT/src/game/local_melee.cpp" \
    ...
    -Wl,--gc-sections -pthread -o "$TEST_DIR/local_script_sequence_test"
"$TEST_DIR/local_script_sequence_test" "$ROOT/tools/tests/fixtures/area_script_world.json"
