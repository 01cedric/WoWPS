...
"${CXX:-c++}" -std=c++20 -O1 -g -ffunction-sections -fdata-sections "${SAN_FLAGS[@]}" \
    ...
    "$ROOT/tools/tests/local_services_controller_actions_test.cpp" "$ROOT/src/game/local_gameplay.cpp" "$ROOT/src/game/local_melee.cpp" \
    ...
    -Wl,--gc-sections -pthread -o "$TEST_DIR/local_services_controller_actions_test"
# LeakSanitizer cannot enumerate threads in this sandbox. ASan/UBSan remain enabled.
ASAN_OPTIONS="${ASAN_OPTIONS:-detect_leaks=0}" "$TEST_DIR/local_services_controller_actions_test"
