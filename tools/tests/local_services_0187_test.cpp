...
static void statAuraTest(){
 ...
 row[31]=0;row[86]=21;sd=dbc({row});tables.spells=sd.get();decoded={};assert(!detail::decodeClientSpell(tables,0,decoded));
 std::cout<<"PASS aura DBC decoder: fixed self armor and long duration; proc, stacked and friendly-target variants remain blocked\n";
}
