...
"${CXX:-c++}" -std=c++20 -O1 -g -ffunction-sections -fdata-sections "${SAN_FLAGS[@]}" \
    ...
    "$ROOT/tools/tests/local_escort_test.cpp" "$ROOT/src/game/local_gameplay.cpp" "$ROOT/src/game/local_melee.cpp" \
    ...
    -Wl,--gc-sections -pthread -o "$TEST_DIR/local_escort_test"
"$TEST_DIR/local_escort_test" "$ROOT/tools/tests/fixtures/escort_script_world.json"
