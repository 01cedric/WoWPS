...
TEST_DIR="${CLEARCASTING_TEST_DIR:-$(mktemp -d)}"
mkdir -p "$TEST_DIR"
if [ -z "${CLEARCASTING_TEST_DIR:-}" ]; then trap 'rm -rf "$TEST_DIR"' EXIT; fi
SAN_FLAGS=()
...
for unit in game/local_gameplay game/local_melee game/local_services game/local_travel game/local_world_catalog pipeline/dbc_loader core/logger; do
    ...
    if [ "${CLEARCASTING_REUSE_FROZEN_OBJECTS:-0}" != 1 ] || [ ! -f "$object" ]; then
        "${CXX:-c++}" "${FLAGS[@]}" -c "$ROOT/src/$unit.cpp" -o "$object"
    fi
    OBJECTS+=("$object")
done
for test in local_clearcasting_runtime_test; do
