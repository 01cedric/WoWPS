...
if [ "$#" -ne 1 ]; then
    echo "Usage: run_local_range_facing_tests.sh DBC_DIRECTORY" >&2
    exit 2
fi
TEST_DIR="$(mktemp -d)"
...
# includes src/game/local_realm.cpp directly for the codec constants, so that
# translation unit must not also be compiled here.
"${CXX:-c++}" -std=c++20 -O1 -g -ffunction-sections -fdata-sections "${SAN_FLAGS[@]}" \
    ...
    "$ROOT/tools/tests/local_range_facing_test.cpp" \
    ...
    -o "$TEST_DIR/local_range_facing_test"
ASAN_OPTIONS="${ASAN_OPTIONS:-detect_leaks=0}" UBSAN_OPTIONS="${UBSAN_OPTIONS:-halt_on_error=1}" \
    "$TEST_DIR/local_range_facing_test" "$1" "$ROOT/assets/local_realm/catalog"
