...
static void statAuraTest(){
 ...
 // Class 5 spends mana, so the spell's own resource cost is actually testable;
 // a rage class would pay from a pool localResourcePools() sizes at zero.
 LocalGameplay g;g.useContent(c);LocalRealmPlayer p;p.guid=1;p.classId=5;p.race=1;p.level=1;p.health=p.mana=p.maxMana=100;p.knownSpells={70001,70002};p.gameplayInitialized=true;
 // Applying a buff recomputes derived pools, so the unbuffed maximum is
 // whatever the shipping stat table says for this class/race/level - not a
 // hand-set 100 that the first recompute discards. Pin the buff deltas
 // instead, which is what this file is really about.
 const auto baseHealth=localResourcePools(p,*c).health;p.maxHealth=baseHealth;
 std::string msg;auto cast=[&](uint32_t id,uint64_t target=0){return g.execute(p,{LocalAction::CastSpell,target,id},{&p},msg);};
 assert(!cast(70001,2)&&p.mana==100);assert(cast(70001)&&p.maxHealth==baseHealth+100&&p.health==100&&p.mana==97&&p.statAuras.size()==1);
 assert(cast(70001)&&p.statAuras.size()==1&&p.maxHealth==baseHealth+100);assert(cast(70002)&&p.statAuras.size()==1&&p.maxHealth==baseHealth+200);
 // the implementation (P04 competing auras): the lower rank cast over the higher one REPLACES it - Aura::CanStackWith -> IsRankOf -> false
 // -> RemoveOwnedAuras (SpellAuras.cpp:2160-2177); the build's former refusal of the downgrade had no source and was rewritten.
 auto before=p.mana;assert(cast(70001)&&p.mana<before&&p.statAuras.size()==1&&p.statAuras[0].spellId==70001&&p.maxHealth==baseHealth+100);assert(localStatAuraBonus(p,*c,true)==40);
 for(int i=0;i<10;++i)g.tick(.1f,{&p});assert(p.statAuras.empty()&&p.maxHealth==baseHealth);
 std::cout<<"PASS stat aura authority: self targeting, no free healing, refresh, rank upgrade/downgrade, armor and expiry\n";
 assert(cast(70001));assert(g.execute(p,{LocalAction::CancelStatAura,0,70001},{&p},msg)&&p.statAuras.empty()&&p.maxHealth==baseHealth);assert(!g.execute(p,{LocalAction::CancelStatAura,0,70001},{&p},msg));
 assert(cast(70001));p.dead=true;g.tick(.1f,{&p});assert(p.statAuras.empty());p.dead=false;p.health=100;
 assert(cast(70001));p.mapId=1;g.tick(.1f,{&p});assert(p.statAuras.empty()&&p.maxHealth==baseHealth);
 ...
 assert(cast(70001));Writer w;writeProgress(w,p);Reader r(w.bytes.data(),w.bytes.size());LocalRealmPlayer q=p;q.statAuras.clear();assert(readProgress(r,q)&&r.done()&&reloadedStatAurasMatch(q.statAuras,p.statAuras));
 ...
 for(uint32_t i=0;i<8;++i)assert(dot(60000+i));before=caster.mana;assert(!dot(60008)&&caster.mana==before);assert(dot(60009));
 // the implementation (P04 competing auras): the lower rank cast back over the higher one REPLACES it in its slot, at capacity too
 // (Aura::CanStackWith -> IsRankOf, SpellAuras.cpp:2160-2177); the former downgrade refusal had no source.
 before=caster.mana;assert(dot(60000)&&caster.mana<before);assert(dot(60009));
 std::cout<<"PASS damage auras: eight-effect target bound rejects before spending, rank replacement in both directions and refresh at capacity\n";
 ...
 row[49]=2;sd=dbc({row});tables.spells=sd.get();decoded={};assert(detail::decodeClientSpell(tables,0,decoded)&&decoded.maxAuraStacks==2);
 row[49]=256;sd=dbc({row});tables.spells=sd.get();decoded={};assert(!detail::decodeClientSpell(tables,0,decoded));
 row[49]=0;row[34]=1;sd=dbc({row});tables.spells=sd.get();decoded={};assert(!detail::decodeClientSpell(tables,0,decoded));
 row[34]=0;row[86]=21;sd=dbc({row});tables.spells=sd.get();decoded={};assert(detail::decodeClientSpell(tables,0,decoded) && !decoded.buffSelfOnly);
 std::cout<<"PASS aura DBC decoder: fixed self armor and long duration; proc and excessive stacks blocked; bounded stacks and friendly targets supported\n";
}
...
static void shieldTest(){
 ...
 // (Unit::IsWithinCombatRange, Unit.cpp:766-780), so a 30 yd row reaches
 // 30 + 1.5 + 1.5 = 33 yd between two players. The refusal that was asserted at
 // 31 yd - the reach-blind behaviour - is asserted past the real maximum.
 friendPlayer.x=34;assert(!cast()&&caster.mana==100);friendPlayer.x=0;friendPlayer.dead=true;assert(!cast()&&caster.mana==100);friendPlayer.dead=false;friendPlayer.instanceId=1;assert(!cast());friendPlayer.instanceId=0;
 ...
 Writer w;writeProgress(w,friendPlayer);Reader r(w.bytes.data(),w.bytes.size());auto restored=friendPlayer;restored.statAuras.clear();assert(readProgress(r,restored)&&r.done()&&reloadedStatAurasMatch(restored.statAuras,friendPlayer.statAuras));
 ...
 // Aggro lives in the threat table and targetGuid is re-derived from it on
 // every tick, so a creature carrying only a target reads as having evaded on
 // the first frame and never swings. Seed the threat that target stands for.
 c->npcs[0].damage=20;auto enemy=rewardNpc();enemy.health=enemy.maxHealth=1000;enemy.targetGuid=friendPlayer.guid;enemy.threat[0]={friendPlayer.guid,1000};game.setRemoteNpcs({enemy});
 // Applying the shield recomputes the derived health pool, so the claim here
 // is that the hit costs no health at all - not that it leaves a hand-set 100.
 assert(cast());const auto shielded=friendPlayer.health;game.tick(.1f,group);
 // Armor runs before absorption, so the shield loses exactly what the hit
 // actually attempted after mitigation - less than the creature's raw damage.
 const auto swings=game.combatEvents(); // returned by value; keep one copy alive
 const auto swing=std::find_if(swings.begin(),swings.end(),
     [](const auto& ev){return ev.kind==LocalCombatEventKind::NpcMelee;});
 assert(swing!=swings.end());
 // The swing outcome is rolled from a clock-seeded generator, so state what
 // holds for every roll: whatever survives the block is absorbed in full, none
 // of it reaches health, and the shield is drawn down by exactly that much.
 assert(swing->absorbed+swing->blocked==swing->attempted&&!swing->effective);
 assert(friendPlayer.health==shielded&&friendPlayer.statAuras[0].absorbRemaining==d.buffAbsorb-swing->absorbed);
 // Armor runs before absorption: an ordinary hit reaches the shield already
 // reduced below the creature's authored damage.
 if(swing->outcome==LocalMeleeOutcome::Hit)assert(swing->attempted>0&&swing->attempted<c->npcs[0].damage);
...
static void healingViewTest(){
 ...
 LocalGameplay game;game.useContent(c);auto caster=rewardPlayer(1),target=rewardPlayer(2);caster.knownSpells={d.id};caster.mana=caster.maxMana=100;target.health=10;target.regenerationTickMs=0;std::vector<LocalRealmPlayer*> group{&caster,&target};std::string msg;
 ...
 std::cout<<"PASS transient healing wire: roundtrip, malformed metadata rejection, Save20 excludes transient healing views and no persisted display\n";
 std::vector<uint32_t> row(234),range(40);row[0]=990003;row[28]=row[40]=row[46]=1;row[68]=UINT32_MAX;row[71]=6;row[86]=21;row[80]=6;row[95]=8;row[98]=250;row[35]=100;range[0]=1;range[3]=range[4]=bits(30);
 ...
 for(auto column:{34,36,116}){auto invalid=row;invalid[column]=2;sd=dbc({invalid});tables.spells=sd.get();decoded={};assert(!detail::decodeClientSpell(tables,0,decoded));}
 std::cout<<"PASS periodic import guards: plain healing with unused proc chance accepted; proc flags, charges and triggered effects blocked\n";
}
...
static void auraStackTest(){
 auto c=std::make_shared<LocalWorldContent>();LocalSpellDefinition d;d.id=77001;d.name="Stacked armor";d.clientSpell=true;d.buffArmor=20;d.durationMs=1000;d.maxAuraStacks=3;d.range=30;d.mana=2;
 c->spells.push_back(d);auto high=d;high.id=77002;high.buffArmor=40;c->spells[0].supercededBySpell=high.id;c->spells.push_back(high);
 // Applying a buff recomputes the derived pools, and class 1 has no mana pool
 // at all: the hand-set 100 would become 0 and the second cast in the loop
 // below could not pay for itself. Class 5 spends what the spell actually costs.
 LocalGameplay g;g.useContent(c);LocalRealmPlayer p;p.guid=1;p.classId=5;p.race=1;p.level=1;p.health=p.maxHealth=100;p.mana=p.maxMana=100;p.knownSpells={d.id,high.id};p.gameplayInitialized=true;
 std::string msg;auto cast=[&](uint32_t id){return g.execute(p,{LocalAction::CastSpell,p.guid,id},{&p},msg);};
 for(unsigned i=1;i<=4;++i){assert(cast(d.id));assert(p.statAuras.size()==1&&p.statAuras[0].stacks==std::min(i,3u));assert(localStatAuraBonus(p,*c,true)==20*std::min(i,3u));}
 p.statAuras[0].remainingMs=10;assert(cast(d.id));assert(p.statAuras[0].remainingMs==1000&&p.statAuras[0].stacks==3);
 auto mana=p.mana;assert(cast(high.id));assert(p.statAuras.size()==1&&p.statAuras[0].stacks==1&&p.statAuras[0].spellId==high.id);
 // the implementation (P04 competing auras): the lower rank cast over the higher one REPLACES it and starts its own stack at 1
 // (Aura::CanStackWith -> IsRankOf, SpellAuras.cpp:2160-2177); the former downgrade refusal had no source.
 mana=p.mana;assert(cast(d.id)&&p.mana<mana&&p.statAuras.size()==1&&p.statAuras[0].spellId==d.id&&p.statAuras[0].stacks==1);
 assert(cast(high.id));Writer save;writeProgress(save,p);Reader r(save.bytes.data(),save.bytes.size());auto loaded=p;loaded.statAuras.clear();assert(readProgress(r,loaded)&&r.done()&&reloadedStatAurasMatch(loaded.statAuras,p.statAuras));
 Writer legacy;writeProgress(legacy,p,19);Reader old(legacy.bytes.data(),legacy.bytes.size());assert(readProgress(old,loaded,19)&&old.done()&&loaded.statAuras[0].stacks==1);
 loaded=p;loaded.statAuras[0].stacks=0;assert(!validLocalStatAuras(loaded));Writer corrupt;writeProgress(corrupt,loaded);Reader cr(corrupt.bytes.data(),corrupt.bytes.size());assert(!readProgress(cr,loaded));
 assert(g.execute(p,{LocalAction::CancelStatAura,0,high.id},{&p},msg));assert(p.statAuras.empty());
 std::cout<<"PASS aura stacks: cap, refresh, rank reset in both directions, Save20 roundtrip, Save19 migration, zero rejection and cancellation\n";
 // Stack shields remain bounded; partial consumption survives serialization.
 auto shield=d;shield.id=77003;shield.buffArmor=0;shield.buffAbsorb=50;shield.absorbSchoolMask=127;c->spells.push_back(shield);p.knownSpells.push_back(shield.id);
 assert(cast(shield.id)&&cast(shield.id));assert(p.statAuras[0].stacks==2&&p.statAuras[0].absorbRemaining==100);
 assert(localAbsorbDamage(p,*c,75,1)==0&&p.statAuras[0].absorbRemaining==25);
 Writer partial;writeProgress(partial,p);Reader pr(partial.bytes.data(),partial.bytes.size());assert(readProgress(pr,loaded)&&reloadedStatAurasMatch(loaded.statAuras,p.statAuras));
 assert(localAbsorbDamage(p,*c,30,1)==5&&p.statAuras.empty());
 assert(localStackedAuraAmount(UINT32_MAX,255)==1000000);
 std::cout<<"PASS stacked shields: combined capacity, partial damage, persistence, exhaustion and overflow bound\n";
 // Independent casters have independent periodic stacks.
 auto hot=d;hot.id=77004;hot.buffArmor=0;hot.periodicHeal=hot.periodicHealMax=7;hot.periodicIntervalMs=250;hot.healingSelfOnly=false;c->spells.push_back(hot);p.knownSpells.push_back(hot.id);
 auto recipient=p;recipient.guid=2;recipient.health=10;recipient.statAuras.clear();recipient.regenerationTickMs=0;p.regenerationTickMs=0;
 auto healer=p;healer.guid=3;std::vector<LocalRealmPlayer*> players{&p,&recipient,&healer};
 auto heal=[&](LocalRealmPlayer& owner){return g.execute(owner,{LocalAction::CastSpell,recipient.guid,hot.id},players,msg);};
 assert(heal(p)&&heal(p)&&heal(healer));assert(recipient.healingAuras.size()==2&&recipient.healingAuras[0].stacks==2&&recipient.healingAuras[1].stacks==1);
 g.tick(.25f,players);assert(recipient.health==31);
 Writer view;writeHealingViews(view,recipient);Reader vr(view.bytes.data(),view.bytes.size());assert(readHealingViews(vr,loaded)&&loaded.healingAuras==recipient.healingAuras);
 assert(g.execute(recipient,{LocalAction::CancelStatAura,0,hot.id},players,msg));auto hp=recipient.health;g.tick(.25f,players);assert(recipient.health==hp&&recipient.healingAuras.empty());
 std::cout<<"PASS periodic healing stacks: per-caster isolation, multiplied ticks, owner wire counts and cancellation\n";
 auto dc=rewardContent();dc->spells.clear();auto dot=hot;dot.id=77005;dot.periodicHeal=dot.periodicHealMax=0;dot.periodicDamage=9;dc->spells.push_back(dot);LocalGameplay damage;damage.useContent(dc);auto caster=rewardPlayer(1);caster.mana=caster.maxMana=100;caster.knownSpells={dot.id};auto enemy=rewardNpc();enemy.health=enemy.maxHealth=1000;enemy.targetGuid=caster.guid;damage.setRemoteNpcs({enemy});
 for(int i=0;i<5;++i)assert(damage.execute(caster,{LocalAction::CastSpell,enemy.guid,dot.id},{&caster},msg));
 damage.tick(.25f,{&caster});assert(damage.npcs().size()==1&&damage.npcs()[0].health==973);
 std::cout<<"PASS periodic damage stacks: five applications cap at three and tick for the stacked amount\n";
}


static void combatEventTest(){
 auto c=rewardContent();c->quests.clear();c->spells[0].clientSpell=true;
 LocalSpellDefinition heal;heal.id=2;heal.name="Heal";heal.clientSpell=true;heal.heal=10;heal.range=30;
 LocalSpellDefinition hot=heal;hot.id=3;hot.heal=0;hot.periodicHeal=7;hot.periodicIntervalMs=100;hot.durationMs=300;
 LocalSpellDefinition dot=hot;dot.id=4;dot.periodicHeal=0;dot.periodicDamage=9;
 LocalSpellDefinition shield;shield.id=5;shield.name="Shield";shield.clientSpell=true;shield.buffAbsorb=100;shield.absorbSchoolMask=1;shield.durationMs=10000;shield.range=30;
 c->spells={c->spells[0],heal,hot,dot,shield};c->npcs[0].damage=12;
 LocalGameplay g;g.useContent(c);auto p=rewardPlayer(1),q=rewardPlayer(2);p.knownSpells={1,2,3,4,5};q.health=95;
 std::string result;assert(g.execute(p,{LocalAction::CastSpell,q.guid,2},{&p,&q},result));
 auto e=g.combatEvents();
 // A cast now brackets its effect with the proc-phase markers the proc system
 // observes - SpellCast before, SpellFinish after - so the healing observation
 // is no longer the only thing one cast reports. Pin the whole sequence rather
 // than a lone event, and measure the rejected command as "nothing appended".
 assert(e.size()==3&&e[0].kind==LocalCombatEventKind::SpellCast&&e[2].kind==LocalCombatEventKind::SpellFinish);
 assert(e[1].source==1&&e[1].target==2&&e[1].spell==2&&e[1].attempted==10&&e[1].effective==5&&e[1].kind==LocalCombatEventKind::DirectHeal);
 const auto beforeRejection=e.size();
 assert(!g.execute(p,{LocalAction::CastSpell,999,2},{&p,&q},result));assert(g.combatEvents().size()==beforeRejection);
 auto n=rewardNpc();n.health=4;g.setRemoteNpcs({n});assert(g.execute(p,{LocalAction::CastSpell,n.guid,1},{&p,&q},result));
 e=g.combatEvents();
 const auto damage=std::find_if(e.rbegin(),e.rend(),[](const auto& ev){return ev.kind==LocalCombatEventKind::SpellDamage;});
 assert(damage!=e.rend()&&damage->attempted==10&&damage->effective==4&&damage->killed);
 std::cout<<"PASS combat events: actual healing, overkill, identities and no event from rejected commands\n";
 g.useContent(c);p=rewardPlayer(1);q=rewardPlayer(2);q.health=50;p.knownSpells={1,2,3,4,5};
 n=rewardNpc();n.health=n.maxHealth=1000;n.targetGuid=p.guid;n.attackTimer=1000;g.setRemoteNpcs({n});
 assert(g.execute(p,{LocalAction::CastSpell,q.guid,3},{&p,&q},result));assert(g.execute(p,{LocalAction::CastSpell,n.guid,4},{&p,&q},result));
 // The two casts themselves are observed now, so the stream is not empty just
 // because nothing has ticked. This case is about the periodic observations.
 auto periodicEvents=[&]{auto all=g.combatEvents();decltype(all) out;
     for(const auto& ev:all)if(ev.kind==LocalCombatEventKind::PeriodicDamage||ev.kind==LocalCombatEventKind::PeriodicHeal)out.push_back(ev);
     return out;};
 assert(periodicEvents().empty());g.tick(.1f,{&p,&q});e=periodicEvents();
 assert(e.size()==2&&e[0].kind==LocalCombatEventKind::PeriodicDamage&&e[0].effective==9&&e[0].spell==4);
 assert(e[1].kind==LocalCombatEventKind::PeriodicHeal&&e[1].effective==7&&e[1].spell==3);
 std::cout<<"PASS periodic event timing and spell attribution for damage/healing ticks\n";
 // A self-cast shield puts no threat on the creature, and targetGuid alone is
 // re-derived away on the first tick, so seed the aggro that target stands for.
 g.useContent(c);p=rewardPlayer(1);p.knownSpells={5};n=rewardNpc();n.health=n.maxHealth=1000;n.targetGuid=p.guid;n.threat[0]={p.guid,1000};g.setRemoteNpcs({n});
 assert(g.execute(p,{LocalAction::CastSpell,p.guid,5},{&p},result));
 // A creature swing can be avoided outright (miss/dodge/parry), which attempts
 // no damage at all and therefore absorbs none; the absorb rule under test is
 // about a swing that connects. Tick until the first one does - the shield
 // holds 100 against this creature's 12 per swing, so the first connecting
 // swing is always the fully absorbed one.
 LocalCombatEvent melee{};bool connected=false;
 for(unsigned attempt=0;attempt<64&&!connected;++attempt){
     g.tick(.1f,{&p});
     for(const auto& ev:g.combatEvents())if(ev.kind==LocalCombatEventKind::NpcMelee&&ev.attempted>0){melee=ev;connected=true;break;}
 }
 assert(connected&&melee.source==n.guid&&melee.effective==0&&melee.absorbed==melee.attempted&&melee.attempted>0);
 LocalGameplay guest;guest.setRemoteNpcs({n});assert(guest.combatEvents().empty());
 LocalCombatHistory history;for(unsigned i=0;i<200;++i)history.record({});auto rows=history.snapshot();
 assert(rows.size()==128&&rows.front().sequence==73&&rows.back().sequence==200&&history.overwritten()==72);
 history.clear();assert(history.snapshot().empty()&&history.overwritten()==0);
 std::cout<<"PASS absorbed hits, guest snapshots excluded, bounded ordered history and reset\n";
}


static void legacyAuraColumnTest(){
 auto c=rewardContent();LocalSpellDefinition shield;shield.id=50;shield.name="Legacy shield";shield.clientSpell=true;shield.buffAbsorb=50;shield.absorbSchoolMask=1;shield.buffHealth=20;shield.maxAuraStacks=1;c->spells.push_back(shield);
 LocalGameplay g;g.useContent(c);auto p=rewardPlayer(1);p.knownSpells.push_back(50);p.statAuras={{50,10000,0,0,p.guid,5050,101}};
 g.initializePlayer(p,false);assert(p.statAuras.size()==1&&p.statAuras[0].stacks==1&&p.statAuras[0].absorbRemaining==50);
 Writer writer;writeProgress(writer,p);auto q=p;q.statAuras.clear();Reader reader(writer.bytes.data(),writer.bytes.size());assert(readProgress(reader,q)&&reader.done()&&q.statAuras==p.statAuras);
 std::cout<<"PASS legacy chance-as-stack save normalized before login stats and owner replication\n";
}

int main(){
 std::cout<<std::unitbuf;
 legacyAuraColumnTest();combatEventTest();auraStackTest();mailboxStartTest();healingViewTest();shieldTest();statAuraTest();
 importTest();
 ...
 // Both the health pool and the talent budget come from shipping tables that
 // are revised across checkpoints. What this case is about is what the talents
 // themselves add and spend, so measure the deltas against the live tables
 // rather than pinning the absolute numbers one table revision produced.
 const auto baseHealth=localResourcePools(p,*c).health;const auto talentBudget=localTalentPointsAvailable(p);
 ...
 assert(!run(LocalAction::LearnTalent,10,2));for(uint32_t rank=0;rank<5;++rank)assert(run(LocalAction::LearnTalent,10,rank));assert(p.maxHealth==baseHealth+50 && localTalentPointsAvailable(p)==talentBudget-5);assert(!run(LocalAction::LearnTalent,10,4));
 assert(run(LocalAction::LearnTalent,20) && std::find(p.knownSpells.begin(),p.knownSpells.end(),201)!=p.knownSpells.end());assert(run(LocalAction::ResetTalents));assert(p.talents.empty() && p.maxHealth==baseHealth && p.knownSpells==std::vector<uint32_t>{1});
 ...
 for(uint32_t i=0;i<192;++i){LocalSpellDefinition s;s.id=1000+i;s.name="Spell";c->spells.push_back(s);}member.knownSpells.clear();for(uint32_t i=0;i<192;++i)member.knownSpells.push_back(1000+i);member.knownTaxiNodes.clear();for(uint32_t i=1;i<=512;++i)member.knownTaxiNodes.push_back(i);member.statAuras={{70001,1000,member.mapId,member.instanceId,1,25,3}};h.progress(h.peers[0]);receive();assert(g.self.statAuras==member.statAuras);assert(h.saveRealm());LocalRealm::Impl buffReload;assert(buffReload.parseSave(good+"/realm.wprs")&&buffReload.findSaved(2)->player.statAuras==member.statAuras);assert(g.self.knownSpells.size()==192 && g.self.knownTaxiNodes.size()==512);
 ...
 member.statAuras.clear();assert(host.castSpell(shield.id,2));h.progress(h.peers[0]);receive();assert(reloadedStatAurasMatch(g.self.statAuras,member.statAuras)&&g.self.statAuras[0].casterGuid==h.self.guid&&g.self.statAuras[0].absorbRemaining==50);assert(h.saveRealm());LocalRealm::Impl shieldLoad;assert(shieldLoad.parseSave(good+"/realm.wprs")&&shieldLoad.findSaved(1)->player.statAuras[0].absorbRemaining==7&&reloadedStatAurasMatch(shieldLoad.findSaved(2)->player.statAuras,member.statAuras));
 ...
 LocalSpellDefinition hot;hot.id=75001;hot.name="Network healing";hot.clientSpell=true;hot.periodicHeal=hot.periodicHealMax=7;hot.periodicIntervalMs=250;hot.durationMs=1000;hot.range=30;c->spells.push_back(hot);h.self.knownSpells.push_back(hot.id);member.health=10;member.regenerationTickMs=0;
