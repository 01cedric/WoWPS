...
int main() {
 ...
 for (unsigned trial=0; trial<1000; ++trial) {
  ...
  for(auto& f:flags) f=rng()%32;
  while(!flags.empty()) {
   size_t idx=rng()%flags.size(), oldTail=flags.size()-1;
   std::vector<size_t> lists[5];
   for(size_t i=0;i<flags.size();++i)for(int k=0;k<5;++k)
...
''' + repair + r'''
 ...
 std::cout<<"PASS: production auxiliary index repair matches full rebuild across 1000 randomized unload sequences, five categories, tail removal and reordered particles\n";
}
'''
