...
if [ "$#" -ne 1 ]; then
    echo "Usage: run_local_proc_modifier_admission_tests.sh DBC_DIRECTORY" >&2
    exit 2
fi
TEST_DIR="$(mktemp -d)"
...
# The suite drives the real proc dispatcher for the timed/charged/pet halves and
# the real client importer for the operation 18/26 admission gate, so it links
# the gameplay translation units and the DBC loader together. Everything it does
# not call is removed by linker garbage collection.
"${CXX:-c++}" -std=c++20 -O1 -g -ffunction-sections -fdata-sections "${SAN_FLAGS[@]}" \
    ...
    "$ROOT/tools/tests/local_proc_modifier_admission_test.cpp" \
    ...
    -o "$TEST_DIR/local_proc_modifier_admission_test"
ASAN_OPTIONS="${ASAN_OPTIONS:-detect_leaks=0}" UBSAN_OPTIONS="${UBSAN_OPTIONS:-halt_on_error=1}" \
    "$TEST_DIR/local_proc_modifier_admission_test" "$1"
