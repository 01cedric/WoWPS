...
static void authority(){
    ...
    // A swap keeps the backpack cell the stack already occupied; bank stacks
    // carry no cell, so they keep the unassigned LocalItemStack default.
    reject(deposit(p,1,2,1));assert(call(deposit(p,1,2,4)));assert((p.inventory[0]==LocalItemStack{118,3,0}) && (p.bank[1]==LocalItemStack{117,4}));
    ...
    // Combat is the realm's shared localCombatActive state, so the attacking
    // mode keeps the banker friendly and in range and puts a live hostile
    // creature the player is actually engaged with on the map beside it.
    for(int mode=0;mode<6;++mode){auto n=banker();auto foe=rewardNpc(99);p=player(1);if(mode==0)p.dead=true;if(mode==1)p.flight.active=true;if(mode==2)p.castingSpellId=1;if(mode==3)p.attackTarget=foe.guid;if(mode==4)n.x=100;if(mode==5)n.instanceId=1;std::vector<LocalRealmNpc> around{n};if(mode==3)around.push_back(foe);game.setRemoteNpcs(around);reject(deposit(p,1,28,1));}
...
static void bankLan(){
    ...
    c=deposit(member,1,2,6);auto before=member;h.directory+="/missing/swap";command(c);assert(member.inventory==before.inventory && member.bank==before.bank);h.directory=good;command(c);assert((member.inventory[0]==LocalItemStack{118,3,0}) && (member.bank[1]==LocalItemStack{117,6}) && g.self.inventory==member.inventory);
    assert(host.depositBankSlot(1,2,6,{117,6},{118,3},banker().guid));
    LocalRealm::Impl loaded,scanner;loaded.gameplay.useContent(content);assert(loaded.parseSave(h.directory+"/realm.wprs") && scanner.parseSave(h.directory+"/realm.wprs"));for(uint64_t id:{1,2}){const auto* p=loaded.findSaved(id);assert(p && (p->player.inventory[0]==LocalItemStack{118,3,0}) && (p->player.bank[1]==LocalItemStack{117,6}) && p->player.bank[27].count==4);assert(scanner.findSaved(id)->player.bank==p->player.bank);}
    host.stop();guest.stop();std::filesystem::remove_all(dir);
    std::cout<<"PASS targeted bank LAN/save: actual new command encoding, missing extension rejection, host/guest disk rollback, stale/replayed/changed-destination commands, slot swaps, save28 and catalog-free reload\n";
}
