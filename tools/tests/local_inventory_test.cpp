...
int main(){
 ...
 // Combat is the realm's shared localCombatActive state, so the attacking mode
 // puts the creature the player is engaged with on the map beside the banker.
 for(int mode=0;mode<5;++mode){p=player(1);auto foe=rewardNpc(99);auto c=move(p,1,13,1,true);if(mode==0)c.serviceNpcGuid=11;if(mode==1)p.dead=true;if(mode==2){p.attackTarget=foe.guid;game.setRemoteNpcs({banker(),foe});}if(mode==3)p.flight.active=true;if(mode==4)p.castingSpellId=1;reject(c);game.setRemoteNpcs({banker()});}
 ...
 // Refusing while in combat means the realm's shared localCombatActive
 // state, so the creature being attacked has to be present on the map.
 assert(call(equip));assert(p.equipment[0]==item.id);auto enemy=rewardNpc(99);p.attackTarget=enemy.guid;game.setRemoteNpcs({banker(),enemy});reject({LocalAction::UnequipItem,0,0});
 p.attackTarget=0;game.setRemoteNpcs({banker()});assert(call({LocalAction::UnequipItem,0,0}));
