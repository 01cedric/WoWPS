...
static constexpr auto mailSite=kLocalMailboxSites[0];
static constexpr uint64_t mailboxGuid=mailSite.guid;
static LocalRealmPlayer player(uint64_t id){auto p=rewardPlayer(id);p.mapId=mailSite.mapId;p.x=mailSite.x;p.y=mailSite.y;p.z=mailSite.z;p.race=1;p.classId=1;p.quests.clear();p.inventory={{117,20}};p.money=10000;return p;}
static LocalRealmNpc inn(){auto n=rewardNpc(10);n.hostile=false;n.lootOwner=0;n.innkeeper=true;n.auctioneer=true;n.mapId=mailSite.mapId;n.x=mailSite.x+2;n.y=mailSite.y;n.z=mailSite.z;return n;}
...
int main(){
 ...
 assert(host.mailAccess(mailboxGuid) && guest.mailAccess(mailboxGuid));
 assert(!host.mailAccess(10) && !guest.mailAccess(10)); // Nearby innkeeper cannot serve invisible mail.
 ...
 auto action=[&](LocalAction a,uint32_t id,uint32_t slot=0){LocalRealmCommand c{a,slot,id};c.serviceNpcGuid=mailboxGuid;command(c);return h.peers[0].lastCommandSuccess;};
 auto query=[&]{h.now+=1;g.now+=1;guest.requestMail(mailboxGuid);h.receive();receive();};
 ...
 h.directory+="/missing/send";assert(!host.sendMail(mailboxGuid,"Player2","One","Body",100,0,{{117,4,20,0}}));h.directory=good;
 assert(h.mailbox.messages.empty() && h.self.money==10000 && h.mailbox.nextId==1);
 assert(!host.sendMail(11,"Player2","One","Body",100,0,{}));
 assert(!host.sendMail(mailboxGuid,"Missing","One","Body",100,0,{}));
 assert(host.sendMail(mailboxGuid,"pLaYeR2","One","Body",100,0,{{117,4,20,0}}));
 ...
 assert(host.sendMail(mailboxGuid,"Player3","Offline","Saved character",0,0,{}));assert(h.hasMailAssets(1) && h.hasMailAssets(2));
 ...
 assert(host.sendMail(mailboxGuid,"Player2","COD","",0,500,{{117,2,16,0}}));const auto codId=h.mailbox.messages.back().id;
 ...
 LocalRealmCommand changed{LocalAction::MailTakeMoney,0,payment->id};changed.serviceNpcGuid=mailboxGuid;
 Writer replay;replay.u32(h.peers[0].lastCommand);replay.u8(uint8_t(changed.action));replay.u64(changed.target);replay.u32(changed.id);replay.u32(0);replay.u32(0);replay.u32(0);replay.u64(mailboxGuid);
 ...
 assert(host.sendMail(mailboxGuid,"Player2","Return","",20,0,{}));const auto returnId=h.mailbox.messages.back().id;
 ...
 assert(!host.mailAction(LocalAction::MailReturn,mailboxGuid,returnId));
 ...
 LocalRealmCommand send{LocalAction::MailSend};send.serviceNpcGuid=mailboxGuid;send.mailRecipient="Player3";send.mailSubject="LAN letter";send.mailBody=std::string(160,'b');send.mailAttachments={{117,1,20,0}};
 ...
 const auto fullMoney=h.self.money;assert(!host.sendMail(mailboxGuid,"Player2","Full","",0,0,{}));assert(h.self.money==fullMoney);
 ...
 // The reload above is the CURRENT save format, whatever that is; naming it
 // with a literal only recorded which checkpoint last edited this line.
 std::cout<<"PASS escrow migration: full inbox backpressure, exact item delivery, no repeat migration, atomic Save"<<int(SaveVersion)<<" reload, 64-letter private pages\n";
 ...
 // This realm keeps no letters and no owned creatures, so the payload after the
 // vendor stock is exactly u32 sequence, u32 mail id, u16 mail count, u8 pet
 // count and the u32 checksum; the mail sequence is the second word from the end.
 assert(h.mailbox.messages.empty() && h.gameplay.pets().empty());
 std::vector<uint8_t> bytes;assert(readFile(good+"/realm.wprs",bytes,MaxSaveSize));assert(bytes[4]==SaveVersion);
