...
"${CXX:-c++}" -std=c++20 -O1 -g -ffunction-sections -fdata-sections "${SAN_FLAGS[@]}" \
    ...
    "$ROOT/tools/tests/local_pet_runtime_test.cpp" \
    ...
    "$ROOT/src/core/logger.cpp" -Wl,--gc-sections -pthread -o "$TEST_DIR/local_pet_runtime_test"
ASAN_OPTIONS="${ASAN_OPTIONS:-detect_leaks=0}" "$TEST_DIR/local_pet_runtime_test"
