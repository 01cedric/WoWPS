// Exercise the packaged PSBC archive with the actual sky per-frame descriptor ABI.
...
int main(int argc, char** argv) {
    ...
    const bool vertex = std::string(argv[2]) == "vertex";
    PsbcDescriptorBinding frame[] = {{0,6,1,0,16,UINT32_MAX},{1,1,1,16,48,UINT32_MAX}};
    PsbcDescriptorSetLayout sets[] = {{2,64,0,frame}};
    ...
    std::printf("PSBC GFX7 sky %s: result=%u %s binaryBytes=%zu\n",
    ...
    if (std::string(argv[2]) == "fragment") assert(descriptorTable);
