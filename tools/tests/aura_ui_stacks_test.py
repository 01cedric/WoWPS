...
ui.sync(p,c);auto events=ui.events;p.statAuras[0].stacks=3;ui.sync(p,c);assert(ui.events==events+2&&ui.sp.a[0].charges==3);ui.sync(p,c);assert(ui.events==events+2);p.healingAuras[0].stacks=2;ui.sync(p,c);assert(ui.sp.a[1].charges==2);
