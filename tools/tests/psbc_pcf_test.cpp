// Compile all four optimized production PCF shaders using their descriptor ABI.
...
int main(int argc, char** argv) {
    ...
    PsbcDescriptorBinding terrainMaterial[8]{};
    for (uint32_t i=0;i<7;++i) terrainMaterial[i]={i,1,1,i*48,48,UINT32_MAX};
    terrainMaterial[7]={7,6,1,336,16,UINT32_MAX};
    ...
    if(kind=="terrain") sets[1]={8,352,0,terrainMaterial};
    else if(kind=="m2") sets[1]={2,64,0,m2Material};
    else assert(kind=="wmo" || kind=="character");
    ...
    std::printf("PSBC GFX7 PCF %s: result=%u %s binaryBytes=%zu\n",
