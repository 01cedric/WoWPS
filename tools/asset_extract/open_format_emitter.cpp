...
// The client's renderer.cpp carries the stb_image_write implementation of
// its own; a build that links this file into the client (the PS4 port runs
// the extractor in-process) defines WOWEE_STB_IMAGE_WRITE_EXTERNAL so there
// is one copy in the link. The command-line tool has no other and keeps this.
#ifndef WOWEE_STB_IMAGE_WRITE_EXTERNAL
