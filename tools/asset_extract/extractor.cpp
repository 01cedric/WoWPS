#include "pipeline/client_locale.hpp"
...
// Options::progress is called from the worker threads too; one lock keeps the
// calls from overlapping so the sink needs no locking of its own.
static std::mutex g_progressMutex;
static void notifyProgress(const Extractor::Options& opts, const char* stage,
                           uint64_t done, uint64_t total) {
    if (!opts.progress) return;
    std::lock_guard<std::mutex> lock(g_progressMutex);
    opts.progress(stage, done, total);
}
...
// Diagnostic-only: what our own libc thinks this file's size is, independent
// of whatever StormLib's internal fstat64 call came up with. StormLib reports
// ERROR_BAD_FORMAT (seen on the console as "error 1000") for any file under
// 32 bytes - including a perfectly good multi-gigabyte MPQ, if the size it
// read back was wrong rather than the file actually being that small. This
// says which of those it is, without needing another console round trip.
static std::string diagnosticFileSize(const std::string& path) {
    std::string out;
    std::error_code ec;
    const auto size = fs::file_size(path, ec);
    out += ec ? "stat failed (" + ec.message() + ")" : "stat says " + std::to_string(size) + " byte(s)";
    // Second opinion that does not go through struct stat at all: the
    // seek-to-end offset. If the two disagree, the struct is the problem;
    // if both are zero, the file on the console really is empty.
    if (std::FILE* f = std::fopen(path.c_str(), "rb")) {
        long long endOffset = -1;
        if (std::fseek(f, 0, SEEK_END) == 0) endOffset = static_cast<long long>(std::ftell(f));
        std::fclose(f);
        out += endOffset >= 0 ? ", seek-to-end says " + std::to_string(endOffset) + " byte(s)"
                              : ", seek-to-end failed";
    } else {
        out += ", fopen(rb) failed";
    }
...
// Forwards StormLib's archive-open trace into Options::diagnostic, bounded so
// a scan over eighteen archives cannot flood the log.
static const std::function<void(const std::string&)>* g_openTraceSink = nullptr;
static int g_openTraceLinesLeft = 0;
static void forwardOpenTrace(const char* message) {
    if (!g_openTraceSink || g_openTraceLinesLeft <= 0) return;
    --g_openTraceLinesLeft;
    (*g_openTraceSink)(std::string("stormlib: ") + message);
}
...
// What plain stdio sees in the archive, independent of StormLib: the first
// bytes (an MPQ starts with "MPQ\x1a"), the size by seeking to the end, and
// the last 32 bytes. If these look right while StormLib still refuses the
// file, the fault is in StormLib's own path, not in the console's file I/O.
static void probeArchiveWithStdio(const std::string& path, const Extractor::Options& opts) {
    if (!opts.diagnostic) return;
    std::FILE* f = std::fopen(path.c_str(), "rb");
    if (!f) { opts.diagnostic("probe " + path + ": fopen failed errno=" + std::to_string(errno)); return; }
    unsigned char head[32] = {0};
    const size_t headRead = std::fread(head, 1, sizeof(head), f);
    long long end = -1;
    if (std::fseek(f, 0, SEEK_END) == 0) end = static_cast<long long>(std::ftell(f));
    unsigned char tail[32] = {0};
    size_t tailRead = 0;
    if (end >= 32 && std::fseek(f, static_cast<long>(end - 32), SEEK_SET) == 0) tailRead = std::fread(tail, 1, sizeof(tail), f);
    const long long tell = static_cast<long long>(std::ftell(f));
    std::fclose(f);
    opts.diagnostic("probe " + path + ": head n=" + std::to_string(headRead) + " bytes=" + hexBytes(head, 16) +
                    " seek-end=" + std::to_string(end) + " tail n=" + std::to_string(tailRead) +
                    " bytes=" + hexBytes(tail, 16) + " tell-after=" + std::to_string(tell));
}
...
std::string Extractor::detectLocale(const std::string& mpqDir) {
    // User-facing client text uses an installed English pack whenever possible.
    // Retain the actual directory spelling for case-sensitive MPQ paths.
    return pipeline::englishFirstLocaleDirectory(mpqDir, kKnownLocales);
}
...
std::vector<std::string> Extractor::discoverArchives(const std::string& mpqDir,
...
bool Extractor::enumerateFiles(const Options& opts,
    ...
    if (opts.diagnostic) {
        probeArchiveWithStdio(archives.back(), opts);
        g_openTraceSink = &opts.diagnostic;
        g_openTraceLinesLeft = 60;
        SErrSetOpenTrace(&forwardOpenTrace);
    }
    struct StopTrace { ~StopTrace() { SErrSetOpenTrace(nullptr); g_openTraceSink = nullptr; } } stopTrace;
    for (auto it = archives.rbegin(); it != archives.rend(); ++it) {
        notifyProgress(opts, "Scanning archives", scanned++, archives.size());
        HANDLE hMpq = nullptr;
        if (!SFileOpenArchive(it->c_str(), 0, MPQ_OPEN_READ_ONLY, &hMpq)) {
            const uint32_t err = SErrGetLastError();
            const std::string sizeInfo = diagnosticFileSize(*it);
            std::cerr << "  Failed to open: " << *it << " (error " << err << ", " << sizeInfo << ")\n";
            if (opts.diagnostic) {
                opts.diagnostic("failed to open " + *it + " while scanning (error " +
                                 std::to_string(err) + "; " + sizeInfo +
                                 "); continuing with the other archives");
            }
            scanComplete = false;
            continue;
        }
    ...
    notifyProgress(opts, "Scanning archives", archives.size(), archives.size());
    std::cout << "Enumerated " << outFiles.size() << " unique files\n";
    if (!scanComplete && !outFiles.empty() && opts.diagnostic) {
        opts.diagnostic("one or more archives could not be opened for scanning; "
                         "extracting " + std::to_string(outFiles.size()) +
                         " file(s) found in the rest rather than none at all");
    }
    // A single unreadable archive (a partial FTP/USB copy, a permission
    // quirk on exactly one file among many) used to discard every file the
    // OTHER archives had already listed, turning one bad archive into a
    // total extraction failure with no per-archive detail anywhere the
    // console user could see (see Options::diagnostic above). What was
    // actually found is what decides success now; scanComplete only decides
    // whether to warn.
...
bool Extractor::run(const Options& opts) {
    ...
    struct CloseArchives {
        std::vector<SharedArchive>& archives;
        ~CloseArchives() { for (auto& archive : archives) if (archive.handle) SFileCloseArchive(archive.handle); }
    } closeArchives{sharedHandles};
    for (const auto& path : archives) {
        ...
        if (SFileOpenArchive(path.c_str(), 0, MPQ_OPEN_READ_ONLY, &h)) {
            ...
            const uint32_t err = SErrGetLastError();
            const std::string sizeInfo = diagnosticFileSize(path);
            std::cerr << "  Failed to open archive: " << path << " (error " << err << ", " << sizeInfo << ")\n";
            if (opts.diagnostic) {
                opts.diagnostic("failed to open " + path + " for extraction (error " +
                                 std::to_string(err) + "; " + sizeInfo +
                                 "); files only in that archive will be missing");
            }
        }
    }
    if (sharedHandles.empty()) {
        std::cerr << "Failed to open any archives for extraction\n";
        if (opts.diagnostic) opts.diagnostic("no archive could be opened for extraction");
        return false;
    }
    if (sharedHandles.size() < archives.size()) {
        // Degraded, not fatal: proceed with whatever opened. A file that
        // lived only in a missing/unreadable archive fails its own open a
        // few lines down (per-file, already non-fatal and logged) rather
        // than the entire run being abandoned over one bad archive among many.
        std::cerr << "  Opened " << sharedHandles.size()
                  << "/" << archives.size() << " archives; continuing with those\n";
        if (opts.diagnostic) {
            opts.diagnostic("opened " + std::to_string(sharedHandles.size()) + " of " +
                             std::to_string(archives.size()) +
                             " archives for extraction; continuing with what opened");
        }
    }
    ...
    auto workerFn = [&]() {
        ...
        while (!workerFailed.load(std::memory_order_relaxed)) {
            ...
            std::vector<uint8_t> data;
            {
                ...
                struct CloseFile {
                    HANDLE file;
                    ~CloseFile() { SFileCloseFile(file); }
                } closeFile{hFile};

                DWORD highSize = 0;
                DWORD fileSize = SFileGetFileSize(hFile, &highSize);
                if (fileSize == 0 && highSize == 0) {
                    stats.filesSkipped++;
                    continue;
                }
                if (fileSize == SFILE_INVALID_SIZE || highSize != 0) {
                    stats.filesFailed++;
                    std::cerr << "  FAILED invalid/unsupported MPQ file size: " << wowPath << "\n";
                    continue;
                }

                data.resize(fileSize);
                DWORD bytesRead = 0;
                if (!SFileReadFile(hFile, data.data(), fileSize, &bytesRead, nullptr) || bytesRead != fileSize) {
                    stats.filesFailed++;
                    if (failLogCount < 5) {
                        failLogCount++;
            ...
            std::error_code fileError; // Never share a mutable error_code between workers.
            fs::create_directories(outPath.parent_path(), fileError);
            if (fileError) {
                stats.filesFailed++;
                if (failLogCount++ < 5) std::cerr << "  FAILED mkdir: " << outPath.parent_path() << ": " << fileError.message() << "\n";
                continue;
            }

            const std::string temporary = fullOutputPath.string() + ".wow_ps.tmp";
            ...
            if (!out) {
                ...
                if (failLogCount++ < 5) std::cerr << "  FAILED write/close: " << fullOutputPath << "\n";
                continue;
            }
#ifdef _WIN32
            fs::copy_file(temporary, fullOutputPath, fs::copy_options::overwrite_existing, fileError);
            std::remove(temporary.c_str());
            const bool replaced = !fileError;
#else
            const bool replaced = std::rename(temporary.c_str(), fullOutputPath.string().c_str()) == 0;
#endif
            if (!replaced) {
                stats.filesFailed++;
                std::remove(temporary.c_str());
                if (failLogCount++ < 5) std::cerr << "  FAILED replace: " << fullOutputPath << "\n";
                continue;
            }
            ...
            // Counted in files handled (extracted, skipped or failed) so the
            // bar reaches the end whatever the archives held.
            const uint64_t handled = done + stats.filesSkipped.load() + stats.filesFailed.load();
            if (handled % 256 == 0 || handled >= totalFiles) {
                notifyProgress(opts, "Extracting files", std::min<uint64_t>(handled, totalFiles), totalFiles);
            }
        }
        } catch (const std::exception& e) {
            workerFailed.store(true, std::memory_order_relaxed);
            std::lock_guard<std::mutex> lock(manifestMutex);
            std::cerr << "  Extraction worker exception: " << e.what() << "\n";
        } catch (...) {
            workerFailed.store(true, std::memory_order_relaxed);
            std::lock_guard<std::mutex> lock(manifestMutex);
            std::cerr << "  Extraction worker unknown exception\n";
        }
    };

    std::cout << "Extracting " << totalFiles << " files using " << numThreads << " threads...\n";
    notifyProgress(opts, "Extracting files", 0, totalFiles);

    std::vector<std::thread> threads;
    try {
        for (int i = 0; i < numThreads; ++i) threads.emplace_back(workerFn);
    } catch (const std::exception& e) {
        workerFailed.store(true, std::memory_order_relaxed);
        std::cerr << "  Could not create extraction worker: " << e.what() << "\n";
    }
    for (auto& t : threads) {
        t.join();
    }
    ...
    if (workerFailed.load(std::memory_order_relaxed)) return false;
    ...
    std::cout << "\n  Extracted " << extracted << " files ("
              ...
              << skipped << " skipped, "
              << failed << " failed\n";
    // Keep the previous manifest/stamp and the first-launch pending marker
    // when a read/write failed, so next boot retries instead of accepting an
    // incomplete tree as a successful install.
    ...
    notifyProgress(opts, "Extracting files", totalFiles, totalFiles);

    // Merge with existing manifest so partial extractions don't nuke prior entries
    notifyProgress(opts, "Writing manifest", 0, 0);
    ...
    if (opts.verify) {
        ...
        for (const auto& entry : manifestEntries) {
            if ((verified + verifyFailed) % 256 == 0) {
                notifyProgress(opts, "Verifying files", verified + verifyFailed, manifestEntries.size());
            }
    ...
    if (opts.generateDbcCsv) {
        ...
        notifyProgress(opts, "Converting DBCs", 0, 0);
    ...
    if (opts.emitPng || opts.emitJsonDbc || opts.emitWom || opts.emitWob ||
        ...
        notifyProgress(opts, "Writing open formats", 0, 0);
        OpenFormatStats ofs;
    ...
    // Cache WoW.exe for Warden MEM_CHECK responses
    {
        ...
        for (const auto& dir : searchDirs) {
            ...
            for (const char* name : exeNames) {
                // Non-throwing throughout: this runs after the manifest is
                // written but before the pending marker is cleared, so an
                // exception here (a throwing exists() on the console, a failed
                // copy) would make every later boot re-extract the whole
                // client. The executable is only a Warden convenience.
                std::error_code exeEc;
                auto src = fs::path(dir) / name;
                if (fs::exists(src, exeEc) && !exeEc) {
                    ...
                    fs::create_directories(dstDir, exeEc);
                    auto dst = dstDir / name;
                    fs::copy_file(src, dst, fs::copy_options::overwrite_existing, exeEc);
                    if (exeEc) {
                        std::cerr << "  WoW.exe cache skipped: " << exeEc.message() << "\n";
                        found = true;
                        break;
                    }
                    ...
                    for (const char* companion : {
                        ...
                        if (!fs::exists(companionSrc, exeEc) || exeEc) continue;
                        const auto companionDst = dstDir / companion;
                        fs::copy_file(companionSrc, companionDst,
                                      fs::copy_options::overwrite_existing, exeEc);
                        if (exeEc) continue;
