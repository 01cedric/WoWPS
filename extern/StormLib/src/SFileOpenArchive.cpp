...
static bool OpenArchiveFromStream(TFileStream * pStream, HANDLE hParentMpq, DWORD dwPriority, DWORD dwFlags, HANDLE * phMpq)
{
    ...
    if(dwErrCode == ERROR_SUCCESS)
    {
        ...
        SErrOpenTrace("stream size=%llu", (unsigned long long)FileSize);
    ...
    if(dwErrCode == ERROR_SUCCESS)
    {
        ...
        while(bSearchComplete == false && ByteOffset < EndOfSearch)
        {
            ...
            if(!FileStream_Read(ha->pStream, &ByteOffset, pbHeaderBuffer, dwBytesAvailable))
            {
                ...
                SErrOpenTrace("header read @%llu n=%u FAILED err=%u", (unsigned long long)ByteOffset, (unsigned)dwBytesAvailable, (unsigned)dwErrCode);
                break;
            }
            if(ByteOffset == 0)
                SErrOpenTrace("header read @0 n=%u id=%08x hdrsize=%u", (unsigned)dwBytesAvailable,
                              (unsigned)BSWAP_INT32_UNSIGNED(((LPDWORD)pbHeaderBuffer)[0]),
                              (unsigned)BSWAP_INT32_UNSIGNED(((LPDWORD)pbHeaderBuffer)[1]));
            ...
            for(DWORD dwInBufferOffset = 0; dwInBufferOffset < dwBytesAvailable; dwInBufferOffset += 0x200)
            {
                ...
                if(dwHeaderID == g_dwMpqSignature && dwHeaderSize >= MPQ_HEADER_SIZE_V1)
                {
                    ...
                    SErrOpenTrace("convert @%llu -> %u fmt=%u hdrsize=%u sector=%u hash=%08x hi=%04x n=%u block=%08x hi=%04x n=%u hiblock=%llu het=%llu bet=%llu flags=%08x",
                                  (unsigned long long)ByteOffset, (unsigned)dwErrCode, (unsigned)((TMPQHeader *)ha->HeaderData)->wFormatVersion,
                                  (unsigned)((TMPQHeader *)ha->HeaderData)->dwHeaderSize, (unsigned)((TMPQHeader *)ha->HeaderData)->wSectorSize,
                                  (unsigned)((TMPQHeader *)ha->HeaderData)->dwHashTablePos, (unsigned)((TMPQHeader *)ha->HeaderData)->wHashTablePosHi,
                                  (unsigned)((TMPQHeader *)ha->HeaderData)->dwHashTableSize,
                                  (unsigned)((TMPQHeader *)ha->HeaderData)->dwBlockTablePos, (unsigned)((TMPQHeader *)ha->HeaderData)->wBlockTablePosHi,
                                  (unsigned)((TMPQHeader *)ha->HeaderData)->dwBlockTableSize,
                                  (unsigned long long)((TMPQHeader *)ha->HeaderData)->HiBlockTablePos64,
                                  (unsigned long long)((TMPQHeader *)ha->HeaderData)->HetTablePos64,
                                  (unsigned long long)((TMPQHeader *)ha->HeaderData)->BetTablePos64, (unsigned)ha->dwFlags);
        ...
        SErrOpenTrace("header search done err=%u complete=%d offset=%llu filesize=%llu", (unsigned)dwErrCode, (int)bSearchComplete,
                      (unsigned long long)ByteOffset, (unsigned long long)FileSize);
...
//      DumpMpqHeader(ha->pHeader);
    ...
    if(dwErrCode == ERROR_SUCCESS)
    {
        ...
        SErrOpenTrace("verify table positions -> %u (mpqpos=%llu sectorsize=%u)", (unsigned)dwErrCode,
                      (unsigned long long)ha->MpqPos, (unsigned)ha->dwSectorSize);
    }
    ...
    if(dwErrCode == ERROR_SUCCESS)
    {
        ...
        SErrOpenTrace("load hash/het table -> %u", (unsigned)dwErrCode);
    }
    ...
    if(dwErrCode == ERROR_SUCCESS)
    {
        ...
        SErrOpenTrace("build file table -> %u", (unsigned)dwErrCode);
    }
    ...
    // Cleanup and exit
    SErrOpenTrace("archive open result err=%u", (unsigned)dwErrCode);
    if(dwErrCode != ERROR_SUCCESS)
    {
    ...
    HANDLE * phMpq)
{
    ...
    SErrOpenTrace("SFileOpenArchive(%s) streamflags=%08x stream=%s lasterr=%u", szMpqName, (unsigned)dwStreamFlags,
                  pStream ? "ok" : "NULL", (unsigned)SErrGetLastError());
    if(pStream != NULL)
