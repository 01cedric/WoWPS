...
// wow_ps (PS4 port): optional trace of the archive-open path. When a sink is
// set, SFileOpenArchive reports every syscall result and verification step
// through it; with no sink the calls are a null check. See PS4-PATCHES.md.
typedef void (*SFILE_OPEN_TRACE)(const char * szMessage);
void  SErrSetOpenTrace(SFILE_OPEN_TRACE pfnTrace);
void  SErrOpenTrace(const char * szFormat, ...);
