...
static bool BaseFile_Open(TFileStream * pStream, LPCTSTR szFileName, DWORD dwStreamFlags)
{
...
#if defined(STORMLIB_MAC) || defined(STORMLIB_LINUX)
    {
        ...
        SErrOpenTrace("open(%s, flags=%d) -> fd=%ld errno=%d", szFileName, (int)(oflag | O_LARGEFILE), (long)handle, (int)errno);
        if(handle == -1)
        {
            pStream->Base.File.hFile = INVALID_HANDLE_VALUE;
            SErrSetLastError(errno);
        ...
        {
            SErrOpenTrace("fstat(fd=%ld) failed errno=%d", (long)handle, (int)errno);
            pStream->Base.File.hFile = INVALID_HANDLE_VALUE;
            SErrSetLastError(errno);
            close(handle);
            return false;
        }
        SErrOpenTrace("fstat(fd=%ld) size=%lld mode=%o", (long)handle, (long long)fileinfo.st_size, (unsigned)fileinfo.st_mode);
...
#ifdef __ORBIS__
        // PS4 (wow_ps): the toolchain's struct stat does not match what the
        // kernel writes (see ps4/compat/ps4_kstat.h) - through it, st_size
        // reads the block count, and every retail archive was rejected as
        // shorter than its own hash table. Read the size and time through the
        // kernel's real layout instead.
        {
            struct Ps4KernelStat kstat;
            if(ps4KernelFstat((int)handle, &kstat) == 0)
            {
                pStream->Base.File.FileSize = (ULONGLONG)kstat.st_size;
                pStream->Base.File.FileTime = 0x019DB1DED53E8000ULL + (10000000 * (ULONGLONG)kstat.st_mtim.tv_sec);
                SErrOpenTrace("kernel-layout fstat(fd=%ld) size=%lld mode=%o", (long)handle, (long long)kstat.st_size, (unsigned)kstat.st_mode);
            }
        }
#endif
        pStream->Base.File.hFile = (HANDLE)handle;
    ...
    DWORD dwBytesToRead)                    // Number of bytes to read from the file
{
...
#if defined(STORMLIB_MAC) || defined(STORMLIB_LINUX)
    {
        ...
        {
            off64_t seeked = lseek64((intptr_t)pStream->Base.File.hFile, (off64_t)(ByteOffset), SEEK_SET);
            SErrOpenTrace("lseek(fd=%ld, %llu) -> %lld errno=%d", (long)(intptr_t)pStream->Base.File.hFile,
                          (unsigned long long)ByteOffset, (long long)seeked, (int)errno);
            pStream->Base.File.FilePos = ByteOffset;
        }
        ...
        if(dwBytesToRead != 0)
        {
            ...
            {
                const unsigned char * p = (const unsigned char *)pvBuffer;
                SErrOpenTrace("read(fd=%ld, off=%llu, n=%u) -> %ld errno=%d first8=%02x%02x%02x%02x%02x%02x%02x%02x",
                              (long)(intptr_t)pStream->Base.File.hFile, (unsigned long long)ByteOffset,
                              (unsigned)dwBytesToRead, (long)bytes_read, (int)errno,
                              bytes_read >= 8 ? p[0] : 0, bytes_read >= 8 ? p[1] : 0, bytes_read >= 8 ? p[2] : 0, bytes_read >= 8 ? p[3] : 0,
                              bytes_read >= 8 ? p[4] : 0, bytes_read >= 8 ? p[5] : 0, bytes_read >= 8 ? p[6] : 0, bytes_read >= 8 ? p[7] : 0);
            }
            if(bytes_read == -1)
            {
                SErrSetLastError(errno);
