...
static SFILE_OPEN_TRACE g_pfnOpenTrace = NULL;

void SErrSetOpenTrace(SFILE_OPEN_TRACE pfnTrace)
{
    g_pfnOpenTrace = pfnTrace;
}
...
void SErrOpenTrace(const char * szFormat, ...)
{
    ...
    if(g_pfnOpenTrace == NULL)
        return;
    va_start(argList, szFormat);
    vsnprintf(szBuffer, sizeof(szBuffer), szFormat, argList);
    va_end(argList);
    szBuffer[sizeof(szBuffer) - 1] = 0;
    g_pfnOpenTrace(szBuffer);
}
