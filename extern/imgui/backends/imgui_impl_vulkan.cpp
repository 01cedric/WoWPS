...
#define IMGUI_PS4_BOOT(stage) wowee::platform::ps4::reportBootStage(stage)
#include "imgui_impl_vulkan_ps4_ndc.h"
#include "imgui_impl_vulkan_ps4_ndc_spirv.h"
#else
#define IMGUI_PS4_BOOT(stage) ((void)0)
#endif
#ifndef IM_MAX
...
static bool CreateOrResizeBuffer(VkBuffer& buffer, VkDeviceMemory& buffer_memory, VkDeviceSize& buffer_size, VkDeviceSize new_size, VkBufferUsageFlagBits usage)
{
    ...
    auto fail = [&](VkResult err) {
        check_vk_result(err);
        if (replacement) vkDestroyBuffer(v->Device, replacement, v->Allocator);
        if (memory) vkFreeMemory(v->Device, memory, v->Allocator);
        IMGUI_PS4_BOOT("gpu: overlay buffer growth failed; previous storage retained");
        return false;
    };
    VkDeviceSize aligned = AlignBufferSize(IM_MAX(v->MinAllocationSize, new_size), bd->BufferMemoryAlignment);
    VkBufferCreateInfo ci = {};
    ci.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
    ci.size = aligned;
    ci.usage = usage;
    ci.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
    VkResult err = vkCreateBuffer(v->Device, &ci, v->Allocator, &replacement);
    if (err != VK_SUCCESS) return fail(err);
    VkMemoryRequirements req = {};
    vkGetBufferMemoryRequirements(v->Device, replacement, &req);
    bd->BufferMemoryAlignment = IM_MAX(bd->BufferMemoryAlignment, req.alignment);
    VkMemoryAllocateInfo ai = {};
    ai.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    ai.allocationSize = req.size;
    ai.memoryTypeIndex = ImGui_ImplVulkan_MemoryType(VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, req.memoryTypeBits);
    if (ai.memoryTypeIndex == 0xFFFFFFFF) return fail(VK_ERROR_FEATURE_NOT_PRESENT);
    err = vkAllocateMemory(v->Device, &ai, v->Allocator, &memory);
    if (err != VK_SUCCESS) return fail(err);
    err = vkBindBufferMemory(v->Device, replacement, memory, 0);
    if (err != VK_SUCCESS) return fail(err);
    if (buffer) vkDestroyBuffer(v->Device, buffer, v->Allocator);
    if (buffer_memory) vkFreeMemory(v->Device, buffer_memory, v->Allocator);
    buffer = replacement;
...
static void ImGui_ImplVulkan_SetupRenderState(ImDrawData* draw_data, VkPipeline pipeline, VkCommandBuffer command_buffer, ImGui_ImplVulkan_FrameRenderBuffers* rb, int fb_width, int fb_height)
{
    ...
    // The default PS4 vertex shader consumes the already transformed upload.
    // ResetRenderState rebinds the same buffer and must not transform it again.
#ifdef WOWEE_PS4
    if (bd->VulkanInitInfo.CustomShaderVertCreateInfo.sType == VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO)
#endif
...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData* draw_data, VkCommandBuffer command_buffer, VkPipeline pipeline)
{
    if (draw_data == nullptr || !draw_data->Valid)
        return;
#ifdef WOWEE_PS4
    static unsigned boot_frames = 0;
    const bool boot_trace = boot_frames++ < 3 || wowee::platform::ps4::frameTraceEnabled();
#else
    const bool boot_trace = false;
#endif
    if (boot_trace) IMGUI_PS4_BOOT("gpu: overlay texture updates begin");
    // Avoid rendering when minimized, scale coordinates for retina displays (screen coordinates != framebuffer coordinates)
#ifdef WOWEE_PS4
    ImGui_ImplVulkan_PS4NdcTransform ndc_transform;
    int fb_width = 0, fb_height = 0;
    if (!ImGui_ImplVulkan_PS4MakeNdcTransform(draw_data->DisplayPos, draw_data->DisplaySize,
            draw_data->FramebufferScale, ndc_transform, fb_width, fb_height))
        return;
#else
    ...
    if (boot_trace) IMGUI_PS4_BOOT("gpu: overlay texture updates complete");
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
    ...
    if (draw_data->TotalVtxCount > 0)
    {
        if (boot_trace) IMGUI_PS4_BOOT("gpu: overlay vertex/index allocation begin");
        ...
        if (rb->VertexBuffer == VK_NULL_HANDLE || rb->VertexBufferSize < vertex_size)
            if (!CreateOrResizeBuffer(rb->VertexBuffer, rb->VertexBufferMemory, rb->VertexBufferSize, vertex_size, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT)) return;
        if (rb->IndexBuffer == VK_NULL_HANDLE || rb->IndexBufferSize < index_size)
            if (!CreateOrResizeBuffer(rb->IndexBuffer, rb->IndexBufferMemory, rb->IndexBufferSize, index_size, VK_BUFFER_USAGE_INDEX_BUFFER_BIT)) return;

#ifdef WOWEE_PS4
        if (rb->VertexBuffer == VK_NULL_HANDLE || rb->VertexBufferMemory == VK_NULL_HANDLE ||
            rb->IndexBuffer == VK_NULL_HANDLE || rb->IndexBufferMemory == VK_NULL_HANDLE)
        {
            IMGUI_PS4_BOOT("gpu: overlay upload buffers unavailable; draw skipped");
            return;
        }
#endif
        if (boot_trace) IMGUI_PS4_BOOT("gpu: overlay vertex/index allocation complete; map begin");
        ...
        check_vk_result(err);
#ifdef WOWEE_PS4
        if (err != VK_SUCCESS || vtx_dst == nullptr)
        {
            if (err == VK_SUCCESS) vkUnmapMemory(v->Device, rb->VertexBufferMemory);
            IMGUI_PS4_BOOT("gpu: overlay vertex mapping failed; draw skipped");
            return;
        }
#endif
        ...
        check_vk_result(err);
#ifdef WOWEE_PS4
        if (err != VK_SUCCESS || idx_dst == nullptr)
        {
            if (err == VK_SUCCESS) vkUnmapMemory(v->Device, rb->IndexBufferMemory);
            vkUnmapMemory(v->Device, rb->VertexBufferMemory);
            IMGUI_PS4_BOOT("gpu: overlay index mapping failed; draw skipped");
            return;
        }
#endif
        ...
        {
#ifdef WOWEE_PS4
            if (v->CustomShaderVertCreateInfo.sType != VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO)
            {
                if (!ImGui_ImplVulkan_PS4CopyNdcVertices(vtx_dst, draw_list->VtxBuffer.Data,
                        (size_t)draw_list->VtxBuffer.Size, ndc_transform))
                {
                    vkUnmapMemory(v->Device, rb->VertexBufferMemory);
                    vkUnmapMemory(v->Device, rb->IndexBufferMemory);
                    if (boot_trace) IMGUI_PS4_BOOT("gpu: overlay invalid vertex coordinates; draw skipped");
                    return;
                }
            }
            else
    ...
    if (boot_trace) IMGUI_PS4_BOOT("gpu: overlay vertex/index upload complete");
    // Setup desired Vulkan state
    ImGui_ImplVulkan_SetupRenderState(draw_data, pipeline, command_buffer, rb, fb_width, fb_height);

    if (boot_trace) IMGUI_PS4_BOOT("gpu: overlay pipeline and constants bound");
    ...
    for (const ImDrawList* draw_list : draw_data->CmdLists)
    {
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                if (desc_set == VK_NULL_HANDLE) continue;
                if (boot_trace) IMGUI_PS4_BOOT("gpu: overlay descriptor bind begin");
                if (desc_set != last_desc_set)
                    vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, bd->PipelineLayout, 0, 1, &desc_set, 0, nullptr);
                last_desc_set = desc_set;

                if (boot_trace) IMGUI_PS4_BOOT("gpu: overlay descriptor bound; indexed draw begin");
                // Draw
                vkCmdDrawIndexed(command_buffer, pcmd->ElemCount, 1, pcmd->IdxOffset + global_idx_offset, pcmd->VtxOffset + global_vtx_offset, 0);
                if (boot_trace) IMGUI_PS4_BOOT("gpu: overlay indexed draw recorded");
            }
        }
...
void ImGui_ImplVulkan_UpdateTexture(ImTextureData* tex)
{
    ...
    VkResult err;
#ifdef WOWEE_PS4
    static unsigned boot_textures = 0;
    const bool boot_trace = boot_textures++ < 8 || wowee::platform::ps4::frameTraceEnabled();
#else
    const bool boot_trace = false;
#endif
    if (boot_trace) IMGUI_PS4_BOOT("gpu: atlas update begin");
    ...
    if (tex->Status == ImTextureStatus_WantCreate)
    {
...
#ifdef WOWEE_PS4
            // The dynamic atlas writes rectangles as new glyphs are baked.
            // PS4 supports these transfers for linear RGBA8 images; the
            // optimal-image uploader only accepts complete mip subresources.
        ...
        if (boot_trace) IMGUI_PS4_BOOT("gpu: atlas image and view allocated");
    ...
    if (tex->Status == ImTextureStatus_WantCreate || tex->Status == ImTextureStatus_WantUpdates)
    {
        ...
        if (boot_trace) IMGUI_PS4_BOOT("gpu: atlas staging filled");
        ...
        if (boot_trace) IMGUI_PS4_BOOT("gpu: atlas image copy recorded");
        // End command buffer
        {
            ...
            check_vk_result(err);
            if (boot_trace) IMGUI_PS4_BOOT("gpu: atlas submit begin");
            err = vkQueueSubmit(v->Queue, 1, &end_info, VK_NULL_HANDLE);
            check_vk_result(err);
        }

        if (boot_trace) IMGUI_PS4_BOOT("gpu: atlas submit complete");
        ...
        if (boot_trace) IMGUI_PS4_BOOT("gpu: atlas ready");
    }
...
#ifdef WOWEE_PS4
        default_vert_info.codeSize = sizeof(__glsl_shader_vert_ps4_ndc_spv);
        default_vert_info.pCode = __glsl_shader_vert_ps4_ndc_spv;
#else
...
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    if (!bd || sampler == VK_NULL_HANDLE || image_view == VK_NULL_HANDLE)
        return VK_NULL_HANDLE;
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
    VkDescriptorPool pool = bd->DescriptorPool ? bd->DescriptorPool : v->DescriptorPool;
    if (!pool || !bd->DescriptorSetLayout)
        return VK_NULL_HANDLE;
    ...
    VkDescriptorSet descriptor_set = VK_NULL_HANDLE;
    {
        ...
        check_vk_result(err);
        if (err != VK_SUCCESS || descriptor_set == VK_NULL_HANDLE)
            return VK_NULL_HANDLE;
    }
