...
/// a binding that changes what the solve - or the sizing passes in front of it -
/// would produce and does not raise that counter leaves whatever the last pass
/// drew on the screen. Nine of these said nothing at all, Show and Hide among
/// them, which is why that gate could never be trusted on its own and why the
/// renderer carried a 15 Hz poll behind it. At 84 ms a frame the poll was due on
/// every single frame, so the interface being slow was exactly the condition
/// that turned off the guard against it being slow.
///
/// Named rather than written out at each site because there are twenty of them
/// and the twenty-first is the one that will be forgotten.
void markTreeDirty(lua_State* L) {
    if (auto* tree = wowee::addons::getWidgetTree(L)) tree->markLayoutDirty();
}
...
int lua_Region_SetScale(lua_State* L) {
    if (auto* w = widgetOf(L, 1)) {
        ...
        if (v > 0.0f && v != w->scale) {
            w->scale = v;
            // Every rect under this frame is measured in units this multiplies,
            // so the whole subtree is stale. Said through the generation
            // counter because that is what the renderer's layout gate reads -
            // see the note there: this binding writing the field and saying
            // nothing is one of the nine that made needsLayout untrustworthy
            // and kept a 15 Hz compatibility poll alive to cover for them.
            markTreeDirty(L);
        }
    }
    return 0;
...
int lua_ScrollFrame_SetScrollChild(lua_State* L) {
    ...
    if (auto* w = tree->get(id)) {
        ...
        if (w->scrollChild != childId) {
            w->scrollChild = childId;
            // A scroll child is placed by the frame that scrolls it and is
            // exempted from the "no anchors means not drawn" rule, so which
            // child it is changes what the solve produces. markScrollFrame
            // above sets isScrollFrame and says nothing either.
...
int lua_ScrollFrame_SetVerticalScroll(lua_State* L) {
    ...
    if (auto* w = tree->get(id)) {
        ...
        // The scroll child and everything in it moves with it, so the tree is
        // told for the same reason it is told about a scale.
        if (moved) {
            tree->markLayoutDirty();
            callScriptOnTable(L, 1, "OnVerticalScroll", clamped);
        }
    }
    return 0;
}
int lua_ScrollFrame_SetHorizontalScroll(lua_State* L) {
    ...
    if (auto* w = tree->get(id)) {
        ...
        if (moved) {
            tree->markLayoutDirty();
            callScriptOnTable(L, 1, "OnHorizontalScroll", clamped);
        }
    }
    return 0;
}
...
int lua_CheckButton_SetChecked(lua_State* L) {
    if (auto* w = widgetOf(L, 1)) {
        ...
        const bool on = lua_isnone(L, 2)    ? true
                      : lua_isnumber(L, 2)  ? (lua_tonumber(L, 2) != 0)
                                            : (lua_toboolean(L, 2) != 0);
        // The checked art is put in the draw order by the solve and left out
        // of it when the box is clear, so ticking a box is a change that pass
        // has to see. Compared first: BagSlotButton_UpdateChecked writes the
        // state it already has on every bag update.
        if (on != w->checked) { w->checked = on; markTreeDirty(L); }
    }
    return 0;
}
...
int lua_MessageFrame_AddMessage(lua_State* L) {
    ...
    // Recorded, not only flagged: the ageing pass reads the list rather than
    // looking for the flag on all 24,191 widgets once a frame. On the
    // transition only, so printing a line is not a list scan.
    if (!w->isMessageFrame) {
        w->isMessageFrame = true;
        if (auto* e = engineFrom(L)) e->noteMessageFrame(w->id);
    }
    // A message frame with nothing in it paints nothing and is left out of the
    // draw order; its first line is what puts it in.
    if (w->messages.empty()) markTreeDirty(L);
...
int lua_Tooltip_AddLine(lua_State* L) {
    auto* w = widgetOf(L, 1);
    if (!w) return 0;
    // A tooltip is sized from its lines, by a pass in front of the solve that
    // the layout gate skips with it - so a line added to a tooltip that has
    // not said anything is drawn outside the box that was measured for the
    // previous one.
    markTreeDirty(L);
...
static bool fillItemTooltipById(lua_State* L, wowee::ui::Widget* w,
                                game::GameHandler* gh,
                                uint32_t itemId);
...
static bool fillSpellTooltip(lua_State* L, wowee::ui::Widget* w,
                             game::GameHandler* gh, uint32_t spellId) {
...
int lua_Tooltip_SetAction(lua_State* L) {
    ...
    if (action.type == game::ActionBarSlot::SPELL) {
        lua_pushboolean(L, fillSpellTooltip(L, w, gh, action.id) ? 1 : 0);
        return 1;
    }
...
int lua_Tooltip_SetHyperlink(lua_State* L) {
    ...
    if (kind == "spell" || kind == "enchant" || kind == "talent")
        filled = fillSpellTooltip(L, w, gh, id);
    else if (kind == "item") {
...
int lua_Tooltip_SetGlyph(lua_State* L) {
    ...
    lua_pushboolean(L, spellId && fillSpellTooltip(L, w, gh, spellId) ? 1 : 0);
    return 1;
}
...
int lua_Tooltip_SetLFGCompletionReward(lua_State* L) {
    ...
    lua_pushboolean(L, itemId && fillItemTooltipById(L, w, gh, itemId) ? 1 : 0);
    return 1;
}
...
static void fillItemTooltip(lua_State* L, wowee::ui::Widget* w,
                            const game::ItemDef& item, game::GameHandler* gh) {
    w->isTooltip = true;
    markTreeDirty(L);
...
static bool fillItemTooltipById(lua_State* L, game::GameHandler* gh,
    ...
    if (lua_isfunction(L, -1)) {
        ...
        if (lua_pcall(L, 2, 1, 0) == 0) {
            ...
            lua_settop(L, top);
            if (ok) { w->shown = true; markTreeDirty(L); fireTooltipSetItem(L); return true; }
    ...
    if (fillItemTooltipById(L, w, gh, itemId)) {
...
int lua_Region_Show(lua_State* L) {
    ...
    if (w) {
        ...
        if (!w->shown && w->shownToggles < 200) {
            // On the first toggle only: the pass that reads the counter finds
            // the widget from this list rather than by sweeping the tree, and
            // one entry per widget per pass is all it needs.
            if (w->shownToggles == 0) {
                if (auto* e = engineFrom(L)) e->noteShownToggle(w->id);
            }
            ++w->shownToggles;
        }
        becameShown = !w->shown;
...
int lua_Region_Hide(lua_State* L) {
    ...
    if (auto* w = widgetOf(L, 1)) {
        ...
        // The subtree stops being drawn, which is the change the tree's own
...
int lua_Region_SetAlpha(lua_State* L) {
    if (auto* w = widgetOf(L, 1)) {
        const float a = static_cast<float>(luaL_optnumber(L, 2, 1.0));
        if (a != w->alpha) {
            w->alpha = a;
            // Alpha decides membership of the draw order, which the solve
            // gathers - so a frame faded to nothing and back is a layout
            // change here even though nothing moved.
            markTreeDirty(L);
        }
    }
    return 0;
...
int lua_Texture_SetTexture(lua_State* L) {
    ...
    if (!w) return 0;
    if (lua_isnumber(L, 2)) {
        // A colour where a path was: the region stops being drawn from a file
        // and stops being measured from one.
        if (!w->solidColor || !w->texturePath.empty()) markTreeDirty(L);
        w->solidColor = true;
        w->texturePath.clear();
        ...
        if (w->solidColor || w->texturePath != path) {
            w->solidColor = false;
            w->texturePath = path;
            // A texture that stated no size takes its image's, which the
            // renderer settles in the sizing pass in front of the solve - and
            // that pass is skipped along with the solve. Compared first
            // because the interface re-sets the path it already has on every
            // update of an action button.
            markTreeDirty(L);
        }
    }
    return 0;
...
int lua_Frame_SetFrameStrata(lua_State* L) {
    if (auto* w = widgetOf(L, 1)) {
        ...
        // The stratum is inherited down the subtree and is the first key the
        // draw order sorts on, so both have to be resolved again.
        markTreeDirty(L);
    }
    return 0;
}
...
int lua_Frame_SetFrameLevel(lua_State* L) {
    ...
    // else happened to dirty the tree.
    if (tree) tree->markLayoutDirty();
    return 0;
}
int lua_FontString_SetText(lua_State* L) {
...
int lua_FontString_SetFont(lua_State* L) {
    if (auto* w = widgetOf(L, 1)) {
        // A label with no width of its own is as wide as its own text in its
        // own face at its own size, and all three of those are settled before
        // the solve by a pass the layout gate skips with it.
        markTreeDirty(L);
...
static void applyFontObject(lua_State* L, int fontIndex, wowee::ui::Widget* w) {
    ...
    if (lua_istable(L, -1)) {
        // Same reason as SetFont: a font object carries the face and the size
        // a label is measured in.
        markTreeDirty(L);
...
int lua_FontString_SetTextHeight(lua_State* L) {
    if (auto* w = widgetOf(L, 1)) {
        ...
        if (h > 0.0 && static_cast<float>(h) != w->fontHeight) {
            w->fontHeight = static_cast<float>(h);
            markTreeDirty(L);
        }
    }
    return 0;
...
void populateRegionMethods(lua_State* L, bool isTexture, bool isFontString) {
    ...
    // Resolve Lua animation helpers at call time: the first region can be
    // created before the animation bootstrap is installed in this VM.
    set("CreateAnimationGroup", [](lua_State* vm) -> int {
        const int arguments = lua_gettop(vm);
        lua_getglobal(vm, "__WoweeCreateAnimationGroup");
        if (!lua_isfunction(vm, -1)) return luaL_error(vm, "Animation bootstrap is not ready");
        lua_insert(vm, 1); lua_call(vm, arguments, LUA_MULTRET);
        return lua_gettop(vm);
    });
    set("StopAnimating", [](lua_State* vm) -> int {
        const int arguments = lua_gettop(vm);
        lua_getglobal(vm, "__WoweeStopAnimating");
        if (!lua_isfunction(vm, -1)) return luaL_error(vm, "Animation bootstrap is not ready");
        lua_insert(vm, 1); lua_call(vm, arguments, LUA_MULTRET);
        return lua_gettop(vm);
    });
...
// One method set per region type and per Lua VM, not per texture/font string.
// Instance data stays on the region; ordinary assignment can still override
// any inherited method without modifying the other regions.
void installRegionMethods(lua_State* L, bool isTexture, bool isFontString) {
    const char* key = isTexture ? "wowps_texture_methods_mt" : "wowps_fontstring_methods_mt";
    lua_getfield(L, LUA_REGISTRYINDEX, key);
    if (!lua_istable(L, -1)) {
        lua_pop(L, 1);
        lua_newtable(L); // methods, with the existing known-method fallback
        populateRegionMethods(L, isTexture, isFontString);
        lua_newtable(L); // region metatable
        lua_pushvalue(L, -2);
        lua_setfield(L, -2, "__index");
        lua_pushvalue(L, -1);
        lua_setfield(L, LUA_REGISTRYINDEX, key);
        lua_remove(L, -2); // remove methods; keep region + metatable
    }
    lua_setmetatable(L, -2);
}
...
int lua_wowee_setAnimOffset(lua_State* L) {
    if (auto* w = widgetOf(L, 1)) {
        const float x = static_cast<float>(luaL_optnumber(L, 2, 0.0));
        const float y = static_cast<float>(luaL_optnumber(L, 3, 0.0));
        if (x != w->animOffsetX || y != w->animOffsetY) {
            w->animOffsetX = x;
            w->animOffsetY = y;
            // A running Translation moves the frame and everything anchored to
            // it, which is a layout change in every sense except that it does
            // not touch the anchors. Compared first because the animation tick
            // sets the same offset on every frame of a finished animation, and
            // marking the tree dirty for that would keep the solve running for
            // as long as anything in the interface had ever animated.
            markTreeDirty(L);
        }
    }
    return 0;
...
int lua_Frame_SetHitRectInsets(lua_State* L) {
    if (auto* w = widgetOf(L, 1)) {
        // The hit rect is derived during the solve, so a frame told to stop
        // answering clicks over part of itself keeps answering them until the
        // next pass.
        markTreeDirty(L);
...
int lua_Frame_SetClampedToScreen(lua_State* L) {
    if (auto* w = widgetOf(L, 1)) {
        ...
        // The clamp is applied by the solve, so turning it on has to make one
        // happen or the frame stays wherever it was dragged.
        if (on != w->clampedToScreen) { w->clampedToScreen = on; markTreeDirty(L); }
    }
    return 0;
}
...
int lua_Frame_SetBackdrop(lua_State* L) {
    ...
    if (!lua_istable(L, 2)) {          // SetBackdrop(nil) clears it
        // A frame with a backdrop paints and one without it does not, so this
        // decides membership of the draw order the solve gathers.
        if (w->hasBackdrop) markTreeDirty(L);
        w->hasBackdrop = false;
        return 0;
    }
    if (!w->hasBackdrop) markTreeDirty(L);
...
int lua_StatusBar_SetStatusBarTexture(lua_State* L) {
    if (auto* w = widgetOf(L, 1)) {
        ...
        // A status bar with no fill texture is dropped from the draw order,
        // so gaining one is a change the solve has to see.
        const std::string had = w->barTexture;
        if (lua_isstring(L, 2)) w->barTexture = lua_tostring(L, 2);
        else if (lua_istable(L, 2)) {
            if (auto* tex = widgetOf(L, 2)) w->barTexture = tex->texturePath;
        }
        if (w->barTexture != had) markTreeDirty(L);
    }
    return 0;
}
...
void LuaEngine::shutdown(bool reportErrors) {
    ...
    if (L_) {
        if (reportErrors) {
            try { reportMissingApi(); }
            catch (const std::bad_alloc&) {
                // Diagnostics must not prevent releasing the VM on Quit.
                // Do not allocate another log message while handling OOM.
            }
        }
        ...
        ui::frameXmlResetSession();
        lua_close(L_);
        L_ = nullptr;
        LOG_INFO("[LUA_MEMORY] peakKiB=", luaMemory_.peak / 1024,
                 " remainingBytes=", luaMemory_.used, " failures=", luaMemory_.failures);
        ...
        widgets_.reset();
        // Same reason as the ids below: every one of these names a widget, and
        // the next load hands those numbers to different frames. A message
        // frame left on the list would age a chat window that is now a button,
        // and a stale reported id would fire OnHide on it.
        visReported_.clear();
        visToggled_.clear();
        messageFrames_.clear();
        sizeSeedHi_ = 1;
        hoverWid_ = 0;
        lastClickWid_ = 0;
        lastClickTime_ = 0.0;
...
void LuaEngine::registerCoreAPI() {
    ...
    bootstrap(
        ...
        "function animMeta:GetParent() return self.group end\n"
        "function animMeta:SetParent(p)\n"
        "    if p == self.group then return end\n"
        "    assert(type(p) == 'table' and type(p.animations) == 'table', 'Animation parent must be an AnimationGroup')\n"
        "    if self.group then for i,a in ipairs(self.group.animations) do if a == self then table.remove(self.group.animations,i) break end end end\n"
        "    self.group = p; table.insert(p.animations, self)\n"
        "end\n"
        "function animMeta:GetRegionParent() return self.group and self.group.parent end\n"
        ...
        //
        // Regions use shared method tables; the animation forwarders resolve
        // these VM-local helpers at call time, including early-created regions.
    ...
    // "Do you really want to destroy X", which the real client does ask, and
    // this one did not. Letting a bag item go over the world is how an item is
    // destroyed in WoW, and the drop path here has always said the confirmation
    // was raised "once this dispatch returns" - and nothing raised it, so the
    // item silently stayed on the cursor and the next bag update put it back.
    // The pad path raises the same event, because dropping is the same gesture
    // there: the cursor is carried out of the window that owns it and let go.
    //
    // Written when it is shown rather than here, for the same reason as the
    // dialog above: staticpopup.lua assigns StaticPopupDialogs itself and loads
    // long after this. Both strings are the interface's own and both take the
    // item name through format, so this reads in the player's language.
    //
    // Nothing on cancel, deliberately. The item is still on the cursor and
    // still in its slot; answering No means keep carrying it, and clearing the
    // cursor there would be the client quietly deciding otherwise.
    bootstrap(
        "local deleteFrame = CreateFrame('Frame', '__WoweeDeleteItemConfirm')\n"
        "deleteFrame:RegisterEvent('WOWEE_DELETE_ITEM_CONFIRM')\n"
        "deleteFrame:SetScript('OnEvent', function(self, event, itemName)\n"
        "    if not StaticPopupDialogs or not StaticPopup_Show then return end\n"
        "    local name, quality = itemName or arg1, nil\n"
        "    if GetCursorInfo and GetItemInfo then\n"
        "        local kind, id = GetCursorInfo()\n"
        "        if kind == 'item' and id then\n"
        "            local got, _, q = GetItemInfo(id)\n"
        "            name = name or got\n"
        "            quality = q\n"
        "        end\n"
        "    end\n"
        "    name = name or (ITEMS or 'that item')\n"
        // A quality item gets the sterner of the two lines, which is the
        // difference the real client draws between a grey and a blue.
        "    local pattern = (quality and quality >= 2 and DELETE_GOOD_ITEM) or DELETE_ITEM\n"
        "    StaticPopupDialogs['WOWEE_DELETE_ITEM_CONFIRM'] = {\n"
        "        text = (pattern and format(pattern, name)) or ('Destroy ' .. name .. '?'),\n"
        "        button1 = YES or OKAY or 'Yes', button2 = NO or CANCEL or 'No',\n"
        "        OnAccept = function() if DeleteCursorItem then DeleteCursorItem() end end,\n"
        "        timeout = 0, showAlert = 1, hideOnEscape = 1,\n"
        "    }\n"
        "    StaticPopup_Show('WOWEE_DELETE_ITEM_CONFIRM')\n"
        "end)\n"
        "deleteFrame:Hide()\n");
...
///
/// Only the live part of the tree, which is where the saving is: a widget under
/// a closed panel is not solved at all, so its rect cannot move, and the sweep
/// that used to visit all 24,191 of them once a frame was comparing two floats
/// that had not changed since the panel shut.
///
/// One case is deliberately deferred rather than preserved. The rect of a
/// widget that is not on screen *can* still move, through resolveWidget, which
/// runs whenever a script measures a hidden frame - and FrameXML builds panels
/// that way, sizing them before showing them. The change is not lost by
/// skipping it here: lastReportedW is left alone with it, so the difference is
/// still there when the panel opens and the handler fires then, against the
/// size the frame actually appeared at. What is lost is a frame resized while
/// hidden and resized back before it is ever shown, which is a handler saying
/// nothing has changed.
void LuaEngine::updateSizeChanges() {
    if (!L_) return;
    const auto started = std::chrono::steady_clock::now();
    lastSizeChangeVisits_ = 0;

    // A frame is announced only once it has a size to compare against, and
    // "once" has to mean the same thing it did when this swept everything: the
    // first pass that saw the widget recorded whatever rect it had then and
    // said nothing. New ids are contiguous and the tree never reuses one, so
    // that first sighting is this, and it costs what was created rather than
    // what exists.
    for (uint32_t id = sizeSeedHi_; id < widgets_.size(); ++id) {
        if (auto* fresh = widgets_.get(id)) {
            if (fresh->id != 0 && fresh->lastReportedW < 0.0f) {
                fresh->lastReportedW = fresh->rectW;
                fresh->lastReportedH = fresh->rectH;
            }
        }
    }
    sizeSeedHi_ = static_cast<uint32_t>(widgets_.size());

    forEachLiveWidget(widgets_, sizeWalkStack_, [&](ui::Widget& w) {
        ++lastSizeChangeVisits_;
        if (w.kind != ui::WidgetKind::Frame) return;
        const bool known = (w.lastReportedW >= 0.0f);
        if (known && w.lastReportedW == w.rectW && w.lastReportedH == w.rectH) return;
        w.lastReportedW = w.rectW;
        w.lastReportedH = w.rectH;
        ...
        const uint32_t id = w.id;
        const float rw = w.rectW, rh = w.rectH;
        // Nothing below may touch `w`: the handler is free to create frames.
        lua_getglobal(L_, "__WoweeFramesByWid");
        if (!lua_istable(L_, -1)) { lua_pop(L_, 1); return; }
        ...
        if (lua_istable(L_, -1)) {
            ...
            if (lua_isfunction(L_, -1)) {
                lua_pushvalue(L_, -2);
                lua_pushnumber(L_, rw);
                lua_pushnumber(L_, rh);
    ...
    lastSizeChangeMs_ = std::chrono::duration<double, std::milli>(
                            std::chrono::steady_clock::now() - started).count();
}

/// Reconcile one widget against what it last reported, firing whatever it owes.
///
/// The body of the pass below, lifted out so the three sets it has to cover can
/// each hand it their widgets. Idempotent on purpose: it leaves the widget with
/// reportedVisible equal to visible and no toggles outstanding, so a second call
/// in the same pass - which reparenting mid-walk can produce, and which the
/// overlap between those sets certainly does - finds nothing left to do.
void LuaEngine::reconcileVisibility(uint32_t id) {
    auto* w = widgets_.get(id);
    if (!w || w->id == 0) return;
    ++lastVisibilityVisits_;
    ...
    if (w->visible == w->reportedVisible) {
        ...
        if (toggles < 2 || !w->visibleChain) return;
...
/// Three sets rather than every widget, and the three are exhaustive.
///
/// This was a loop from 1 to widgets_.size(), which is 24,191 on the original
/// interface, every frame, to find the handful of frames anything had happened
/// to. The whole body is a no-op for a widget that is not drawn, was not last
/// reported as drawn, and has no toggle outstanding - so those three conditions,
/// negated, are what has to be visited:
///
///   - drawn or running now: the live walk. Anything that has just appeared is
///     in it by definition, and so is everything the player can see.
///   - reported as drawn last time: visReported_. This is what catches a frame
///     that has *gone*, which the live walk by construction cannot. It cannot
///     drift, because reportedVisible is written here and nowhere else, and
///     because it is only ever set from `visible` - so a widget carrying it is
///     always one that was live when it was set.
///   - toggled by Show or Hide since: visToggled_, filled by those two
///     bindings. A hidden frame fires nothing, but its counter still has to be
///     cleared, or the pair would be replayed as a fresh rebuild whenever the
///     panel around it finally opens.
///
/// A widget in none of the three has visible false, reportedVisible false and
/// no toggles, which is exactly the case the old loop skipped.
///
/// `visible` here and `visibleChain` in the OnUpdate pump, which is a
/// deliberate difference rather than an oversight.
///
/// WoW fires OnShow on becoming shown-with-ancestors-shown, with no regard for
/// anchors, so visibleChain is the faithful answer. Using it would mean frames
/// that are never drawn start running their OnShow and OnHide, and those
/// handlers do real work - LootFrame_OnHide calls CloseLoot, which releases the
/// loot on the server, and UIParent's OnShow calls CloseAllWindows.
///
...
/// anywhere. None can ever be drawn or used, so none of their OnShow handlers is
/// worth the risk of waking the rest.
///
/// (UIParent looks like a seventh and is not: it carries setAllPoints.)
///
/// None of that is changed by walking rather than sweeping: the walk descends
/// on either flag, so an unanchored driver frame is still reached and still
/// reconciled against `visible`.
void LuaEngine::updateVisibility() {
    if (!L_) return;
    const auto started = std::chrono::steady_clock::now();
    lastVisibilityVisits_ = 0;
    // Taken and cleared before anything runs, the same way the text-change
    // queue is: a handler here is free to call Show, and that toggle belongs to
    // the next pass rather than to an extension of this one.
    std::vector<uint32_t> toggled;
    toggled.swap(visToggled_);
    std::vector<uint32_t> wasReported;
    wasReported.swap(visReported_);
    // Rebuilt from the walk rather than edited. Every widget that ends this
    // pass with reportedVisible set was assigned it from `visible`, and a
    // visible widget is a live one - so the live walk sees all of them and the
    // list cannot accumulate stale ids however panels come and go.
    forEachLiveWidget(widgets_, visWalkStack_, [&](ui::Widget& w) {
        const uint32_t id = w.id;
        reconcileVisibility(id);
        // Re-read: a handler may have grown the deque, and `w` is a reference
        // into it. (A deque keeps references valid, so this is belt and braces
        // against the widget having been reparented or reshown by its own
        // handler rather than against a dangling reference.)
        if (const auto* after = widgets_.get(id); after && after->visible) {
            visReported_.push_back(id);
        }
    });
    // What has gone. Everything still live was reconciled above and finds
    // nothing to do here; what dropped out of the tree's live region is only
    // reachable this way.
    for (uint32_t id : wasReported) reconcileVisibility(id);
    for (uint32_t id : toggled) reconcileVisibility(id);
    lastVisibilityMs_ = std::chrono::duration<double, std::milli>(
                            std::chrono::steady_clock::now() - started).count();
}
...
void LuaEngine::updateScrollRanges() {
    if (!L_) return;
    const auto started = std::chrono::steady_clock::now();
    lastScrollRangeWalks_ = 0;
    for (uint32_t id : widgets_.scrollFrames()) {
        auto* w = widgets_.get(id);
        if (!w) continue;
        // Only the ones on screen. scrollContentExtent walks the whole of a
        // scroll child's subtree and allocates a stack to do it, and this ran
        // it for every scroll frame in the interface once a frame - the quest
        // log's, the spellbook's, the auction house's and the rest, all of
        // them inside panels the player has closed. Nothing under a closed
        // panel is laid out, so its extent is whatever it was.
        //
        // Deferred rather than dropped, the same way the size pass defers:
        // reportedRangeX and Y are left alone with it, so a list that grew
        // while its panel was shut announces the new range when the panel
        // opens - which is when the scroll bar it drives is first drawn - and
        // GetVerticalScrollRange resolves on demand for anything that asks
        // sooner.
        if (!w->visible && !w->visibleChain) continue;
        ++lastScrollRangeWalks_;
    ...
    lastScrollRangeMs_ = std::chrono::duration<double, std::milli>(
                             std::chrono::steady_clock::now() - started).count();
}
...
void LuaEngine::releaseMouseHover() {
    ...
    if (widgets_.hoveredWidget() != 0 || widgets_.pressedWidget() != 0) {
        widgets_.markLayoutDirty();   // see the note in dispatchMouse
    }
...
void LuaEngine::dispatchMouse(float x, float y, float screenH, MouseButtons buttons) {
    ...
    // A button draws its highlight, its pushed art or neither depending on
    // these two, and which of its textures are in the draw order is decided by
    // the solve - so the cursor moving from one button to another is a layout
    // change even though nothing has moved.
    const uint32_t held = buttonDown_[0] ? pressedWid_[0] : 0;
    if (hit != widgets_.hoveredWidget() || held != widgets_.pressedWidget()) {
        widgets_.markLayoutDirty();
    }
    widgets_.setInteraction(hit, held);
            ...
            if (pressedWid_[i] != 0) {
                ...
                if (wasDragged) {
                    ...
                    if (dropOn != 0 && dropOn != draggingWid_) {
                        ...
                        // how it is destroyed, so it keeps the cursor and the
                        // interface's own confirmation is raised over it -
                        // reading the cursor, which is why clearing it first
                        // was the whole of "dragging a quest item onto the
                        // ground does nothing": the prompt looked, found an
                        // empty cursor, and asked about nothing, and the next
                        // bag update put the item back. An item off an action
                        // bar has no such prompt - the bar held a reference,
                        // not the item - so it still clears here, which is what
                        // takes it off the bar.
                        //
                        // The prompt itself is __WoweeDeleteItemConfirm, which
                        // this event reaches; its Accept calls DeleteCursorItem
                        // and its cancel leaves the item exactly where it is.
                        // For years this branch said the confirmation was
                        // "raised once this dispatch returns" and nothing
                        // raised it, so an item dragged out of a bag onto the
                        // ground stuck to the pointer instead.
                        if (onWorld && !cursorItemCameFromInventory()) {
                            ...
                            fireEvent("WOWEE_DELETE_ITEM_CONFIRM");
                        }
                    }
...
///
/// Over the frames that have taken a message, which the AddMessage binding
/// records as it marks them, rather than over every widget in the tree looking
/// for the flag. There are about a dozen: the chat windows, the two error
/// frames and the combat text. Ageing them is a handful of floats, and finding
/// them was 24,191 deque lookups a frame.
///
/// Every message frame, not only the ones on screen: a line that expired behind
/// a closed chat window has expired, and coming back to find it still lit would
/// be a hidden frame keeping time of its own.
void LuaEngine::expireMessages(float elapsed) {
    const auto started = std::chrono::steady_clock::now();
    for (uint32_t id : messageFrames_) {
        auto* w = widgets_.get(id);
    ...
    lastMessageAgeMs_ = std::chrono::duration<double, std::milli>(
                            std::chrono::steady_clock::now() - started).count();
}

/// The two registrations the once-a-frame passes depend on.
///
/// Both replace a whole-tree sweep for a flag with a list the writer keeps, so
/// both are correct only as long as every writer of the flag calls them. Show
/// and Hide are the only two that raise shownToggles, and AddMessage is the only
/// place isMessageFrame is set; the tests hold both of those facts.
void LuaEngine::noteShownToggle(uint32_t wid) {
    if (wid != 0) visToggled_.push_back(wid);
}

void LuaEngine::noteMessageFrame(uint32_t wid) {
    if (wid == 0) return;
    // Called only on the transition, so this is not a scan of the list on every
    // line printed - but a frame that has been marked and re-marked (a reload
    // that rebuilds the chat windows) must not be listed twice, or its lines
    // would age at double speed.
    if (std::find(messageFrames_.begin(), messageFrames_.end(), wid) ==
        messageFrames_.end()) {
        messageFrames_.push_back(wid);
    }
}
...
void LuaEngine::dispatchOnUpdate(float elapsed) {
    ...
    // Everything below, and its own message-ageing above excluded because that
    // is timed separately. The update half of the frame reported 17-25 ms with
    // nothing saying which of the passes in it that was.
    const auto onUpdateStart = std::chrono::steady_clock::now();
    struct Timed {
    ...
    } timed{onUpdateStart, lastOnUpdateMs_};
    lastOnUpdateCalls_ = 0;
    ...
    for (int i = 1; i <= count; i++) {
        ...
        // silently still.
        // Resolved once and kept for the rest of the iteration. This lookup
        // reads __wid out of the frame table and then indexes the tree, and it
        // was being done three times over for every OnUpdate frame on every
        // frame: here, again to have a name ready in case the handler ran out
        // of memory, and a third time to name it if the handler failed. The
        // widget is a pointer into a deque that never shrinks, so holding it
        // across the call is sound where holding a stack index would not be -
        // which is the reason the name used to be copied out in advance.
        const ui::Widget* frameWidget = widgetOf(L_, lua_gettop(L_));
        bool visible;
        if (const auto* uw = frameWidget) {
        ...
        if (lua_istable(L_, -1)) {
            ...
            if (lua_isfunction(L_, -1)) {
                ...
                ++lastOnUpdateCalls_;
                int updateResult=0;
                try { updateResult=pcallScript(L_, "OnUpdate", 2, hIdx); }
                catch (const std::bad_alloc&) {
                    // Off the widget rather than the Lua stack. An allocation
                    // failure unwinds through Lua's own frames and leaves its
                    // stack in no defined state, which is why the name used to
                    // be copied into a buffer before the call rather than read
                    // after it. The pointer answers the same question without
                    // paying for the copy on every frame that does not fail.
                    char note[220];
                    std::snprintf(note,sizeof(note),"FrameXML OnUpdate allocation failed: %s",
                                  frameWidget ? frameWidget->name.c_str() : "");
#ifdef WOWEE_PS4
                ...
                if (updateResult != 0) {
                    ...
                    const char* fname = (frameWidget && !frameWidget->name.empty())
                                            ? frameWidget->name.c_str() : "(unnamed)";
...
bool LuaEngine::executeSource(const std::string& code, const std::string& sourceName) {
    ...
    BudgetGuard guard(L_, chunkTimeoutMs_);
    int err = runChunk(L_, code.c_str(), code.size(), sourceName.c_str());
    if (err != 0) {
        const char* errMsg = lua_tostring(L_, -1);
        std::string msg = errMsg ? errMsg : "(unknown error)";
        lastError_ = msg;
