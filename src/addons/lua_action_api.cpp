...
/// A spell out of the local realm's published book, by slot or by id. Declared
/// here because the action bar needs its icon and it is defined further down,
/// beside the pickup that needs its id.
static bool localRealmSpell(lua_State* L, int bookSlot, uint32_t& id, std::string& icon);
...
static int lua_PickupAction(lua_State* L) {
    ...
    if (hadAction) {
        ...
        if (existing.type == game::ActionBarSlot::SPELL) {
            // The local realm's own artwork first, for the same reason
            // localRealmSpell exists: SpellIcon.dbc is keyed on the retail
            // spell id and a local catalog need not use one.
            std::string icon;
            uint32_t byId = existing.id;
            if (!localRealmSpell(L, 0, byId, icon) || icon.empty())
                icon = gh->getSpellIconPath(existing.id);
...
/// One spell out of the local realm's book, by the slot the bridge published
/// it at, or by its id when `bookSlot` is zero.
///
/// spellIdForCall answers from GameHandler's spell tabs, and a local realm has
/// none: its spells belong to the realm rather than to a server that sent them.
/// So a spell dragged out of the book in singleplayer resolved to nothing and
/// picked up nothing at all. The snapshot the bridge publishes is the same list
/// the spellbook is drawn from, which is what makes the slot mean the same
/// thing on both sides. Its icon comes with it, because SpellIcon.dbc is keyed
/// on the retail spell id and a local catalog need not use one.
static bool localRealmSpell(lua_State* L, int bookSlot, uint32_t& id, std::string& icon) {
    const int top = lua_gettop(L);
    bool found = false;
    lua_getglobal(L, "__WoWPSLocal");
    if (lua_istable(L, -1)) {
        lua_getfield(L, -1, "spells");
        if (lua_istable(L, -1)) {
            if (bookSlot > 0) {
                lua_rawgeti(L, -1, bookSlot);
            } else {
                // By id: the list is short and this runs once per pickup.
                lua_pushnil(L);
                while (lua_next(L, -2) != 0) {
                    lua_getfield(L, -1, "id");
                    const bool match = static_cast<uint32_t>(lua_tonumber(L, -1)) == id;
                    lua_pop(L, 1);
                    if (match) break;      // leaves key+value, value on top
                    lua_pop(L, 1);
                }
            }
            if (lua_istable(L, -1)) {
                lua_getfield(L, -1, "id");
                const auto entry = static_cast<uint32_t>(lua_tonumber(L, -1));
                lua_pop(L, 1);
...
static int lua_PickupSpell(lua_State* L) {
    ...
    uint32_t picked = spellIdForCall(L, gh);
    std::string icon;
    if (picked == 0 && lua_isnumber(L, 1)) {
        // Nothing on this server's books - so ask the local realm's, unless
        // the caller named the pet's, which is a list of its own and is empty
        // rather than absent.
        const char* book = lua_tostring(L, 2);
        if (!book || std::string(book) != "pet") {
            localRealmSpell(L, static_cast<int>(lua_tonumber(L, 1)), picked, icon);
        }
    }
    ...
    if (icon.empty()) icon = gh->getSpellIconPath(s_cursorId);
    wowee::ui::frameXmlSetCursorItem(icon);
    return 0;
}
...
static void pickupFromContainerSlot(lua_State* L, game::GameHandler* gh,
    ...
    // in it.
    const game::ItemSlot* itemSlot = gh->isLocalExploration() && bag==0
        ? gh->localBagSlot(slot-1) : containerItemSlot(inv, bag, slot);
...
static int lua_PickupContainerItem(lua_State* L) {
    ...
    if (cursorWireSlot(srcBag, srcSlot)) {
        if (gh->isLocalExploration()) {
            // Local bags keep their authoritative order; putting the cursor down
            // cancels the drag rather than sending an unhandled network move.
            clearCursorItem(L); return 0;
        }
...
void registerActionLuaAPI(lua_State* L) {
    ...
    // Item artwork for the local realm's bags, which carry a display id and no
    // path of their own - so every stack in them drew a question mark. Here for
    // the same reason as the bridge above: the handler that owns the
    // ItemDisplayInfo.dbc lookup is reachable from this state and not from the
    // bridge that publishes those bags, and this goes down with the state too.
    // The handler is asked for per call rather than captured, because it is set
    // on the state after the API is registered.
    ui::frameXmlSetItemIconResolver([L](uint32_t displayId) -> std::string {
        auto* gh = getGameHandler(L);
        return gh ? gh->getItemIconPath(displayId) : std::string{};
    });
}
