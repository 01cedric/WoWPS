...
#include "addons/interface_memory_budget.hpp"
...
namespace {
// Normal logs are buffered for frame-time reasons. Source-load checkpoints
// must survive a signal before the next flush, without changing per-frame I/O.
void frameXmlCheckpoint(const char* stage, const char* path = nullptr) {
#ifdef WOWEE_PS4
    char message[384];
    std::snprintf(message, sizeof(message), "FrameXML: %s%s%s", stage,
                  path ? " | " : "", path ? path : "");
    platform::ps4::reportBootStage(message);
#else
...
void AddonManager::scanAddons(const std::string& addonsPath) {
    ...
    std::error_code rec;
#ifdef WOWEE_PS4
    // The console has no usable CWD. Packaged addons are independent of the
    // writable extracted interface tree searched above.
    const fs::path packaged = core::resolveRelativeAssetPath("addons");
    if (fs::is_directory(packaged, rec)) roots.push_back(packaged);
#else
    ...
    for (const auto& path : interfaceSource_.addonManifests()) {
        auto toc = interfaceSource_.toc(path);
        if (!toc) continue;
        if (toc->isLoadOnDemand()) {
            if (lodSeen.insert(toc->addonName).second) { lodAddons_.push_back(*toc); ++loadOnDemand; }
        } else if (seen.insert(toc->addonName).second) addons_.push_back(*toc);
        LOG_WARNING("[FRAMEXML_MPQ] addon=", toc->addonName, " files=", toc->files.size());
    }
...
std::vector<std::string> AddonManager::deferredAddonGlobals() const {
    ...
    for (const TocFile& addon : lodAddons_) {
        ...
        for (size_t qi = 0; qi < queue.size(); ++qi) {
            ...
            const fs::path file = interfaceSource_.resolve(fs::path(addon.basePath), rel);
            if (file.empty()) continue;
            const auto sourceText = interfaceSource_.read(file.string());
            if (!sourceText) continue;
            const std::string& text = *sourceText;
...
bool AddonManager::beginOriginalInterfaceAttempt() {
    if (originalSessionArmed_) return true;
    ui::frameXmlSetActive(false);
#ifdef WOWEE_PS4
    const char* setting = std::getenv("WOWEE_LOAD_FRAMEXML");
    if (!setting || std::strcmp(setting, "1") != 0) return false;
    originalRecoveryPath_ = frameXmlRecoveryPath(core::getConfigRoot());
    const auto previous = readFrameXmlRecovery(originalRecoveryPath_);
    if (!previous.allowsAttempt())
        LOG_WARNING("[FRAMEXML_RECOVERY] mandatory retry after state=", previous.state,
                    " stage=", previous.stage, " readable=", previous.readable);
    if (!writeFrameXmlRecovery(originalRecoveryPath_, "loading", "manifest preflight / Lua bootstrap")) {
        LOG_WARNING("[FRAMEXML_RECOVERY] cannot persist diagnostic marker; continuing mandatory UI load");
    }
#endif
    originalSessionArmed_ = true;
#ifdef WOWEE_PS4
    const auto memory = platform::ps4::queryAvailableCpuMemory();
    if (!interfaceMemoryAvailable(memory.bytes, memory.measured, true)) {
        LOG_WARNING("[FRAMEXML_MEMORY] low CPU headroom; trimming caches and continuing mandatory load");
    }
#endif
    originalErrorBaseline_ = luaEngine_.luaErrorCount();
    // Resolve only the tiny manifest. Assets continue to stream from MPQs.
    std::error_code ec;
    fs::path dir(frameXmlDir_);
    if (!fs::is_directory(dir, ec))
        dir = interfaceSource_.resolve(dir.parent_path(), dir.filename().string());
    if (dir.empty() || !fs::is_directory(dir, ec)) dir = "mpq/Interface/FrameXML";
    const auto toc = interfaceSource_.toc(interfaceSource_.resolve(dir, "FrameXML.toc").string());
    if (!toc || toc->files.empty()) {
        failOriginalInterface("original FrameXML.toc missing, unreadable or empty");
        return false;
    }
    LOG_WARNING("[FRAMEXML_RECOVERY] original UI attempt armed; manifest=", dir.string());
    return true;
}

const char* AddonManager::missingOriginalInterfaceRoot() const {
    // Retail core roots; load-on-demand panels are deliberately not required.
    for (const char* name : {"UIParent", "PlayerFrame", "MainMenuBar", "ActionButton1",
                             "MinimapCluster", "GossipFrame", "QuestFrame", "GameMenuFrame",
                             "QuestLogFrame", "SpellBookFrame", "ContainerFrame1"})
        if (!luaEngine_.widgets().findByName(name)) return name;
    return nullptr;
}

void AddonManager::failOriginalInterface(const char* reason) {
    originalInterfaceLoaded_ = false;
    addonsLoaded_ = false;
    ui::frameXmlSetActive(false);
    LOG_ERROR("[FRAMEXML_REQUIRED] original UI unavailable: ", reason);
#ifdef WOWEE_PS4
    if (originalRecoveryPath_.empty()) originalRecoveryPath_ = frameXmlRecoveryPath(core::getConfigRoot());
    if (!writeFrameXmlRecovery(originalRecoveryPath_, "failed", reason))
        LOG_WARNING("[FRAMEXML_RECOVERY] failed marker write; existing armed marker retained");
    platform::ps4::setInputMenuNavigation(platform::ps4::MenuOwner::FrameXml, false);
#endif
    originalSessionArmed_ = false; // Never overwrite a failure with clean on shutdown.
}

bool AddonManager::completeOriginalInterfaceAttempt() {
    if (!originalInterfaceLoaded_) return false;
    if (const char* missing = missingOriginalInterfaceRoot()) {
        LOG_WARNING("[FRAMEXML_RECOVERY] missing critical root: ", missing);
        failOriginalInterface("missing critical retail UI root");
        return false;
    }
    if (luaEngine_.luaErrorCount() != originalErrorBaseline_) {
        failOriginalInterface("Lua error during original UI bootstrap/login");
        return false;
    }
#ifdef WOWEE_PS4
    if (!writeFrameXmlRecovery(originalRecoveryPath_, "active", "original UI session active")) {
        failOriginalInterface("cannot persist active session marker");
        return false;
    }
#endif
    ui::frameXmlSetActive(true);
    LOG_WARNING("[FRAMEXML_RECOVERY] original UI active; crash recovery remains armed");
    return true;
}

void AddonManager::finishOriginalInterfaceSession() {
    // Call only after lua_close and destruction of the widget tree returned.
    if (!originalSessionArmed_) return;
#ifdef WOWEE_PS4
    if (!writeFrameXmlRecovery(originalRecoveryPath_, "clean", "Lua/widget session shutdown complete"))
        LOG_WARNING("[FRAMEXML_RECOVERY] clean marker write failed; next boot stays native");
#endif
    originalSessionArmed_ = false;
}
...
void AddonManager::loadAllAddons() {
    ...
    if (loadIt && !beginOriginalInterfaceAttempt()) return;
    frameXmlCheckpoint("deferred globals begin");
    luaEngine_.declareDeferredGlobals(deferredAddonGlobals());
    frameXmlCheckpoint("deferred globals complete");
    if (loadIt) {
        ui::frameXmlSetActive(true);
        ...
        // mute.
        // Upstream initializes screen dimensions before FrameXML OnLoad
        // handlers query anchored rectangles. Use our existing full layout
        // pass so both the screen and UIParent are already valid as well.
        if (luaServices_.window) {
            LOG_WARNING("[FRAMEXML_STAGE] initial layout begin; widgets=", luaEngine_.widgets().size());
            luaEngine_.widgets().layout(
                static_cast<float>(luaServices_.window->getWidth()),
                static_cast<float>(luaServices_.window->getHeight()));
            LOG_WARNING("[FRAMEXML_STAGE] initial layout complete");
        }
        {
            // Restore even if parsing, widget allocation or an event throws.
            // This flag is static and survives replacing the Lua state.
            struct RestoreUiSound {
                bool previous = LuaEngine::uiSoundsSuppressed();
                RestoreUiSound() { LuaEngine::setUiSoundsSuppressed(true); }
                ~RestoreUiSound() { LuaEngine::setUiSoundsSuppressed(previous); }
            } restoreUiSound;
            originalInterfaceLoaded_ = loadFrameXml(frameXmlDir_);
        }
        if (!originalInterfaceLoaded_ || missingOriginalInterfaceRoot() ||
            luaEngine_.luaErrorCount() != originalErrorBaseline_) {
            if (const char* missing = missingOriginalInterfaceRoot())
                LOG_WARNING("[FRAMEXML_RECOVERY] missing critical root: ", missing);
            failOriginalInterface("retail source load failed or required roots unavailable");
            return;
        }
...
#ifdef WOWEE_PS4
// Packaged addons live under the read-only /app0; their saved variables go
// to the writable config tree instead, where they actually persist.
static std::string ps4SavedVariablesDir() {
    const std::string dir = core::getConfigRoot() + "/saved_variables";
    std::error_code ec;
...
#ifdef WOWEE_PS4
    return ps4SavedVariablesDir() + "/" + addon.addonName + ".lua.saved";
#else
...
#ifdef WOWEE_PS4
    return ps4SavedVariablesDir() + "/" + addon.addonName + "." + characterName_ + ".lua.saved";
#else
...
bool AddonManager::loadFrameXml(const std::string& frameXmlDir) {
    frameXmlCheckpoint("manifest", frameXmlDir.c_str());
    LOG_WARNING("[FRAMEXML_STAGE] resolve manifest; root=", frameXmlDir);
    std::error_code ec;
    std::filesystem::path dir(frameXmlDir);
    if (!std::filesystem::is_directory(dir, ec)) {
        // The directory itself may be spelled differently on disk.
        dir = interfaceSource_.resolve(std::filesystem::path(frameXmlDir).parent_path(),
                          std::filesystem::path(frameXmlDir).filename().string());
    }
    if (dir.empty() || !std::filesystem::is_directory(dir, ec)) {
        dir = "mpq/Interface/FrameXML";
    }
    const std::filesystem::path tocPath = interfaceSource_.resolve(dir, "FrameXML.toc");
    auto toc = interfaceSource_.toc(tocPath.string());
    if (!toc) {
    ...
    if (!interfaceVersion && InterfaceSource::archived(dir.string()) && luaServices_.expansionRegistry) {
        if (const auto* profile = luaServices_.expansionRegistry->getActive())
            interfaceVersion = profile->majorVersion * 10000 + profile->minorVersion * 100 + profile->patchVersion;
    }
    ...
    if (const auto bindings = interfaceSource_.resolve(dir, "Bindings.xml"); !bindings.empty()) {
    ...
    bool reclaimedSourceCache = false;
    for (const auto& filename : toc->files) {
        ++sourceIndex;
        frameXmlCheckpoint("source preflight / GC", filename.c_str());
#ifdef WOWEE_PS4
        // A full Lua collection and kernel memory query for every one of the
        // ~139 sources dominated startup from 01.38 onward. Let Lua's normal
        // incremental collector work, and sample the expensive platform
        // counters only at bounded checkpoints.
        if (auto* L = luaEngine_.getState()) lua_gc(L, LUA_GCSTEP, 96);
        if ((sourceIndex & 7u) == 0u || sourceIndex == toc->files.size()) {
            const auto cpu = platform::ps4::queryAvailableCpuMemory();
            const auto heap = platform::ps4::heapGrowthStats();
            LOG_INFO("[FRAMEXML_MEMORY] source=", sourceIndex, "/", toc->files.size(),
                     " freeMiB=", cpu.bytes/(1024*1024), " measured=",cpu.measured,
                     " widgets=",luaEngine_.widgets().size(),
                     " luaKiB=",luaEngine_.getState()?lua_gc(luaEngine_.getState(),LUA_GCCOUNT,0):0,
                     " arenaMappedKiB=",heap.mappedBytes/1024," arenaExtensions=",heap.extensions,
                     " arenaLargestKiB=",heap.largestMapping/1024,
                     " arenaMapFailures=",heap.mappingFailures);
            if (!interfaceMemoryAvailable(cpu.bytes, cpu.measured, false) && !reclaimedSourceCache) {
                reclaimedSourceCache = true;
                const auto released = interfaceSource_.releaseSourceCache();
                if (auto* L = luaEngine_.getState()) lua_gc(L, LUA_GCCOLLECT, 0);
                LOG_WARNING("[FRAMEXML_MEMORY] reclaimed sourceBytes=", released,
                            "; continuing bounded allocations in reusable heap arenas");
            }
            // Unmapped flexible pages are not malloc's available capacity.
            // Do not synthesize bad_alloc while the retained heap can satisfy
            // requests. Per-source and Lua budgets plus real allocation errors
            // remain enforced, and the world loader owns failed-load recovery.
        }
#endif
        ...
        LOG_INFO("FrameXML: loading ", filename);
        std::string lower = filename;
        for (char& c : lower) c = static_cast<char>(std::tolower(static_cast<unsigned char>(c)));
        const std::filesystem::path resolved = interfaceSource_.resolve(dir, filename);
        if (resolved.empty()) {
...
bool AddonManager::loadXmlFile(const std::string& path, int depth) {
    frameXmlCheckpoint("XML read", path.c_str());
    ...
    auto sourceText = interfaceSource_.read(path);
    if (!sourceText) {
    ...
    ui::XmlNode root;
    std::string error;
    frameXmlCheckpoint("XML parse", path.c_str());
    if (!ui::parseXml(*sourceText, root, error)) {
        lastXmlError_ = "XML parse: " + error;
        LOG_ERROR("AddonManager: ", path, ": ", error);
        return false;
    }

    frameXmlCheckpoint("XML emit", path.c_str());
    ui::EmitResult emitted = ui::emitFrameXml(root);
    // The emitted Lua owns everything needed below. Release the XML source
    // and DOM before includes, compilation and live widget allocation overlap.
    root = ui::XmlNode{};
    sourceText.reset();
    ...
    auto sibling = [&](const std::string& rawName) {
        ...
        if (fs::path p = interfaceSource_.resolve(dir, name); !p.empty()) return p;
        ...
        if (!base.empty()) {
            if (fs::path p = interfaceSource_.resolve(base, fs::path(name).filename().string());
                !p.empty()) {
                return p;
            }
        }
    ...
    for (const auto& script : emitted.scriptFiles) {
        ...
        if (!interfaceSource_.exists(sibling(script).string())) {
        ...
        const bool loaded = isLua(script)
            ? executeSourceFile(sibling(script).string())
    ...
    if (!emitted.lua.empty()) {
        frameXmlCheckpoint("XML Lua compile/execute", path.c_str());
...
bool AddonManager::loadAddon(const TocFile& addon) {
    ...
    for (const auto& filename : addon.files) {
        ...
        const fs::path resolved = interfaceSource_.resolve(fs::path(addon.basePath), filename);
        const std::string fullPath =
            resolved.empty() ? (addon.basePath + "/" + filename) : resolved.string();

        if (lower.size() >= 4 && lower.substr(lower.size() - 4) == ".lua") {
            if (!executeSourceFile(fullPath)) {
...
void AddonManager::update(float deltaTime) {
    if (!addonsLoaded_) {
        // A script event can fail between updates. Dispose only here, after its
        // Lua stack unwound, never from inside a callback that owns that stack.
        if (!ui::frameXmlActive() && luaEngine_.luaErrorCount() != originalErrorBaseline_)
    ...
    try {
        if (originalInterfaceLoaded_ && (missingOriginalInterfaceRoot() ||
            luaEngine_.luaErrorCount() != originalErrorBaseline_)) {
            failOriginalInterface("original UI runtime state failed");
            discardPartialInterface();
            return;
        }
...
void AddonManager::saveAllSavedVariables() {
    // Merely scanning manifests in the main menu does not load their Lua.
    // Saving those absent globals would overwrite real settings on Quit.
    if (!addonsLoaded_) return;
    for (const auto& addon : addons_) {
        auto savedVars = addon.getSavedVariables();
        if (!savedVars.empty()) {
...
bool AddonManager::discardPartialInterface() {
    // Never save a half-built interface over the player's existing settings.
    originalInterfaceLoaded_ = false;
    ui::frameXmlSetActive(false);
    addonsLoaded_ = false;
    addons_.clear();
    ...
    luaEngine_.shutdown(false); // release first; error reports may allocate
    // A discarded VM must not trigger another discard on every native UI tick.
    originalErrorBaseline_ = luaEngine_.luaErrorCount();
    if (!luaEngine_.initialize()) return false;
    originalErrorBaseline_ = luaEngine_.luaErrorCount();
...
bool AddonManager::unloadAll() {
    ...
    luaEngine_.shutdown();
    finishOriginalInterfaceSession();
...
bool AddonManager::reload() {
    ...
    LOG_INFO("AddonManager: reload complete; loaded=", addonsLoaded_);
    return addonsLoaded_;
}
