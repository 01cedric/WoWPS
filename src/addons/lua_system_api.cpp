...
static void pushCvarDefault(lua_State* L, const std::string& n) {
    ...
    // What casts a shadow, and how big the map they cast into is. Answered from
    // the same constant the renderer reads at start-up, because it reads that
    // long before this file exists and the two saying different numbers is a
    // dropdown that shows one setting while the world draws another. It used to
    // answer 3 here against the renderer's 2.
    //
    // See rendering/shadow_quality.hpp for what the levels mean.
...
constexpr ClientCVarBinding kClientCVars[] = {
    ...
    // The three this client added itself. Bound rather than left as bare CVars
    // so the settings panel's row and SetCVar are the same number: without a
    // binding each keeps its own copy, and a player who moves the slider then
...
static void applyCVarSideEffects(lua_State* L, const std::string& key,
    ...
    // How far in from each screen edge the interface is kept.
    //
    // A television magnifies the picture slightly and loses whatever falls
    // outside its panel, so the corners are exactly where a set clips the
    // action bar, the chat frame and the minimap. This client had no margin at
    // all. Stated as a percentage in the interface and a fraction in the tree,
    // which is the shape the UI Scale slider already uses.
    if (key == "extsafearea") {
        if (auto* tree = getWidgetTree(L)) {
            tree->setSafeAreaInset(static_cast<float>(std::atof(value.c_str())) / 100.0f);
        }
    }
    // What casts a shadow. Only the scope travels this way: the map's own size
    // is fixed when its images are built at start-up, and the panel marks the
    // control as needing a restart for that half. Without this the dropdown
    // wrote the store and nothing else, so nothing moved until the next run -
    // including turning shadows off, which is the position a player picks when
    // the client is already too slow to wait for a restart.
    if (key == "extshadowquality") {
...
void applyStoredCVarSideEffects(lua_State* L) {
    ...
    // renderer has already read it out of the CVar file itself, because it is
    // built before any of this exists. So this is not what makes it work on a
    // fresh profile - it is what keeps the two readers from drifting. The
    // interface reloads several times in a session and replays this each time;
    // if the renderer's own start-up read is ever changed, the shipped default
    // is still stated in exactly one place and both paths reach it.
    //
    // No platform split, unlike the margin above: a television needs a margin
    // and a monitor does not, while shadow casters are wanted everywhere and
...
constexpr CVarRange kCVarRanges[] = {
...
#ifdef WOWEE_PS4
    {"farclip", rendering::ps4budget::kMinViewDistance, rendering::ps4budget::kMaxViewDistance},
#else
    ...
    // Title-safe margin, as a percentage of each screen edge. Not a shipped
    // setting: the original client ran on monitors, which lose nothing to
    // overscan. Ten percent is the tree's own ceiling, and past about five a
    // television is no longer the reason for the margin.
    {"extsafearea", 0.0f, 10.0f},
    // What casts a shadow. Five levels, and both ends are what
    // Renderer::setShadowQuality clamps to - so every position the control
    // offers is a setting the renderer will actually hold, rather than one it
...
void registerSystemLuaAPI(lua_State* L) {
    ...
    static const struct { const char* name; lua_CFunction func; } api[] = {
            ...
            // outside a session.
            auto* gh = getGameHandler(L);
            std::string path = "Interface\\TaxiFrame\\TaxiMap" +
                               std::to_string(gh ? gh->getCurrentMapId() : 0u);
            if (path != w->texturePath) {
                w->texturePath = std::move(path);
                // A texture with no size of its own takes the size of its
                // image, and the renderer settles that in the sizing pass its
                // layout gate skips - so swapping the path is a layout change.
                // The only Widget field this file writes; the chat-window
                // setters near SetChatWindowShown look like more of them and
                // are not - those are a settings record, not the tree.
