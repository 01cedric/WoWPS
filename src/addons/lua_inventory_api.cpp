...
uint32_t countItemInBags(game::GameHandler* gh, uint32_t itemId) {
    if (gh->isLocalExploration()) {
        uint32_t count=0;
        for (int i=0;i<24;++i) if (const auto* s=gh->localBagSlot(i); s && s->item.itemId==itemId) count+=s->item.stackCount;
        return count;
    }
...
static const game::ItemSlot* auctionSellItemSlot(game::GameHandler* gh,
    ...
    if (sel.bag == 0xFF) {
        ...
        if (gh->isLocalExploration()) {
            uint64_t handle=0;const auto* s=gh->localBagSlot(index, &handle);
            if (!s || (sel.localHandle && sel.localHandle!=handle)) return nullptr;
            if (guidOut) *guidOut=handle;
            return s;
        }
...
void registerInventoryLuaAPI(lua_State* L) {
    static const struct { const char* name; lua_CFunction func; } api[] = {
            ...
            if (gh->isLocalExploration()) {
                const double stacks = luaL_optnumber(L, 5, 1);
                if (!std::isfinite(stacks) || stacks < 1 || stacks > 24 || std::floor(stacks) != stacks ||
                    !stack || stack > 65535) {
                    gh->fireAddonEvent("UI_ERROR_MESSAGE", {"Invalid auction stack size or count"});
                    return 0;
                }
                if (!gh->submitLocalAuctionStacks(guid,uint16_t(stack),uint16_t(stacks),bid,buy,dur)) return 0;
            ...
            uint64_t handle=0;
            if (!auctionSellItemSlot(gh, &handle)) {
                auctionSellSlot() = previous;
                return 0;
            }
            if (gh->isLocalExploration()) auctionSellSlot().localHandle=handle;
            ...
            auto* gh = getGameHandler(L);
            // The local exchange is deposit-free. Never quote a charge that the
            // local authority does not debit. Network auction rules are unchanged.
            if (gh && gh->isLocalExploration()) {lua_pushnumber(L,0);return 1;}
