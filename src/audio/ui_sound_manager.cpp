...
bool UiSoundManager::playByName(const std::string& soundName) {
    ...
    return AudioEngine::instance().playSound2D(samplePlaybackSource(cached->second),
...
void UiSoundManager::playSound(const std::vector<UISample>& library) {
    ...
    AudioEngine::instance().playSound2D(samplePlaybackSource(library[0]), volume, 1.0f);
}
...
void UiSoundManager::playLevelUp() {
    // The gameplay event and legacy toast may both request this same ding.
    const auto now = std::chrono::steady_clock::now();
    if (lastLevelUpAt_.time_since_epoch().count() != 0 &&
        std::chrono::duration<float>(now - lastLevelUpAt_).count() < 0.2f) return;
    lastLevelUpAt_ = now;
    playSound(levelUpSounds_);
}
