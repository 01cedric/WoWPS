...
void ActivitySoundManager::resetWorldState() {
    stopSwimLoop();
    stopOneShot();
    swimmingActive = swimMoving = false;
    lastJumpAt = lastLandAt = lastSplashAt = lastMeleeSwingAt = {};
    lastAttackGruntAt = lastWoundAt = lastDeathAt = lastSwimStrokeAt = {};
}
...
void ActivitySoundManager::playDeath() {
    if (!initialized || deathClips.empty()) return;
    const auto now = std::chrono::steady_clock::now();
    if (lastDeathAt.time_since_epoch().count() != 0 &&
        std::chrono::duration<float>(now - lastDeathAt).count() < 2.0f) return;
    std::uniform_int_distribution<size_t> choice(0, deathClips.size() - 1);
    if (AudioEngine::instance().playSound2D(deathClips[choice(rng)].data, volumeScale))
        lastDeathAt = now;
}
