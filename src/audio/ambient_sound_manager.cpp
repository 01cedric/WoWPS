...
void AmbientSoundManager::resetWorldState() {
    emitters_.clear();
    activeSounds_.clear();
    // Preserve emitter ID monotonicity: a stale world handle must never name a new emitter.
    birdTimer_ = cricketTimer_ = windLoopTime_ = blacksmithLoopTime_ = 0.0f;
    weatherLoopTime_ = oceanLoopTime_ = zoneLoopTime_ = cityLoopTime_ = bellTollTime_ = 0.0f;
    wasIndoor_ = wasBlacksmith_ = wasSwimming_ = false;
    currentWeather_ = WeatherType::NONE;
    currentZoneId_ = 0;
    currentZone_ = ZoneType::NONE;
    currentCity_ = CityType::NONE;
    gameTimeHours_ = 12.0f;
}
        ...
        switch (emitter.type) {
            case AmbientType::FIREPLACE_SMALL:
                if (emitter.lastPlayTime >= FIRE_LOOP_INTERVAL && !fireSoundsSmall_.empty() && fireSoundsSmall_[0].loaded) {
                    ...
                    AudioEngine::instance().playSound3D(samplePlaybackSource(fireSoundsSmall_[0]), emitter.position, volume);
            ...
            case AmbientType::FIREPLACE_LARGE:
                if (emitter.lastPlayTime >= FIRE_LOOP_INTERVAL && !fireSoundsLarge_.empty() && fireSoundsLarge_[0].loaded) {
                    float volume = FIRE_VOLUME * volumeScale_ * (1.0f - (distance / maxDist));
                    AudioEngine::instance().playSound3D(samplePlaybackSource(fireSoundsLarge_[0]), emitter.position, volume);
                    emitter.lastPlayTime = 0.0f;
                }
                break;
            ...
            case AmbientType::TORCH:
                if (emitter.lastPlayTime >= FIRE_LOOP_INTERVAL && !torchSounds_.empty() && torchSounds_[0].loaded) {
                    ...
                    AudioEngine::instance().playSound3D(samplePlaybackSource(torchSounds_[0]), emitter.position, volume);
                    emitter.lastPlayTime = 0.0f;
                }
                break;
            ...
            case AmbientType::WATER_SURFACE:
                if (emitter.lastPlayTime >= 5.0f && !waterSounds_.empty() && waterSounds_[0].loaded) {
                    ...
                    AudioEngine::instance().playSound3D(samplePlaybackSource(waterSounds_[0]), emitter.position, volume);
                    emitter.lastPlayTime = 0.0f;
                }
                break;
            ...
            case AmbientType::RIVER:
                if (emitter.lastPlayTime >= 5.0f && !riverSounds_.empty() && riverSounds_[0].loaded) {
                    float volume = WATER_VOLUME * volumeScale_ * (1.0f - (distance / maxDist));
                    AudioEngine::instance().playSound3D(samplePlaybackSource(riverSounds_[0]), emitter.position, volume);
                    emitter.lastPlayTime = 0.0f;
                }
                break;
            ...
            case AmbientType::WATERFALL:
                if (emitter.lastPlayTime >= 4.0f && !waterfallSounds_.empty() && waterfallSounds_[0].loaded) {
                    ...
                    AudioEngine::instance().playSound3D(samplePlaybackSource(waterfallSounds_[0]), emitter.position, volume);
                    emitter.lastPlayTime = 0.0f;
                }
                break;
            ...
            case AmbientType::FOUNTAIN:
                if (emitter.lastPlayTime >= 6.0f && !fountainSounds_.empty() && fountainSounds_[0].loaded) {
                    ...
                    AudioEngine::instance().playSound3D(samplePlaybackSource(fountainSounds_[0]), emitter.position, volume);
                    emitter.lastPlayTime = 0.0f;
                }
                break;
    ...
    if (isDaytime() && currentZoneId_ != 10) {
        ...
        if (birdTimer_ >= randomFloat(BIRD_MIN_INTERVAL, BIRD_MAX_INTERVAL)) {
            ...
            if (!birdSounds_.empty()) {
                ...
                if (snd.loaded)
                    AudioEngine::instance().playSound2D(samplePlaybackSource(snd), BIRD_VOLUME, 1.0f);
            }
        }
    }
    ...
    if (isNighttime()) {
        ...
        if (cricketTimer_ >= randomFloat(CRICKET_MIN_INTERVAL, CRICKET_MAX_INTERVAL)) {
            ...
            if (!cricketSounds_.empty()) {
                ...
                if (snd.loaded)
                    AudioEngine::instance().playSound2D(samplePlaybackSource(snd), CRICKET_VOLUME, 1.0f);
            }
        }
    }
}
...
void AmbientSoundManager::updateBlacksmithAmbience(float deltaTime) {
    ...
    if (hasSound && blacksmithSounds_[0].loaded) {
        ...
        if (blacksmithLoopTime_ >= 15.0f) {
            ...
            AudioEngine::instance().playSound2D(samplePlaybackSource(blacksmithSounds_[0]), volume, 1.0f);
    ...
    if (isIndoor) {
        if (!tavernSounds_.empty() && tavernSounds_[0].loaded) {
            ...
            if (windLoopTime_ >= 15.0f) {
                ...
                if (AudioEngine::instance().playSound2D(samplePlaybackSource(tavernSounds_[0]), volume, 1.0f))
    ...
    // Outdoor wind ambience
        ...
        if (!windSounds_.empty() && windSounds_[0].loaded) {
            ...
            if (windLoopTime_ >= 30.0f) {
                ...
                AudioEngine::instance().playSound2D(samplePlaybackSource(windSounds_[0]), volume, 1.0f);
...
uint64_t AmbientSoundManager::addEmitter(const glm::vec3& position, AmbientType type, uint64_t owner) {
    // Retrying incomplete buildings must not register the same sound again.
    if (owner) for (const auto& existing : emitters_) {
        const auto delta = existing.position - position;
        if (existing.owner == owner && existing.type == type && glm::dot(delta, delta) < 0.0001f)
            return existing.id;
    }
...
void AmbientSoundManager::removeEmitters(uint64_t owner) {
    if (!owner) return;
    std::erase_if(emitters_, [owner](const auto& emitter) { return emitter.owner == owner; });
    // Already playing one-shots finish normally; no new sounds are scheduled.
    // Session teardown additionally stops all voices through AudioCoordinator.
}
...
void AmbientSoundManager::updateWeatherAmbience(float deltaTime, bool isIndoor) {
    ...
    if (weatherLibrary && !weatherLibrary->empty() && (*weatherLibrary)[0].loaded) {
        if (weatherLoopTime_ >= loopInterval) {
            ...
            AudioEngine::instance().playSound2D(samplePlaybackSource((*weatherLibrary)[0]), volume, 1.0f);
    ...
    if (isSwimming) {
        if (!underwaterSounds_.empty() && underwaterSounds_[0].loaded) {
            ...
            if (oceanLoopTime_ >= 18.0f) {
                float volume = 0.5f * volumeScale_;
                AudioEngine::instance().playSound2D(samplePlaybackSource(underwaterSounds_[0]), volume, 1.0f);
...
void AmbientSoundManager::updateZoneAmbience(float deltaTime, bool isIndoor) {
    ...
    if (zoneLibrary && !zoneLibrary->empty() && (*zoneLibrary)[0].loaded) {
        ...
        if (zoneLoopTime_ >= 30.0f) {
            ...
            AudioEngine::instance().playSound2D(samplePlaybackSource((*zoneLibrary)[0]), volume, 1.0f);
...
void AmbientSoundManager::updateCityAmbience(float deltaTime) {
    ...
    if (cityLibrary && !cityLibrary->empty() && (*cityLibrary)[0].loaded) {
        ...
        if (cityLoopTime_ >= 20.0f) {
            ...
            if (AudioEngine::instance().playSound2D(samplePlaybackSource((*cityLibrary)[0]), volume, 1.0f))
...
void AmbientSoundManager::updateBellTolls(float deltaTime) {
    ...
    if (bellLibrary && !bellLibrary->empty() && (*bellLibrary)[0].loaded) {
        if (bellTollTime_ >= bellInterval) {
            ...
            AudioEngine::instance().playSound2D(samplePlaybackSource((*bellLibrary)[0]), volume, 1.0f);
