...
void UIScreenCallbackHandler::setupCallbacks() {
    ...
    uiManager_.getCharacterScreen().setOnCharacterSelected([this](uint64_t characterGuid) {
        ...
        // A local character: the realm starts for its slot and enters the world.
        if (auto& app = Application::getInstance(); app.localCharacterFlowActive()) {
            app.enterLocalCharacter(characterGuid);
            return;
        }
