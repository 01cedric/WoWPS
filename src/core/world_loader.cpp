#include "core/transport_transfer_snapshot.hpp"
...
void WorldLoader::processPendingEntry() {
    ...
    try {
        ...
        LOG_ERROR("World load stopped: CPU allocation failed on map ", entry.mapId);
        failure = "There was not enough memory to load this area. Return to character selection and try again.";
    } catch (const std::exception& e) {
        LOG_ERROR("World load stopped on map ", entry.mapId, ": ", e.what());
        failure = "The world could not load. Check your game data and the loading log.";
    }
    if (failure) {
        // World loads run between frames. Finish any partially recorded
        // upload batch before resetting resources; clear pointers into a
        // failed tile's predecoded caches before their payload is released.
        if (renderer_) {
            if (auto* vk = renderer_->getVkContext()) {
                while (vk->isInUploadBatch()) vk->endUploadBatchSync();
            }
            if (auto* m2 = renderer_->getM2Renderer()) m2->setPredecodedBLPCache(nullptr);
            if (auto* wmo = renderer_->getWMORenderer()) {
                wmo->setPredecodedBLPCache(nullptr);
                wmo->setPredecodedNormalMapCache(nullptr, nullptr);
                wmo->setDeferNormalMaps(false);
            }
        }
        pendingWorldEntry_.reset();
        loadingWorld_ = false;
        app_.disconnectNotice_ = failure;
        app_.logoutToLogin();
    }
}
...
void WorldLoader::loadMapGeometry(uint32_t mapId, const std::string& mapName,
    ...
    if (isWMOOnlyMap) {
        ...
        if (terrainOk && renderer_->getTerrainManager() && renderer_->getCamera()) {
            ...
            const int savedUnloadRadius = renderer_->getTerrainUnloadRadius();
#ifdef WOWEE_PS4
            // Start with nearby tiles; retain the configured streaming radius
            // afterward instead of silently expanding it to169 tiles.
            terrainMgr->setLoadRadius(std::min(savedLoadRadius, 2));
#else
            terrainMgr->setLoadRadius(std::min(savedLoadRadius, 4));
#endif
            terrainMgr->setUnloadRadius(savedUnloadRadius);

            // Trigger tile streaming for surrounding area
            struct StationaryPreloadScope {
                decltype(terrainMgr) manager;
                ~StationaryPreloadScope() { manager->setStationaryPreload(false); }
            } stationaryPreload{terrainMgr};
            terrainMgr->setStationaryPreload(true);
            ...
            while (terrainMgr->getRemainingTileCount() > 0) {
                ...
                // PS4 advances one upload phase per iteration. A phase is
                // progress even while the completed tile count stays unchanged.
                const bool hadFinalizationWork = terrainMgr->hasFinalizationWork();
                ...
                if (remaining != lastRemaining || hadFinalizationWork) {
                ...
                if (remaining > 0 && !terrainMgr->hasFinalizationWork()) {
                    SDL_Delay(16);
                }
            }

            LOG_INFO("Initial terrain streaming ended: ", terrainMgr->getLoadedTileCount(),
                     " tiles loaded; unfinished=", terrainMgr->getRemainingTileCount());
...
void WorldLoader::loadOnlineWorldTerrain(uint32_t mapId, float x, float y, float z) {
    ...
    // Direct callers must also defer until the current render pass is done:
    // waiting for submitted work cannot retire an unsubmitted preview draw.
    if (loadingWorld_ || app_.renderingFrame_) {
        setPendingEntry(mapId, x, y, z);
        LOG_INFO("World load deferred before scene teardown: map=", mapId,
                 " loading=", loadingWorld_, " recording=", app_.renderingFrame_);
        return;
    }
    loadingWorld_ = true;
    struct LoadingGuard {
    ...
    } loadingGuard{loadingWorld_};
    pendingWorldEntry_.reset();

    std::optional<TransportTransferSnapshot> transportHandoff;
    if (app_.localRealm_ && gameHandler_ && gameHandler_->hasPendingPlayerTransportWorldTransfer()) {
        if (const auto* rider = app_.localRealm_->localPlayer())
            transportHandoff = captureTransportTransfer(*rider, mapId,
                app_.localRealm_->transports(), app_.localRealm_->travel().transportRoutes(),
                app_.localRealm_->players(), app_.localRealm_->npcs());
        if (transportHandoff)
            LOG_INFO("[TRANSPORT_TRANSFER] snapshot entry=", transportHandoff->hull.entry,
                " map=", mapId, " segment=", transportHandoff->hull.segment,
                " passengers=", transportHandoff->passengers.size(), " gpuHandles=0");
    }

    if (auto* ui = app_.getUIManager()) ui->releaseFrontendScenes();
    const size_t releasedFileBytes = assetManager_->trimFileCache(0);
    LOG_INFO("World entry memory: frontend 3D scenes released; raw asset cache freed_bytes=",
             releasedFileBytes);
    ...
    // The map's own loading picture when the archives are extracted; the
    // packaged image otherwise.
    loadingScreen.setBackgroundFromAssets(assetManager_, mapId);
    bool loadingScreenOk = loadingScreen.initialize();

#ifdef WOWEE_PS4
    int lastMemoryProgress = -1;
#endif
    auto showProgress = [&](const char* msg, float progress) {
#ifdef WOWEE_PS4
        // At most 21 samples per world transition, including terrain/instance
        // loops. This identifies the next allocation stage if a console's
        // installed assets require more headroom than the test fixtures.
        const int bucket = std::clamp(static_cast<int>(progress * 20.0f), 0, 20);
        if (bucket > lastMemoryProgress) {
            lastMemoryProgress = bucket;
            const auto memory = platform::ps4::queryAvailableCpuMemory();
            LOG_INFO("World load CPU: progress=", bucket * 5, "% stage=", msg,
                     " flexible_free_bytes=", memory.bytes, " measured=", memory.measured);
        }
#endif
        SDL_Event event;
        while (SDL_PollEvent(&event)) {
    ...
    if (loadedMapId_ != 0xFFFFFFFF || hasRendererData) {
        ...
        // Stop producers, then retire source actors while their renderer handles
        // are still valid. The authority retains passenger identities/offsets;
        // no source-map render or collision object survives this boundary.
        if (auto* terrain = renderer_->getTerrainManager()) terrain->stopWorkers();
        entitySpawner_->waitForAsyncLoads();
        if (auto* ctx = renderer_->getVkContext(); ctx &&
            !ctx->waitIdleAndDrainCleanup("map transfer: producers stopped")) {
            window_->setShouldClose(true);
            loadingScreen.shutdown();
            return;
        }
        entitySpawner_->resetAllState();
        app_.localRealmTransportGuids_.clear();
        LOG_INFO("[TRANSPORT_TRANSFER] source presentation and pending uploads retired");
        ...
        if (renderer_) {
            // Stop CPU workers before either they or their map-local caches
            // can observe render resources from the replacement map.
            if (auto* terrain = renderer_->getTerrainManager()) terrain->stopWorkers();
    ...
    // An occupied transport is required geometry, not a decoration to load
    // after nine terrain tiles. Prime it while the old map has been retired and
    // CPU headroom is still available. Never retain map-local renderer handles.
    if (transportHandoff && gameHandler_) {
        const auto& handoff = *transportHandoff; // Owns values across UI pumps.
        const auto& hull = handoff.hull;
        showProgress("Preparing transport...", 0.12f);
        if (renderer_->initializeRenderers(assetManager_, mapName)) {
            if (auto* tm = gameHandler_->getTransportManager()) {
                tm->setWMORenderer(renderer_->getWMORenderer());
                tm->setM2Renderer(renderer_->getM2Renderer());
                const uint64_t guid = 0xf1c0000000000000ULL | uint64_t(hull.entry);
                const auto pos = coords::serverToCanonical(glm::vec3(hull.x, hull.y, hull.z));
                const float yaw = coords::normalizeAngleRad(hull.orientation +
                    game::TransportManager::transportModelBowOffset(handoff.displayId));
                const auto& offset = handoff.localPlayer.offset;
                gameHandler_->resumeLocalTransport(hull.entry, mapId,
                    glm::vec3(offset[0], offset[1], offset[2]));
                entitySpawner_->primePassengerTransport(guid, hull.entry, handoff.displayId,
                    pos.x, pos.y, pos.z, yaw);
                tm->updateLocalTransportPose(guid, pos, yaw, hull.docked);
                const auto until = std::chrono::steady_clock::now() + std::chrono::seconds(4);
                while (entitySpawner_->hasPendingTransportVisuals(guid) &&
                       std::chrono::steady_clock::now() < until && !window_->shouldClose()) {
                    entitySpawner_->pumpTransportVisuals();
                    showProgress("Preparing transport...", 0.16f);
                }
                if (app_.audioCoordinator_) app_.audioCoordinator_->setTransportMusic(hull.entry,
                    handoff.displayId == 3031 || handoff.displayId == 7546, assetManager_);
                // Crew and other riders are republished from the same realm IDs
                // by syncLocalRealm after entry; no source-map actor is copied.
                LOG_INFO("[TRANSPORT_TRANSFER] destination hull primed entry=", hull.entry,
                    " map=", mapId, " passengers=", handoff.passengers.size(),
                    " pendingParts=", entitySpawner_->hasPendingTransportVisuals(guid));
            }
        }
    }
        ...
        while (true) {
            ...
            if (renderer_) {
                ...
                if (!groundReady && elapsed >= 5.0f) {
                    if (auto* tm = renderer_->getTerrainManager()) {
                        if (tm->isTileLoadedAt(rx, ry)) {
                            groundReady = true;
                            LOG_DEBUG("Warmup: spawn tile is loaded; accepting height offset after ", elapsed, "s");
                        }
                    }
                }
            ...
            if (readyToExit || elapsed >= kMaxWarmupSeconds) {
                if (elapsed >= kMaxWarmupSeconds && !groundReady) {
                    LOG_ERROR("World entry rejected: no loaded terrain or walkable instance floor at spawn (",
                              spawnRender.x, ",", spawnRender.y, ",", spawnRender.z, ")");
                    if (loadingScreenOk) loadingScreen.shutdown();
                    pendingWorldEntry_.reset();
                    loadingWorld_ = false;
                    app_.disconnectNotice_ = "The world could not load at your character's position. Check the game data and loading log.";
                    app_.logoutToLogin();
                    return;
    ...
    // A normal login is immediately playable. New characters may run their
    // authored race/class cinematic through Application after this load.
    if (renderer_->getCameraController()) {
        renderer_->getCameraController()->cancelIntroPan();
    }
    ...
    if (addonManager_ && !app_.addonsLoaded_) {
        const auto failRequiredInterface = [&](const char* reason) {
            app_.localFrameXml_.reset();
            if (assetManager_) assetManager_->trimFileCache(0);
            addonManager_->failOriginalInterface(reason);
            const bool reset = addonManager_->discardPartialInterface();
            app_.addonsLoaded_ = true; // attempted; a clean retry occurs on next world entry
            LOG_ERROR("[FRAMEXML_REQUIRED] original UI unavailable; native fallback disabled; vmReady=", reset);
        };
        try {
#ifdef WOWEE_PS4
            const auto before = platform::ps4::queryAvailableCpuMemory();
            const size_t reclaimed = assetManager_ ? assetManager_->trimFileCache(0) : 0;
            const auto after = platform::ps4::queryAvailableCpuMemory();
            LOG_WARNING("[FRAMEXML_MEMORY] begin beforeMiB=",before.bytes/(1024*1024),
                        " afterMiB=",after.bytes/(1024*1024)," reclaimedBytes=",reclaimed,
                        " measured=",after.measured);
            if (!addons::interfaceMemoryAvailable(after.bytes, after.measured, true)) {
                LOG_WARNING("[FRAMEXML_MEMORY] below preferred bootstrap reserve; "
                            "continuing because original UI is mandatory");
            }
#endif
        if (!addonManager_->beginOriginalInterfaceAttempt()) {
            failRequiredInterface("original UI preflight declined attempt");
            return;
        }
        app_.prepareLocalFrameXml();
        ...
        app_.localFrameXml_.activate(addonManager_->originalInterfaceLoaded());
        LOG_WARNING("[LOCAL_FRAMEXML] original interface loaded=",addonManager_->originalInterfaceLoaded(),
                    " local bridge active=",app_.localFrameXml_.ready());
        if (!addonManager_->originalInterfaceLoaded() ||
            (gameHandler_ && gameHandler_->isLocalExploration() && !app_.localFrameXml_.ready())) {
            failRequiredInterface("original UI source or local API bridge failed");
            return;
        }
        ...
        if (!addonManager_->completeOriginalInterfaceAttempt())
            failRequiredInterface("original UI login validation failed");
        } catch (const std::bad_alloc&) {
            failRequiredInterface("original UI memory budget/allocation failure");
        } catch (const std::exception& e) {
            LOG_ERROR("[FRAMEXML_REQUIRED] original UI exception: ", e.what());
            failRequiredInterface("original UI bootstrap exception");
        }
    } else if (addonManager_ && app_.addonsLoaded_ && addonManager_->originalInterfaceLoaded()) {
        // Never dispatch world events into an empty/partially discarded VM.
        addonManager_->fireEvent("PLAYER_ENTERING_WORLD");
    }
}
...
void WorldLoader::saveLastWorldInfo(uint32_t mapId, const std::string& mapName,
    ...
    std::string dir = home ? std::string(home) + "/.wowps" : ".";
#endif
    std::error_code ec;
