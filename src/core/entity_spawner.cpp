...
void EntitySpawner::update() {
    const char* stage = "player spawn";
#ifdef WOWEE_PS4
    // How much of a character the console can pay for this frame, asked of the
    // allocator rather than of the free-page count.
    //
    // The gate this replaces read sceKernelAvailableFlexibleMemorySize, saw
    // 44 MiB, admitted - and the next composite threw std::bad_alloc anyway.
    // The reading was not wrong; the question was. Free pages are sampled once
    // per [WORLD_PERF] window, a composite holds around five times its finished
    // size for the length of one blend-and-upload, and that peak is spent and
    // released between two samples. The asset log says the same thing from the
    // other side: twenty-four reads lost to allocation failure and *zero*
    // optional reads declined, because at 44 MiB nothing in the streaming
    // ladder declines anything.
    //
    // probeCompositeAdmission asks for the block that fails - one contiguous
    // 4 MiB atlas - and only then consults free pages, for the part of the
    // ledger a single block cannot speak for.
    //
    // Players and gameobjects are still processed at every rung - a missing
    // player is a player who is not there, and a missing gameobject can be a
    // door. Only the creature queue waits, and it waits rather than dropping,
    // so the crowd fills back in as soon as there is room.
    {
        // Not every frame, and not on a frame with nothing to spend it on.
        //
        // The probe is a malloc and a free of four megabytes, and on this
        // platform that is a flexible-memory map and unmap of a thousand pages
        // with the TLB shootdown behind it - not the few hundred nanoseconds
        // the same call costs on a desktop libc that would have served it from
        // a bin. Sixty of those a second, on the frames where the queues are
        // busiest and the frame time is already the complaint, is a cost the
        // decision does not need: the answer moves with the heap, and the heap
        // does not move measurably inside a tenth of a second.
        //
        // Between probes the previous answer stands. That is safe in a way it
        // would not have been before this round: every allocation the answer
        // governs is now individually fallible, so a stale Full costs a caught
        // failure and a plainer character rather than the session.
        const auto now = std::chrono::steady_clock::now();
        const bool creatureWorkPending =
            !pendingCreatureSpawns_.empty() || !asyncCreatureLoads_.empty() ||
            !asyncNpcCompositeLoads_.empty() || creatureSpawnHeld_;
        if (creatureWorkPending &&
            now - lastCompositeProbeAt_ >= std::chrono::milliseconds(100)) {
            compositeAdmission_ = platform::ps4::probeCompositeAdmission();
            lastCompositeProbeAt_ = now;
        }
        const bool holdNow =
            compositeAdmission_ == platform::ps4::CompositeAdmission::Defer;
        if (holdNow != creatureSpawnHeld_) {
            const auto cpu = platform::ps4::queryAvailableCpuMemory();
            LOG_WARNING("NPC skin compositing ", holdNow ? "deferred" : "resumed",
                        ": the heap ", holdNow ? "refused " : "returned ",
                        platform::ps4::compositeBytesFor(platform::ps4::kCompositeEdgeMin) / 1024,
                        " KiB in one block, flexible free=", cpu.bytes / (1024 * 1024),
                        " MiB measured=", cpu.measured);
        }
        creatureSpawnHeld_ = holdNow;
        // And the reserve of decoded skins waiting for spawns is cut to what
        // this rung pays for. Here rather than anywhere else because this is
        // the one point in the frame where nothing holds a borrowed pointer
        // into that map - applyCreatureDisplayTextures hands the renderer one
        // for the length of a call, and trimming under it would leave the
        // decoder reading a destroyed image.
        //
        // The reserve asks the free-page count, not the probe, and that
        // difference is the whole lesson of the two faults above: a composite
        // fails on one contiguous four-megabyte block, so it is asked for
        // directly; two dozen separately decoded images fail on the total, so
        // the total is what governs them. Same session, same pressure, two
        // questions.
        if (const size_t dropped = trimPredecodedSkins(
                platform::ps4::predecodedSkinReserveFor(compositeAdmission_))) {
            predecodedSkinsDropped_ += dropped;
        }
        // The renderer builds composites for the player and the character
        // preview as well as for this queue, so the limit is set on it rather
        // than passed down each call. Zero when there is room, which is the
        // same as never having had a limit.
        if (renderer_) {
            if (auto* charRenderer = renderer_->getCharacterRenderer()) {
                charRenderer->setCompositeEdgeLimit(
                    platform::ps4::rendererCompositeEdgeCapFor(compositeAdmission_));
            }
        }
    }
#endif
    ...
    stage = "creature spawn/upload";
    // Atlas admission governs optional compositing, not the creature body.
    processCreatureSpawnQueue();
    stage = "NPC texture composite";
    ...
    stage = "presentation asset retirement";
    reclaimUnusedPresentationAssets();
    } catch (const std::bad_alloc&) {
        LOG_ERROR("EntitySpawner allocation failure stage=", stage);
        throw;
    }
}
...
void EntitySpawner::reclaimUnusedPresentationAssets() {
    ...
    // This runs after upload/spawn work on the render thread. A newly uploaded
    // model may have no instance yet: the corresponding spawn or skin job is
    // queued for a later frame. Protect those users explicitly; only testing
    // instance counts would evict them between upload and first use.
    bool allocationFailed = false;
    try {
        preparingCharacterModels_.clear();
        const auto retainDisplay = [this](uint32_t displayId) {
            const auto it = displayIdModelCache_.find(displayId);
            if (it != displayIdModelCache_.end()) preparingCharacterModels_.insert(it->second);
        };
        for (const auto& spawn : pendingCreatureSpawns_) retainDisplay(spawn.displayId);
        for (const auto& load : asyncNpcCompositeLoads_) {
            if (load.modelId) preparingCharacterModels_.insert(load.modelId);
        }
        for (const auto& spawn : pendingPlayerSpawns_) {
            const uint32_t key = (uint32_t(spawn.raceId) << 8) | uint32_t(spawn.genderId);
            const auto it = playerModelCache_.find(key);
            if (it != playerModelCache_.end()) preparingCharacterModels_.insert(it->second);
        }
        if (pendingMountDisplayId_) retainDisplay(pendingMountDisplayId_);
        for (const auto& [guid, displayId] : pendingRemotePlayerMounts_) retainDisplay(displayId);

        // Active characters, weapons and effects are retained by the renderer.
        // GPU destruction remains deferred behind frame completion; this neither
        // waits for the device nor changes NPC simulation or visibility distance.
        charRenderer->reclaimUnusedResources(preparingCharacterModels_);
    } catch (const std::bad_alloc&) {
        // Retirement is optional work and may itself need a small bookkeeping
        // allocation. Reconcile any models already retired before the failure,
        // then retry on the next maintenance interval instead of ending play.
        allocationFailed = true;
    }

    size_t displaysRetired = 0;
    for (auto it = displayIdModelCache_.begin(); it != displayIdModelCache_.end();) {
        if (charRenderer->getModelData(it->second)) { ++it; continue; }
        displayIdTexturesApplied_.erase(it->first);
        releasePredecodedSkins(it->first);
        modelIdIsWolfLike_.erase(it->second);
        it = displayIdModelCache_.erase(it);
        ++displaysRetired;
    }
    for (auto it = playerModelCache_.begin(); it != playerModelCache_.end();) {
        if (charRenderer->getModelData(it->second)) { ++it; continue; }
        playerTextureSlotsByModelId_.erase(it->second);
        it = playerModelCache_.erase(it);
    }
    for (auto it = attachmentModelIds_.begin(); it != attachmentModelIds_.end();) {
        if (charRenderer->getModelData(it->second)) ++it;
        else it = attachmentModelIds_.erase(it);
    }
    size_t attachmentCopiesRetired = 0;
    for (auto it = attachmentModelData_.begin(); it != attachmentModelData_.end();) {
        if (!it->second) { ++it; continue; } // Keep cheap missing-file results until map change.
        const auto& path = it->first;
        const bool stillUsed = std::any_of(attachmentModelIds_.begin(), attachmentModelIds_.end(),
            [&path](const auto& model) {
                return model.first.size() > path.size() && model.first[path.size()] == '|' &&
                       model.first.compare(0, path.size(), path) == 0;
            });
        if (stillUsed) { ++it; continue; }
        it = attachmentModelData_.erase(it);
        ++attachmentCopiesRetired;
    }
    {
        // Expired weak references release payloads already, but their path
        // strings and control blocks must not grow with every visited zone.
        std::lock_guard<std::mutex> lock(creatureAssetCache_->mutex);
        for (auto it = creatureAssetCache_->models.begin(); it != creatureAssetCache_->models.end();) {
            if (it->second.expired()) it = creatureAssetCache_->models.erase(it);
            else ++it;
        }
    }
    if (!allocationFailed && (displaysRetired || attachmentCopiesRetired)) {
        LOG_INFO("[ENTITY_RESIDENCY] retiredDisplays=", displaysRetired,
                 " retiredAttachmentCopies=", attachmentCopiesRetired,
                 " preparing=", preparingCharacterModels_.size(),
                 " cachedDisplays=", displayIdModelCache_.size(),
                 " cachedAttachments=", attachmentModelData_.size());
    }
}

uint32_t EntitySpawner::cachedCreatureModelId(uint32_t displayId) {
    const auto it = displayIdModelCache_.find(displayId);
    if (it == displayIdModelCache_.end()) return 0;
    const auto* charRenderer = renderer_ ? renderer_->getCharacterRenderer() : nullptr;
    if (!charRenderer) return 0;
    if (charRenderer->getModelData(it->second)) return it->second;
    // Renderer cleanup can also happen at a world boundary or during pressure
    // recovery. A stale display id must trigger a reload, not repeated spawns
    // against a model which no longer exists.
    modelIdIsWolfLike_.erase(it->second);
    displayIdModelCache_.erase(it);
    displayIdTexturesApplied_.erase(displayId);
    releasePredecodedSkins(displayId);
    return 0;
}

namespace {
/// What one display's decoded skins occupy on the heap.
///
/// The vectors themselves, not approxUploadBytes(): this is a question about
/// the arena that runs out, and a block-compressed image holds its levels in
/// `mipmaps` where a decoded one holds RGBA8 in `data`. Either way the answer
/// is the bytes actually allocated.
size_t predecodedEntryBytes(
    const std::unordered_map<std::string, pipeline::BLPImage>& textures) {
    size_t bytes = 0;
    for (const auto& [path, image] : textures) {
        bytes += image.data.capacity();
        for (const auto& level : image.mipmaps) bytes += level.capacity();
    }
    return bytes;
}
} // namespace
...
void EntitySpawner::storePredecodedSkins(
    uint32_t displayId, std::unordered_map<std::string, pipeline::BLPImage> textures) {
    releasePredecodedSkins(displayId);
    if (textures.empty()) return;
    predecodedSkinBytes_ += predecodedEntryBytes(textures);
    displayIdPredecodedTextures_[displayId] = std::move(textures);
    predecodedSkinOrder_.push_back(displayId);
}

void EntitySpawner::releasePredecodedSkins(uint32_t displayId) {
    auto it = displayIdPredecodedTextures_.find(displayId);
    if (it == displayIdPredecodedTextures_.end()) return;
    const size_t bytes = predecodedEntryBytes(it->second);
    predecodedSkinBytes_ = bytes < predecodedSkinBytes_ ? predecodedSkinBytes_ - bytes : 0;
    displayIdPredecodedTextures_.erase(it);
    // Keep the age index bounded by resident entries, too. Otherwise frequent
    // insert/consume pairs could retain old ids for an entire trip while a
    // single unrelated entry kept the map nonempty and below its byte limit.
    predecodedSkinOrder_.erase(
        std::remove(predecodedSkinOrder_.begin(), predecodedSkinOrder_.end(), displayId),
        predecodedSkinOrder_.end());
}

size_t EntitySpawner::trimPredecodedSkins(size_t budgetBytes) {
    size_t released = 0;
    while (predecodedSkinBytes_ > budgetBytes && !predecodedSkinOrder_.empty()) {
        const uint32_t displayId = predecodedSkinOrder_.front();
        predecodedSkinOrder_.pop_front();
        auto it = displayIdPredecodedTextures_.find(displayId);
        if (it == displayIdPredecodedTextures_.end()) continue;  // already consumed
        released += predecodedEntryBytes(it->second);
        releasePredecodedSkins(displayId);
    }
    // Nothing left to account for, so nothing left to remember an order over.
    if (displayIdPredecodedTextures_.empty()) {
        predecodedSkinOrder_.clear();
        predecodedSkinBytes_ = 0;
    }
    return released;
}
...
#ifdef WOWEE_PS4
void EntitySpawner::logCompositeDegradation() {
    // Beside the asset manager's own degradation line and for the same reason:
    // it is the only record that a character the client could draw was drawn
    // with less on it. Zero on a healthy session, which is what makes a
    // non-zero one worth reading.
    if (!compositesHalved_ && !compositesPlain_ && !compositesDeferred_ &&
        !compositesLostToAllocation_ && !predecodedSkinsDropped_) {
        return;
    }
    LOG_WARNING("Character degradation: ", compositesHalved_,
                " composite(s) built at half size, ", compositesPlain_,
                " skin(s) bound without equipment, ", compositesDeferred_,
                " deferred for headroom, ", compositesLostToAllocation_,
                " lost to allocation failure, ", predecodedSkinsDropped_ / 1024,
                " KiB of decoded skin given back");
}
#endif
...
void EntitySpawner::shutdown() {
    ...
    // reached was held to process exit.
    trimPredecodedSkins(0);
    attachmentModelData_.clear();
    attachmentModelIds_.clear();
    playerModelCache_.clear();
    playerTextureSlotsByModelId_.clear();
    preparingCharacterModels_.clear();
}

void EntitySpawner::waitForAsyncLoads() {
    const auto join = [](auto& loads) {
        for (auto& load : loads) if (load.future.valid()) load.future.wait();
    };
    join(asyncCreatureLoads_);
    join(asyncEquipmentLoads_);
    join(asyncNpcCompositeLoads_);
    join(asyncGameObjectLoads_);
}
...
void EntitySpawner::resetAllState() {
    ...
    playerTextureSlotsByModelId_.clear();
    preparingCharacterModels_.clear();
    lastPresentationReclaimAt_ = {};
    // Carried over, this was the old world's decoded skins kept alive into the
    // new one - and with displayIdTexturesApplied_ cleared beside it, every one
    // of those displays was decoded a second time rather than found here. The
    // reserve is keyed by displayId, which is not stable across a world change.
    trimPredecodedSkins(0);
...
void EntitySpawner::queueGameObjectSpawn(uint64_t guid, uint32_t entry, uint32_t displayId,
    ...
    // Keep one request per GUID/generation, rather than uploading the same
    // zeppelin multiple times while its first asynchronous load is pending.
    for (auto& pending : pendingGameObjectSpawns_) {
        if (pending.guid != guid) continue;
        pending = {guid, entry, displayId, x, y, z, orientation, scale};
        return;
    }
    const uint64_t generation = gameObjectGenerations_[guid];
    for (const auto& load : asyncGameObjectLoads_)
        if (load.guid == guid && load.generation == generation) return;
    for (const auto& upload : pendingWmoUploads_)
        if (upload.result.guid == guid && upload.result.generation == generation) return;
...
void EntitySpawner::clearAllQueues() {
    ...
    creatureSpawnRetryWindowsUsed_.clear();
    ...
    { std::lock_guard<std::mutex> lock(creatureAssetCache_->mutex);
      creatureAssetCache_->models.clear(); }
    creatureUploadRetryAt_ = {};
...
std::string EntitySpawner::getHumanoidBakePath(uint32_t displayId) const {
    ...
    return pipeline::resolveNpcBakePath(extra->second.bakeName,
        [this](const std::string& path) { return assetManager_->fileExists(path); });
}
...
void EntitySpawner::applyCreatureDisplayTextures(uint32_t displayId, uint32_t modelId,
    ...
    //
    // The borrow is released through a guard rather than by the line at the
    // bottom, because everything between here and there can throw. Left set
    // through an unwind, the renderer holds a pointer into a map entry this
    // function is also the only thing that erases - so the entry survives, and
    // the reserve trim that now bounds it would later free the image the
    // renderer is still pointing at. processAsyncCreatureResults and
    // processAsyncNpcCompositeResults have held their borrowed caches this way
    // since they learned the same thing.
    ...
    if (dispData.extraDisplayId != 0) {
        ...
        if (itExtra != humanoidExtraMap_.end()) {
            ...
            charRenderer->requireModelBodyTexture(modelId, !skinSlots.empty());
            if (!charSectionsCacheBuilt_) buildCharSectionsCache();
            const std::string baseSkin = lookupCharSection(
                extra.raceId, extra.sexId, 0, 0, extra.skinId, 0);
            if (!baseSkin.empty()) {
                try {
                    auto* texture = charRenderer->loadTexture(baseSkin);
                    if (charRenderer->isUsableTexture(texture))
                        for (auto slot : skinSlots) charRenderer->setModelTexture(modelId, slot, texture);
                } catch (const std::bad_alloc&) {
                    // Keep the body pending; the bounded asynchronous path retries.
                }
            }
                ...
                [am, extraCopy, skinSlots = std::move(skinSlots),
                 hairSlots = std::move(hairSlots), modelId, displayId, isHdCharacterModel]() mutable -> PreparedNpcComposite {
                    ...
                    if (!extraCopy.bakeName.empty() && !isHdCharacterModel) {
                        def.bakedSkinPath = pipeline::resolveNpcBakePath(extraCopy.bakeName,
                            [am](const std::string& path) { return am->fileExists(path); });
                        def.hasBakedSkin = !def.bakedSkinPath.empty();
                        if (def.hasBakedSkin) allPaths.push_back(def.bakedSkinPath);
                    }
...
void EntitySpawner::spawnOnlineCreature(uint64_t guid, uint32_t displayId, float x, float y, float z, float orientation, float scale) {
    ...
    const uint32_t modelId = cachedCreatureModelId(displayId);
    if (modelId == 0) {
