...
bool Window::initialize() {
...
#ifdef WOWEE_PS4
    // No piglet, no EGL: ps4_vulkan is a real Vulkan 1.0 ICD over GNM/VideoOut
    // (see docs/ps4.md and ps4/third_party/ps4_vulkan/NOTICE.md), so
    // VkContext::initialize() below does the entire "window" setup itself
    // (instance/device/swapchain) - there is nothing to bootstrap first, at
    // the fixed 1080p VideoOut owns.
    width = 1920;
    height = 1080;
    windowedWidth = width;
    ...
    // The pad, before the UI manager asks padConnected() to advertise a
    // gamepad to ImGui. Not fatal: without it the client draws and the
    // on-screen keyboard reports the missing user.
    if (!platform::ps4::initInput()) {
        LOG_WARNING("PS4 pad not opened; the client has no input");
    }

    vkContext = std::make_unique<rendering::VkContext>();
    vkContext->setVsync(vsync);
    if (!vkContext->initialize(nullptr)) {
        LOG_ERROR("Failed to initialize Vulkan context");
        return false;
    }

    // Keep the splash until VkContext successfully presents the first frame.
    // Renderer setup and first-run extraction below can take substantial time.
    LOG_INFO("Window initialized successfully (PS4 Vulkan/GNM, ", width, "x", height, ")");
    return true;
#else
#ifdef __APPLE__
...
///
/// The build installs assets/WoWPS.png as a hicolor icon and writes a .desktop
...
void Window::setWindowIcon() {
    ...
    // No window, no icon.
#else
    static constexpr const char* kIconPath = "assets/WoWPS.png";
...
#ifdef WOWEE_PS4
    LOG_DEBUG("Window::shutdown - platform::ps4::shutdownInput...");
    platform::ps4::shutdownInput();
#else
...
#ifdef WOWEE_PS4
    // Always full screen on the console.
    (void)enable;
    return;
#else
    if (!window) return;
...
#ifdef WOWEE_PS4
    // The EGL surface is created once at the size initVideo settled on and
    // cannot be resized afterwards; the settings panel's choice is noted and
    // takes effect on the next launch through the saved config.
    if (w != width || h != height) {
        LOG_INFO("Resolution ", w, "x", h, " noted; the PS4 surface stays ",
                 width, "x", height, " until restart");
    }
    return;
#else
    if (!window) return;
