...
void AnimationCallbackHandler::setupCallbacks() {
    ...
    gameHandler_.setSpellCastAnimCallback([this](uint64_t guid, bool start, bool isChannel,
        ...
        if (start) {
            ...
            // Local starters are imported into a compact authority cache and
            // do not include professions/rest channels. Do not rebuild the
            // full Spell.dbc name cache merely to classify their animation.
            if (isLocalPlayer && currentSpell != 0 && !gameHandler_.isLocalExploration()) {
