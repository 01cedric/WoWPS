...
/// Give a frame the texture the client rendered into it, and tell the tree.
///
/// externalTexture is what decides whether a model frame is in the draw order
/// at all, so writing it silently leaves the paperdoll, the dressing room, the
/// pet and inspect models and the unit portraits blank until something else
/// happens to dirty the tree. That used to be hidden by a relayout forced
/// fifteen times a second - and that forced relayout is most of why the
/// interface cost sixty milliseconds a frame, so it is going away and these
/// writes have to announce themselves instead.
///
/// Compared before writing because these run every frame with the same handle:
/// marking unconditionally would hold a full solve open forever and give back
/// the whole saving.
void setExternalTexture(ui::WidgetTree& tree, ui::Widget* w, uint64_t handle) {
    if (!w || w->externalTexture == handle) return;
    w->externalTexture = handle;
...
bool Application::initialize() {
    ...
    // Create window
#ifdef WOWEE_PS4
    platform::ps4::reportBootStage("window_initialize");
#endif
    WindowConfig windowConfig;
    windowConfig.title = "WoWPS";
    ...
    // Create renderer
#ifdef WOWEE_PS4
    platform::ps4::reportBootStage("renderer_initialize");
#endif
    ...
    // Create UI manager
#ifdef WOWEE_PS4
    platform::ps4::reportBootStage("ui_initialize");
#endif
...
#ifdef WOWEE_PS4
    // The console reads the same extracted tree as the desktop, but nobody
    // ran extract_assets.sh for it: if the user's archives are under
    // WOW_DATA_PATH/Data and the tree is missing or older, build it now,
    // before the registry and the asset manager below look for it. The
    // window is up, so the system progress dialog overlays a black frame;
    // the system queue is pumped meanwhile. Fail-safe: on any error the
    // client goes on with what is on disk (and reports the missing data as
    // it would on the desktop).
    platform::ps4::reportBootStage("assets_prepare");
    // B4 test 7: the console streams from the archives (pipeline::MpqAssetSource);
    // nothing is extracted. Only the package's expansion metadata is mirrored
    // into the data root for the registry.
    platform::ps4::prepareAssetMetadata();
    platform::ps4::reportBootStage("assets_initialize");
#endif
    ...
    if (assetManager->initialize(assetPath)) {
        ...
        luaSvc.playEmoteAnimation = [this](const std::string& name) {
        ...
        luaSvc.isPlayerIndoors = [this]() -> bool {
            auto* r = renderer.get();
            return r && r->isPlayerIndoors();
        };
        luaSvc.getMinimapRotate = [this]() -> bool {
            auto* r = renderer.get();
            auto* mm = r ? r->getMinimap() : nullptr;
            return mm && mm->isRotateWithCamera();
        };
        luaSvc.setMinimapRotate = [this](bool rotate) {
            auto* r = renderer.get();
        ...
        luaSvc.listIconTextures = [am = assetManager.get()]() -> const std::vector<std::string>& {
            ...
            for (const auto& path : am->listFilesWithPrefix(kPrefix)) {
        ...
        if (addonManager_->initialize(gameHandler.get(), luaSvc)) {
            // Prefer an explicit loose interface override. Otherwise the UI
            // source provider reads FrameXML/AddOns from the mounted MPQs,
            // using AssetManager's existing patch and locale precedence.
            ...
            addonManager_->interfaceSource().setArchive(
                [am=assetManager.get()](const std::string& p) {
                    // Required UI is not optional scenery. Bound each read and
                    // bypass the raw cache without evicting the world's hot set
                    // for every Lua/XML include. InterfaceSource owns its cache.
                    return am->readFileBounded(p, addons::InterfaceSource::MaxSourceBytes, false);
                },
                [am=assetManager.get()](const std::string& p) { return am->fileExists(p); },
                [am=assetManager.get()](const std::string& p) { return am->listFilesWithPrefix(p); });
            ...
            // Wire spell icon path resolver for Lua API (GetSpellInfo, UnitBuff icon, etc.)
            {
                ...
                auto iconsLoaded = std::make_shared<bool>(false);
                auto ensureIcons = std::make_shared<std::function<void()>>(
                    [spellIconPaths, iconsLoaded, am]() {
                        if (!am || !am->isInitialized() || *iconsLoaded) return;
                        pipeline::loadSpellIconPaths(am, *spellIconPaths);
                        *iconsLoaded = true;
                    });
                auto ensureLoaded = std::make_shared<std::function<void()>>(
                    [spellIconPaths, spellIconIds, loaded, ensureIcons, am]() {
                ...
                gameHandler->setSpellIconPathResolver(
                    [this, spellIconPaths, spellIconIds, ensureLoaded](uint32_t spellId) -> std::string {
                ...
                gameHandler->setIconPathResolver(
                    [spellIconPaths, ensureIcons](uint32_t iconId) -> std::string {
                    if (iconId == 0) return {};
                    (*ensureIcons)();
            ...
            // Wire spell data resolver: spellId -> {castTimeMs, minRange, maxRange}
            {
                ...
                auto loaded = std::make_shared<bool>(false);
                auto* am = assetManager.get();
                gameHandler->setSpellDataResolver([this, castTimeMap, rangeMap, spellCastIdx, spellRangeIdx, spellCostMap, loaded, am](uint32_t spellId) -> game::GameHandler::SpellDataInfo {
                    if (gameHandler && gameHandler->isLocalExploration()) {
                        if (spellId == 6603) return {0,0,5.0f,0,0};
                        if (localRealm_) if (const auto* spell=localRealm_->content().spell(spellId)) {
...
#ifdef WOWEE_PS4
    // The first frames after the menu comes up are where a console crash has
    // been landing without a signal marker. A breadcrumb per frame for the
    // first few localizes it to the frame, and to before or after the
    // present, from boot.log alone.
    unsigned ps4FrameBreadcrumbs = 0;
    rendering::FrameTimingWindow ps4WorldTiming;
#endif
    while (running && !window->shouldClose()) {
        const auto frameStart = std::chrono::steady_clock::now();
#ifdef WOWEE_PS4
        const bool ps4StartedInWorld = state == AppState::IN_GAME;
#endif
        beatWatchdog();
#ifdef WOWEE_PS4
        if (ps4FrameBreadcrumbs < 6) {
            char stage[64];
            std::snprintf(stage, sizeof(stage), "main loop: frame %u begin", ps4FrameBreadcrumbs);
            platform::ps4::reportBootStage(stage);
        }
#endif
        ...
        ui::ageChatSlashEcho();
#ifdef WOWEE_PS4
        // No SDL on the console. The system queue (suspend/resume, the PS
        // button) and the pad are read once per frame here; the pad layer
        // turns the pad into the SDL-style key, text, mouse and wheel events
        // that the loop below drains through the compat queue
        // (platform/ps4/sdl_scancode_compat.h), so an edit box, a key
        // binding and the camera see a pad press exactly as they see a key.
        platform::ps4::pumpSystemEvents();
        platform::ps4::pumpInput();
#endif
        SDL_Event event;
        while (SDL_PollEvent(&event)) {
            ...
            // Cinematics own gameplay input; still allow window/quit events and
            // feed ImGui above so Circle/Escape can request a skip in the overlay.
            if (characterIntroOwnsView() &&
                (event.type == SDL_KEYDOWN || event.type == SDL_KEYUP ||
                 event.type == SDL_TEXTINPUT || event.type == SDL_MOUSEMOTION ||
                 event.type == SDL_MOUSEBUTTONDOWN || event.type == SDL_MOUSEBUTTONUP ||
                 event.type == SDL_MOUSEWHEEL)) {
                if (event.type == SDL_KEYDOWN && event.key.keysym.sym == SDLK_ESCAPE &&
                    event.key.repeat == 0) introSkipRequested_ = true;
                continue;
            }
...
#ifdef WOWEE_PS4
        const auto ps4UpdateStart = std::chrono::steady_clock::now();
#endif
        try {
            FrameMark;
            update(deltaTime);
        ...
        if (ps4FrameBreadcrumbs < 6) {
            char stage[64];
            std::snprintf(stage, sizeof(stage), "main loop: frame %u updated", ps4FrameBreadcrumbs);
            platform::ps4::reportBootStage(stage);
        }
#endif
        // Render
#ifdef WOWEE_PS4
        const auto ps4RenderStart = std::chrono::steady_clock::now();
#endif
        try {
            render();
        } catch (const std::bad_alloc& e) {
        ...
        if (ps4FrameBreadcrumbs < 6) {
            char stage[64];
            std::snprintf(stage, sizeof(stage), "main loop: frame %u rendered", ps4FrameBreadcrumbs);
            platform::ps4::reportBootStage(stage);
            ++ps4FrameBreadcrumbs;
        }
#endif
...
#ifdef WOWEE_PS4
        const auto ps4RenderEnd = std::chrono::steady_clock::now();
#endif
        ...
        if (inWorld && ps4StartedInWorld && renderer) {
            ps4WorldTiming.addFrame(frameStart, ps4UpdateStart, ps4RenderStart,
            ...
            if (ps4WorldTiming.ready()) {
                ...
                // over it; both halves of the interface's cost are read here.
                auto* fx = addonManager_ ? addonManager_->getLuaEngine() : nullptr;
                const auto* m2 = renderer->getM2Renderer();
                const auto* wmo = renderer->getWMORenderer();
                const auto* characters = renderer->getCharacterRenderer();
                ...
                if (localRealm_ && localRealm_->ready()) {
                    ...
                    for (const auto& peer : localRealm_->players())
                        if ((peer.guid & 0xffff000000000000ULL) == game::kLocalBotGuidPrefix) ++bots;
                    game::LocalCatalogIoStats io;
                    if (const auto& catalog = localRealm_->content().catalog) io = catalog->ioStats();
                    LOG_INFO("[LOCAL_PERF] bots=", bots, " intro=", characterIntroOwnsView(),
                             " buffering=", introBuffering_, " npcs=", localRealm_->npcs().size(),
                             " catalogDiskReads=", io.diskReads, " catalogCacheHits=", io.cacheHits,
                             " catalogCacheBytes=", io.cacheBytes);
                }
                LOG_INFO("[WORLD_PERF] loopFps=", ps4WorldTiming.fps(),
                    ...
                    // these the log said a frame took 100 ms and nothing about
                    // where it went. The two pre-passes run serially on the
                    // main thread before the workers start, so they add to the
                    // frame; the worker figures run concurrently with each
                    // other, so there the largest is the one that matters.
                    // These are last-frame samples, not window means.
                    " prepareMs=", renderer->getLastPrepareMs(),
                    " shadowMs=", renderer->getLastShadowPassMs(),
                    " reflectionMs=", renderer->getLastReflectionPassMs(),
                    " cameraMs=", renderer->getLastCameraUpdateMs(),
                    " terrainMs=", renderer->getLastTerrainRenderMs(),
                    ...
                    // render, and the rest was the widget tree. fxSolved
                    // against fxWidgets is the number that matters - a closed
                    // interface should solve a few hundred of twenty-four
                    // thousand, and fxLayoutSkipped should be 1 on an idle
                    // frame now that nothing forces a relayout.
                    " fxLayoutMs=", widgetRenderer_.lastLayoutMs(),
                    " fxSizingMs=", widgetRenderer_.lastSizingMs(),
                    " fxDrawMs=", widgetRenderer_.lastDrawMs(),
                    " fxLayoutSkipped=", widgetRenderer_.lastLayoutSkipped() ? 1 : 0,
                    " fxSolveMs=", fx ? fx->widgets().lastSolveMs() : 0.0,
                    " fxOrderMs=", fx ? fx->widgets().lastDrawOrderMs() : 0.0,
                    " fxSolved=", fx ? fx->widgets().lastSolvedWidgets() : size_t{0},
                    " fxWidgets=", fx ? fx->widgets().size() : size_t{0},
                    " fxVisMs=", fx ? fx->lastVisibilityMs() : 0.0,
                    " fxVisVisits=", fx ? fx->lastVisibilityVisits() : size_t{0},
                    " fxSizeMs=", fx ? fx->lastSizeChangeMs() : 0.0,
                    " fxMsgMs=", fx ? fx->lastMessageAgeMs() : 0.0,
                    " fxScrollMs=", fx ? fx->lastScrollRangeMs() : 0.0,
...
void Application::shutdown() {
    ...
    running = false; // No further frame or new gameplay work after exit begins.
    LOG_INFO("[SHUTDOWN] begin: stopping producers and saving realm");
    if (audioCoordinator_) audioCoordinator_->stopWorldAudio();
    if (worldLoader_) worldLoader_->cancelWorldPreload();
    if (renderer && renderer->getTerrainManager()) renderer->getTerrainManager()->stopWorkers();
    if (entitySpawner_) entitySpawner_->waitForAsyncLoads();
    stopLocalRealm(); // Joins the save writer and closes LAN before UI teardown.
    if (authHandler) authHandler->disconnect();
    // Despawn callbacks still need EntitySpawner and its live renderer here.
    if (gameHandler) gameHandler->disconnect();
    LOG_INFO("[SHUTDOWN] world workers, realm/save writer and networking stopped");
    if (renderer && renderer->getVkContext() &&
        !renderer->getVkContext()->waitIdleAndDrainCleanup("quit: producers stopped"))
        LOG_WARNING("[SHUTDOWN] GPU drain failed; device-loss cleanup path active");
    stopCharacterIntro(false);
    characterIntro_.reset();
    widgetRenderer_.releaseSessionTextures();
    localFrameXml_.reset();
    if (addonManager_) { addonManager_->shutdown(); addonManager_.reset(); }
    addonsLoaded_ = false;
    ...
    // Entity loading has its own futures, separate from the terrain workers.
    // Join them and release their results while assets and renderers still
    // exist, rather than waiting for member destruction after those owners.
    if (entitySpawner_) entitySpawner_->resetAllState();

    if (audioCoordinator_) audioCoordinator_->stopWorldAudio();

    // Networking and entity despawn callbacks were drained before GPU cleanup.
    worldLoader_.reset();
    entitySpawner_.reset();
    appearanceComposer_.reset();
    LOG_WARNING("[SHUTDOWN] world/entity producers and session helpers released");
    ...
    if (uiManager) {
        // Menu scenes and screen destructors retain renderer, audio and asset
        // pointers. Destroy them while those owners AND ImGui still exist.
        uiManager->releaseFrontendScenes();
        uiManager->shutdown();
        uiManager.reset();
        LOG_WARNING("[SHUTDOWN] UI/screens released before renderer/assets");
    }
...
void Application::setState(AppState newState) {
    ...
    switch (newState) {
        ...
        case AppState::IN_GAME: {
            if (audioCoordinator_) audioCoordinator_->beginWorldAudio();
...
void Application::reloadExpansionData() {
    localSpellImport_.reset(); // A changed profile/assets must import its own records.
...
void Application::logoutToLogin() {
    // Input is also pumped inside loading screens. Never destroy WorldLoader or
    // a renderer while one of their methods is still on the stack. The main
    // loop consumes this request only after update, rendering and presentation.
    if (!logoutToLoginPending_) LOG_INFO("[SESSION_RESET] logout queued for safe frame boundary");
    logoutToLoginPending_ = true;
}

void Application::processDeferredLogoutToLogin() {
    if (!logoutToLoginPending_ || renderingFrame_ ||
        (worldLoader_ && worldLoader_->isLoadingWorld())) return;
...
bool Application::rebuildSessionRenderer() {
    // This function is called between frames, after workers, the session, portraits
    // and frontend previews have stopped. Device/swapchain and UI fonts stay alive.
    auto* ctx = renderer ? renderer->getVkContext() : nullptr;
    if (ctx && !ctx->waitIdleAndDrainCleanup("session reset: before destroy")) return false;
    worldLoader_.reset();
    appearanceComposer_.reset();
    entitySpawner_.reset();
    if (gameHandler && gameHandler->getTransportManager())
        gameHandler->getTransportManager()->clearTransports();
    if (gameHandler) {
        gameHandler->setAchievementEarnedCallback(nullptr);
        gameHandler->setBindPointCallback(nullptr);
        gameHandler->setCharCreateCallback(nullptr);
        gameHandler->setCharDeleteCallback(nullptr);
        gameHandler->setCharLoginFailCallback(nullptr);
        gameHandler->setChargeCallback(nullptr);
        gameHandler->setCreatureDespawnCallback(nullptr);
        gameHandler->setCreatureMoveCallback(nullptr);
        gameHandler->setCreatureSpawnCallback(nullptr);
        gameHandler->setEmoteAnimCallback(nullptr);
        gameHandler->setGameObjectCustomAnimCallback(nullptr);
        gameHandler->setGameObjectDespawnCallback(nullptr);
        gameHandler->setGameObjectInfoCallback(nullptr);
        gameHandler->setGameObjectMoveCallback(nullptr);
        gameHandler->setGameObjectSpawnCallback(nullptr);
        gameHandler->setGameObjectStateCallback(nullptr);
        gameHandler->setGhostStateCallback(nullptr);
        gameHandler->setHearthstonePreloadCallback(nullptr);
        gameHandler->setHitReactionCallback(nullptr);
        gameHandler->setLevelUpCallback(nullptr);
        gameHandler->setLootWindowCallback(nullptr);
        gameHandler->setMountCallback(nullptr);
        gameHandler->setNpcAggroCallback(nullptr);
        gameHandler->setNpcDeathCallback(nullptr);
        gameHandler->setNpcFarewellCallback(nullptr);
        gameHandler->setNpcGreetingCallback(nullptr);
        gameHandler->setNpcRespawnCallback(nullptr);
        gameHandler->setNpcSwingCallback(nullptr);
        gameHandler->setNpcVendorCallback(nullptr);
        gameHandler->setOpenLfgCallback(nullptr);
        gameHandler->setOtherPlayerLevelUpCallback(nullptr);
        gameHandler->setOtherPlayerMountCallback(nullptr);
        gameHandler->setPlayMusicCallback(nullptr);
        gameHandler->setPlayPositionalSoundCallback(nullptr);
        gameHandler->setPlaySoundCallback(nullptr);
        gameHandler->setPlayerDespawnCallback(nullptr);
        gameHandler->setPlayerEquipmentCallback(nullptr);
        gameHandler->setPlayerHealthCallback(nullptr);
        gameHandler->setPlayerPositionCorrectionCallback(nullptr);
        gameHandler->setPlayerSpawnCallback(nullptr);
        gameHandler->setSpellCastAnimCallback(nullptr);
        gameHandler->setSprintAuraCallback(nullptr);
        gameHandler->setStandStateCallback(nullptr);
        gameHandler->setStealthStateCallback(nullptr);
        gameHandler->setStunStateCallback(nullptr);
        gameHandler->setTaxiFlightStartCallback(nullptr);
        gameHandler->setTaxiOrientationCallback(nullptr);
        gameHandler->setTaxiPrecacheCallback(nullptr);
        gameHandler->setTransportMoveCallback(nullptr);
        gameHandler->setTransportSpawnCallback(nullptr);
        gameHandler->setUnitAnimHintCallback(nullptr);
        gameHandler->setUnitMoveFlagsCallback(nullptr);
        gameHandler->setUnstuckCallback(nullptr);
        gameHandler->setUnstuckGyCallback(nullptr);
    ...
    // every callback that captures a handler; Lua renderer getters resolve live owners.
    worldEntryCallbacks_.reset();
    entitySpawnCallbacks_.reset();
    animationCallbacks_.reset();
    npcInteractionCallbacks_.reset();
    audioCallbacks_.reset();
    transportCallbacks_.reset();
    uiScreenCallbacks_.reset();
    if (ctx && ctx->getAllocator()) {
    ...
    if (ctx) {
        if (!ctx->waitIdleAndDrainCleanup("session reset: final release")) return false;
        ctx->resetFrameSyncState();
        if (ctx->isDeviceLost()) return false;
        if (ctx->getAllocator()) {
            VmaTotalStatistics stats{};
            vmaCalculateStatistics(ctx->getAllocator(), &stats);
    ...
    renderer->setAudioCoordinator(audioCoordinator_.get());
    gameServices_.renderer = renderer.get();
    if (auto* lighting = renderer->getLightingManager()) lighting->initialize(assetManager.get());
    world = std::make_unique<game::World>();
    entitySpawner_ = std::make_unique<EntitySpawner>(
        renderer.get(), assetManager.get(), gameHandler.get(), &gameServices_);
    entitySpawner_->initialize();
    appearanceComposer_ = std::make_unique<AppearanceComposer>(
        renderer.get(), assetManager.get(), gameHandler.get(), entitySpawner_.get());
    worldLoader_ = std::make_unique<WorldLoader>(
        *this, renderer.get(), assetManager.get(), gameHandler.get(),
        entitySpawner_.get(), appearanceComposer_.get(), window.get(),
        world.get(), addonManager_.get());
    ui::UIServices services;
    services.window = window.get(); services.renderer = renderer.get();
    services.assetManager = assetManager.get(); services.gameHandler = gameHandler.get();
    services.expansionRegistry = expansionRegistry_.get(); services.addonManager = addonManager_.get();
    services.audioCoordinator = audioCoordinator_.get(); services.entitySpawner = entitySpawner_.get();
    services.appearanceComposer = appearanceComposer_.get(); services.worldLoader = worldLoader_.get();
    if (uiManager) {
        uiManager->setAppearanceComposer(appearanceComposer_.get());
        uiManager->setServices(services);
        uiManager->getGameScreen().applySavedAntiAliasing(renderer.get());
    }
    setupUICallbacks();
    LOG_INFO("[SESSION_RESET] fresh callback graph ready; world instances=0; intro state reset");
    return true;
}
...
void Application::performLogoutToLogin() {
    ...
    localQuestMarkerRefresh_ = 0.0f;
    // Back on the login card, not in the character screens' local flow.
    ...
    if (entitySpawner_) entitySpawner_->waitForAsyncLoads();
    if (renderer && renderer->getVkContext() &&
        !renderer->getVkContext()->waitIdleAndDrainCleanup("logout: before despawn callbacks")) {
        running = false;
        return;
    }
    ...
    if (gameHandler && gameHandler->getTransportManager()) {
        ...
        gameHandler->getTransportManager()->setM2Renderer(nullptr);
    }
    ...
    if (renderer && renderer->getVkContext() && !renderer->getVkContext()->isDeviceLost()) {
        ...
        if (!renderer->getVkContext()->waitIdleAndDrainCleanup("logout")) { running = false; return; }
    }

    widgetRenderer_.releaseSessionTextures();

    // Portraits are session-owned too. Unregister them after the GPU drain,
    // before clearing world renderers, so reconnect cannot retain the old
    // character's face or its offscreen allocations.
    unitPortrait_.shutdown(renderer.get());
    targetPortrait_.shutdown(renderer.get());
    petPortrait_.shutdown(renderer.get());
    focusPortrait_.shutdown(renderer.get());
    ...
    stableModel_.shutdown(renderer.get());
    companionModel_.shutdown(renderer.get());
    for(auto& portrait:partyPortraits_)portrait.shutdown(renderer.get());
    paperdollModel_.shutdown(renderer.get());
    LOG_INFO("[LAN_RECOVERY] session portraits released");

    // --- Reset all EntitySpawner state (mount, creatures, players, GOs, queues, caches) ---
    if (entitySpawner_) entitySpawner_->resetAllState();
    // All world producers have stopped before their queued voices are released.
    // Front-end music starts later, after this boundary, in UIManager.
    if (audioCoordinator_) audioCoordinator_->stopWorldAudio();

    if (uiManager) uiManager->releaseFrontendScenes();
    if (!rebuildSessionRenderer()) {
        LOG_FATAL("[SESSION_RESET] Renderer reconstruction failed; closing safely");
        running = false;
        return;
    }
#ifdef WOWEE_PS4
    const auto logoutMemory = platform::ps4::queryAvailableCpuMemory();
    LOG_INFO("[SESSION_MEMORY] new frontend renderer kernelHeadroomMiB=",
             logoutMemory.bytes / (1024*1024), " measured=", logoutMemory.measured,
             " (allocator reusable arenas are not included)");
#endif
...
void Application::applyServerMovementState(float deltaTime) {
    if (renderer && gameHandler && renderer->getCameraController()) {
        renderer->getCameraController()->setCollisionHeight(gameHandler->getServerCollisionHeight());
...
#ifdef WOWEE_PS4
    // Taxi travel keeps the same bounded working set. Desktop's radius 8/12
    // override can queue hundreds of tiles and overwhelm the console heap.
    renderer->getTerrainManager()->setLoadRadius(configuredLoadRadius);
    renderer->getTerrainManager()->setUnloadRadius(configuredUnloadRadius);
#else
    ...
    if (onTaxi) {
        ...
        gameHandler->updateM2TransportBoarding(canonical, deltaTime);
        ...
        if (gameHandler->getTransportManager()) {
            ...
            gameHandler->updateM2TransportBoarding(playerCanonical, deltaTime);
        }
    }
    }
}
...
void Application::updateInGame(float deltaTime, const char*& updateCheckpoint) {
    if (gameHandler && gameHandler->isLocalExploration()) {
        updateCheckpoint = "local realm: entity spawner";
        const auto spawnStart = std::chrono::steady_clock::now();
        if (entitySpawner_) entitySpawner_->update();
        noteStageTime("local entity loading", std::chrono::duration<float, std::milli>(
            std::chrono::steady_clock::now() - spawnStart).count());
        updateCheckpoint = "local realm: interpolation";
        gameHandler->updateEntityInterpolation(deltaTime);
        updateCheckpoint = "local realm: movement state";
        ...
        noteStageTime("local character sync", std::chrono::duration<float, std::milli>(
            std::chrono::steady_clock::now() - syncStart).count());
        updateCheckpoint = "local realm: quest markers";
        localQuestMarkerRefresh_ -= std::isfinite(deltaTime) ? std::max(0.0f, deltaTime) : 0.0f;
        // QuestHandler owns the map read by getNpcQuestStatuses(), the target
        // frame and minimap. GameHandler also retains a legacy map which those
        // readers no longer use.
        if (auto* quests = gameHandler->getQuestHandler(); quests && !characterIntroOwnsView()) {
            auto& statuses = quests->npcQuestStatusRef();
            const auto* player = localRealm_ ? localRealm_->localPlayer() : nullptr;
            if (!player) {
                statuses.clear();
                localQuestMarkerRefresh_ = 0.0f;
            } else if (localQuestMarkerRefresh_ <= 0.0f) {
                localQuestMarkerRefresh_ = 0.2f;
                statuses.clear();
                for (const auto& npc : localRealm_->npcs()) {
                    if (!npc.questGiver || npc.dead || npc.health == 0 || npc.aggressive ||
                        npc.mapId != player->mapId || npc.instanceId != player->instanceId) continue;
                    const auto status = game::localQuestMarkerStatus(*player, npc,
                        localRealm_->questsForNpc(npc.entry));
                    if (status != game::QuestGiverStatus::NONE) statuses.emplace(npc.guid, status);
                }
            }
        }
        // Positions follow interpolated instances every frame. Only catalog
        // status queries are throttled; markers never inherit the 8m talk gate.
        if (!characterIntroOwnsView()) updateQuestMarkers();
        updateCheckpoint = "local realm: original UI";
        introUiElapsed_ += std::isfinite(deltaTime) ? std::max(0.0f, deltaTime) : 0.0f;
        // Hidden retail HUD snapshots and OnUpdate scripts do not need the
        // cinematic frame rate. Keep their time/event state alive at 5 Hz and
        // restore per-frame updates immediately when camera ownership ends.
        if (addonManager_ && addonsLoaded_ &&
            (!characterIntroOwnsView() || introUiElapsed_ >= 0.2f)) {
            const auto uiStart = std::chrono::steady_clock::now();
            const float uiDelta = introUiElapsed_;
            introUiElapsed_ = 0.0f;
            try {
                localFrameXml_.update(uiDelta);
                addonManager_->update(uiDelta);
            } catch (const std::bad_alloc&) {
                localFrameXml_.reset();
                addonManager_->failOriginalInterface("local UI snapshot allocation failed");
                addonManager_->discardPartialInterface();
            }
            if (!addonManager_->originalInterfaceLoaded()) localFrameXml_.reset();
            noteStageTime("local FrameXML update", std::chrono::duration<float, std::milli>(
    ...
    if (addonManager_ && addonsLoaded_) {
        try {
            localFrameXml_.update(deltaTime);
            addonManager_->update(deltaTime);
        } catch (const std::bad_alloc&) {
            localFrameXml_.reset();
            addonManager_->failOriginalInterface("UI snapshot allocation failed");
            addonManager_->discardPartialInterface();
        }
        if (!addonManager_->originalInterfaceLoaded()) localFrameXml_.reset();
    }
                ...
                float retryScale = 1.0f;
                {
                    ...
                    if (si != 0xFFFF) {
                        ...
                        if (raw != 0) {
                            float s2 = 1.0f;
#ifdef WOWEE_PS4
                            // The console's libc++ predates std::bit_cast.
                            static_assert(sizeof(s2) == sizeof(raw));
                            std::memcpy(&s2, &raw, sizeof(s2));
#else
...
void Application::update(float deltaTime) {
    ...
    if (reloadUiPending_) {
        ...
        if (addonManager_) {
            localFrameXml_.reset();
            const auto recoverInterface = [&](const char* reason) {
                localFrameXml_.reset();
                addonManager_->failOriginalInterface(reason);
                addonManager_->discardPartialInterface();
                addonsLoaded_ = true;
            };
            try {
                // /reload is serviced between Lua calls. The attempt is armed
                // before rebuilding either the local bridge or retail scripts.
                if (!addonManager_->unloadAll() || !addonManager_->beginOriginalInterfaceAttempt()) {
                    recoverInterface("UI reload preflight/recovery guard failed");
                } else {
                    prepareLocalFrameXml();
                    addonManager_->loadAllAddons();
                    localFrameXml_.activate(addonManager_->originalInterfaceLoaded());
                    if (!addonManager_->originalInterfaceLoaded() ||
                        (gameHandler && gameHandler->isLocalExploration() && !localFrameXml_.ready())) {
                        recoverInterface("UI reload source/local bridge failed");
                    } else {
                        struct RestoreUiSound {
                            bool previous = addons::LuaEngine::uiSoundsSuppressed();
                            RestoreUiSound() { addons::LuaEngine::setUiSoundsSuppressed(true); }
                            ~RestoreUiSound() { addons::LuaEngine::setUiSoundsSuppressed(previous); }
                        } restoreUiSound;
                        for (const char* event : {"VARIABLES_LOADED", "UPDATE_CHAT_WINDOWS", "PLAYER_LOGIN", "PLAYER_ENTERING_WORLD"})
                            addonManager_->fireEvent(event);
                        if (!addonManager_->completeOriginalInterfaceAttempt())
                            recoverInterface("UI reload login validation failed");
                    }
                }
            } catch (const std::exception& e) {
                LOG_WARNING("[FRAMEXML_RECOVERY] UI reload exception: ", e.what());
                recoverInterface("UI reload allocation/bootstrap exception");
            }
        }
    }
    // Local sessions are started here, after the menu render has finished.
    {
        ...
        noteStageTime("local authority/snapshots", std::chrono::duration<float, std::milli>(
    ...
    // Apply authored camera ownership before renderer streaming/culling runs.
    updateCharacterIntro(deltaTime);
    if (characterIntroOwnsView() && renderer && renderer->getQuestMarkerRenderer())
        renderer->getQuestMarkerRenderer()->clear();

    // Renderer::update reaps world voices. Menus need the same lifecycle even
    // when no further button click arrives after a short UI sound.
    if (state != AppState::IN_GAME) audio::AudioEngine::instance().update(deltaTime);
...
void Application::beatWatchdog() {
    if (localRealm_ && worldLoader_ && worldLoader_->isLoadingWorld())
        localRealm_->update(0.0f); // keep the LAN host responsive while assets load
...
void Application::render() {
...
#ifdef WOWEE_PS4
    // Frames 0..2 name every render stage before it runs, so a crash without
    // a signal marker still says which stage it was in.
    static unsigned ps4RenderedFrames = 0;
    static unsigned ps4WorldFrames = 0;
    static AppState ps4PreviousRenderState = AppState::AUTHENTICATION;
    if (state != ps4PreviousRenderState) {
        if (state == AppState::IN_GAME) ps4WorldFrames = 0;
        ps4PreviousRenderState = state;
    }
    const bool ps4TraceWorld = state == AppState::IN_GAME && ps4WorldFrames < 3;
    const bool ps4TraceStages = ps4RenderedFrames < 3 || ps4TraceWorld;
    if (ps4RenderedFrames < 3) ++ps4RenderedFrames;
    if (ps4TraceWorld) ++ps4WorldFrames;
    platform::ps4::setFrameTraceEnabled(ps4TraceStages);
    struct FrameTraceScope {
        ~FrameTraceScope() { platform::ps4::setFrameTraceEnabled(false); }
    } frameTraceScope;
#endif
    auto runRenderStage = [this
#ifdef WOWEE_PS4
                           , ps4TraceStages
#endif
                          ](const char* stageName, auto&& fn) {
#ifdef WOWEE_PS4
        if (ps4TraceStages) {
            char stage[96];
            std::snprintf(stage, sizeof(stage), "render: %s", stageName);
            platform::ps4::reportBootStage(stage);
        }
#endif
        auto stageStart = std::chrono::steady_clock::now();
        ...
        if (ps4TraceStages) {
            char stage[96];
            std::snprintf(stage, sizeof(stage), "render complete: %s", stageName);
            platform::ps4::reportBootStage(stage);
        }
#endif
    ...
    // Menu models and UI textures may allocate/upload GPU resources. Complete
    // them before acquiring the frame or recording the main render pass.
    // Selection input only schedules work for this preparation stage.
    if (uiManager) {
        runRenderStage("prepareGlueScene", [&] {
            beatWatchdog();
            uiManager->prepareScene(state, gameHandler.get(), ImGui::GetIO().DeltaTime);
            beatWatchdog();
        });
    }
#ifdef WOWEE_PS4
    if (auto* postProcess = renderer->getPostProcessPipeline()) {
        postProcess->setWorldRendering(state == AppState::IN_GAME);
    }
#endif
    ...
    if (renderer && !characterIntroOwnsView()) {
    ...
    const bool drawWidgets = state == AppState::IN_GAME && !characterIntroOwnsView() &&
        (!(gameHandler && gameHandler->isLocalExploration()) || localFrameXml_.ready());
        ...
        runRenderStage("addonWidgets", [&] {
            ...
            if (gameHandler && assetManager) {
                ...
                if (doll && doll->visible) {
                    ...
                    setExternalTexture(widgets, doll, paperdollModel_.textureId());
                } else if (doll) {
                    setExternalTexture(widgets, doll, 0);
                }
                ...
                for (const DressUp& room : kDressUps) {
                    ...
                    if (dressUp) {
                        setExternalTexture(widgets, dressUp, shown ? room.model->textureId() : 0);
                    }
                }
                ...
                for (const CreatureModel& cm : kCreatureModels) {
                    ...
                    if (frame) {
                        setExternalTexture(widgets, frame, shown ? cm.model->textureId() : 0);
                    }
                }
                ...
                // display id its portrait is drawn from.
                {
                    ...
                    if (petModel) {
                        setExternalTexture(widgets, petModel, shown ? petModel_.textureId() : 0);
                    }
                }
                ...
                // result is where that guid already lives.
                {
                    ...
                    if (inspectModel) {
                        setExternalTexture(widgets, inspectModel, shown ? inspectModel_.textureId() : 0);
                    }
                }

                if (portrait && portrait->visible && portrait->alpha > .001f) {
                    ...
                    setExternalTexture(widgets, portrait, unitPortrait_.textureId());
                }
                ...
                for (uint32_t id : widgets.playerPortraits()) {
                    if (ui::Widget* w = widgets.get(id)) {
                        setExternalTexture(widgets, w, playerFace);
                    }
                }
                ...
                for (const UnitFace& face : kFaces) {
                    ...
                    const auto visibleClaim = [&widgets](const auto& ids) {
                        for (auto id : ids) if (const auto* w = widgets.get(id);
                            w && w->visible && w->alpha > .001f) return true;
                        return false;
                    };
                    if (!visibleClaim(claimed) && !visibleClaim(alsoClaimed)) continue;
                    ...
                    if (face.guid != 0 &&
                        ...
                        // A portrait slot previously used by a humanoid NPC must
                        // not keep that NPC's baked face/armour on a real player.
                        face.portrait->setBakedSkin({});
                    ...
                    for (uint32_t id : claimed) {
                        if (ui::Widget* w = widgets.get(id)) setExternalTexture(widgets, w, drawn);
                    }
            ...
            if (!characterIntroOwnsView() && !localRealmMenuOpen_ && !localRealmDialogueNpc_ &&
                    !localRealmInventoryOpen_ && !localRealmJournalOpen_ && localFrameXml_.navigateBars()) {
                // Raw shoulder/Square icon selection owns this press exclusively.
            } else if (!overClientUi && !localRealmMenuOpen_ && !localRealmDialogueNpc_ && !localRealmInventoryOpen_ && !localRealmJournalOpen_ && localFrameXml_.navigate()) {
                // D-pad/Cross has dispatched this frame to the focused original widget.
            } else if (overClientUi) {
    ...
    if (uiManager) {
        runRenderStage("uiManager->render", [&] {
            ...
            if (characterIntroOwnsView()) renderCharacterIntroOverlay();
            else renderLocalRealmOverlay();
        });
        ...
        // Mandatory means exactly that: do not silently put the native HUD
        // back when retail FrameXML failed. Keep the world alive so the error
        // is diagnosable and the player can return, but make the missing UI
        // explicit instead of showing a misleading second implementation.
        if (state == AppState::IN_GAME && ui::frameXmlRequired() &&
            !ui::frameXmlActive()) {
            const ImGuiIO& errIo = ImGui::GetIO();
            ImGui::SetNextWindowPos({errIo.DisplaySize.x * 0.5f,
                                     errIo.DisplaySize.y * 0.5f},
                                    ImGuiCond_Always, {0.5f, 0.5f});
            ImGui::SetNextWindowSize({std::min(720.0f, errIo.DisplaySize.x - 80.0f), 0.0f});
            ImGui::Begin("Original interface load error", nullptr,
                         ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove |
                         ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_AlwaysAutoResize);
            ImGui::TextWrapped("The original 3.3.5 FrameXML interface could not be loaded. "
                               "The native fallback is disabled. Verify the Interface/FrameXML "
                               "files in the game MPQs, then return to character selection and retry.");
            ImGui::End();
        }
...
void Application::spawnPlayerCharacter() {
    ...
    if (!loaded && gameHandler && gameHandler->isLocalExploration()) {
        LOG_ERROR("[LOCAL_REALM] Cannot create the required character model: ", m2Path);
        return;
    }
...
void Application::updateQuestMarkers() {
    ...
    for (const auto& [guid, status] : questStatuses) {
        ...
        auto entity = gameHandler->getEntityManager().getEntity(guid);
        if (!entity) continue;
        float boundingHeight = core::DEFAULT_COLLISION_HEIGHT;
        if (entity->getType() == game::ObjectType::UNIT) {
            auto unit = std::static_pointer_cast<game::Unit>(entity);
            if (game::isUnitCorpseState(unit->getHealth(), unit->getMaxHealth(), unit->getDynamicFlags())) continue;
            boundingHeight = core::collisionHeightOrDefault(unit->getCollisionHeight());
        ...
        // Anchor to the interpolated, floor-adjusted character instance. A
        // loading/despawned NPC must not leave an icon hanging on empty ground.
        // Collision height is a vertical extent; the visual bounding sphere's
        // diameter also includes wings/weapons and put marks far above heads.
        const bool hasRenderPosition = getRenderPositionForGuid(guid, renderPos);
        if (entity->getType() == game::ObjectType::UNIT && !hasRenderPosition) continue;
