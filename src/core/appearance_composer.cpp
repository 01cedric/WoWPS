...
void AppearanceComposer::compositePlayerSkin(uint32_t modelSlotId, const PlayerTextureInfo& texInfo) {
    ...
    // Composite body skin + face + underwear overlays
    {
        ...
        if (layers.size() > 1) {
            // Fallible, and the fallback is the body skin alone. This is the
            // one character the player is looking at, so it is the last thing
            // to degrade and it still degrades rather than throwing: it runs
            // inside the same EntitySpawner::update the console died in, and a
            // crowd can exhaust the heap between this call and the last one.
            rendering::VkTexture* compositeTex = nullptr;
            try {
                compositeTex = charRenderer->compositeTextures(layers);
            } catch (const std::bad_alloc&) {
                LOG_WARNING("Player skin composite could not be allocated; "
                            "falling back to the body skin without overlays");
                try {
                    compositeTex = charRenderer->loadTexture(texInfo.bodySkinPath);
                } catch (const std::bad_alloc&) {
...
void AppearanceComposer::showRangedWeapon(bool show) {
    ...
    // Relics share the ranged equipment slot but have no held weapon model.
    if (rangedSlot.item.inventoryType != game::InvType::RANGED_BOW &&
        rangedSlot.item.inventoryType != game::InvType::THROWN &&
        rangedSlot.item.inventoryType != game::InvType::RANGED_GUN) return;
