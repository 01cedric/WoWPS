...
void EntitySpawner::spawnOnlinePlayer(uint64_t guid,
    ...
    rendering::VkTexture* compositeTex = nullptr;
    {
        ...
        if (layers.size() > 1) {
            // Fallible, and the fallback is the body skin on its own. Another
            // player's face and underwear are worth an atlas when there is one
            // to spare; they are not worth ending the session, and a spawn that
            // throws here unwinds through EntitySpawner::update into the frame
            // loop exactly as the NPC composite did.
            try {
        ...
        if (!compositeTex) {
            try {
                ...
                compositeTex = nullptr;  // the model keeps its own texture
            }
        }
    }
...
void EntitySpawner::setOnlinePlayerEquipment(uint64_t guid,
    ...
    // Fallible for the same reason as the spawn composite above. Leaving the
    // slot override untouched keeps whatever this player was already wearing,
    // which is the previous equipment rather than nothing - a re-composite that
    // could not be afforded shows stale armour, and the next one puts it right.
    rendering::VkTexture* newTex = nullptr;
    try {
        newTex = charRenderer->compositeWithRegions(st.bodySkinPath, st.underwearPaths, regionLayers);
    } catch (const std::bad_alloc&) {
        newTex = nullptr;
    }
    if (newTex) {
...
std::string EntitySpawner::transportModelPath(uint32_t entry, uint32_t displayId) {
    ...
    for (const TransportModel& t : kTransportModels) {
        if (entry == t.entry) return t.path;
    }
    for (const TransportModel& t : kTransportModels) {
        if (displayId == t.displayId) return t.path;
    }
