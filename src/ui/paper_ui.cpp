...
void PaperUI::setWotlkSkin(WotlkGlue* skin) {
    wotlkSkin_ = skin;
    theme_ = PaperTheme{};
    if (!skin) return;
    theme_.paperTop = IM_COL32(9, 9, 12, 242);
    theme_.paperBottom = IM_COL32(9, 9, 12, 242);
    theme_.paperEdge = IM_COL32(131, 116, 90, 255);
    theme_.shadow = IM_COL32(0, 0, 0, 130);
    theme_.ink = IM_COL32(255, 209, 0, 255);
    theme_.inkSoft = IM_COL32(240, 230, 206, 255);
    theme_.pencil = IM_COL32(165, 157, 142, 255);
    theme_.fieldFill = IM_COL32(0, 0, 0, 220);
    theme_.crayonRed = IM_COL32(225, 66, 51, 255);
    theme_.crayonBlue = IM_COL32(106, 176, 255, 255);
    theme_.crayonGreen = IM_COL32(70, 221, 105, 255);
    theme_.highlighter = IM_COL32(186, 124, 13, 85);
    theme_.onCrayon = IM_COL32(255, 209, 0, 255);
    theme_.photoFade = 0;
    theme_.photoTint = IM_COL32_WHITE;
}
...
void PaperUI::rule(ImVec2 a, ImVec2 b, ImU32 col, float thickness, uint32_t seed,
                   float amplitude) {
    if (wotlkSkin_) { dl_->AddLine(a, b, col, thickness); return; }
...
void PaperUI::wobblyRect(ImDrawList* dl, ImVec2 a, ImVec2 b, ImU32 col, float thickness,
    ...
    if (wotlkSkin_) { dl->AddRect(a, b, col, 1, 0, thickness); return; }
    ImDrawList* keep = dl_;
    dl_ = dl;
...
void PaperUI::drawSheetBody(ImDrawList* dl, ImVec2 a, ImVec2 b, bool taped) {
    if (wotlkSkin_) { wotlkSkin_->drawFrame(dl, a, b); return; }
...
bool PaperUI::button(const char* idStr, ImVec2 a, ImVec2 b, const char* label,
    ...
    if (wotlkSkin_) {
        wotlkSkin_->drawButton(dl_, a, b, over, held, enabled);
        const float fitSize = std::min(size, size * std::max(1.0f, b.x - a.x - px(14)) /
                                               std::max(1.0f, textWidth(label, size)));
        const float y = pa.y + ((b.y - a.y) - fitSize) * .5f - px(1);
        textCentered(cx + px(1), y + px(1), label, fitSize, IM_COL32(0, 0, 0, 255));
        textCentered(cx, y, label, fitSize, enabled ? theme_.ink : theme_.pencil);
...
bool PaperUI::chip(const char* idStr, ImVec2 a, ImVec2 b, const char* label, bool selected,
    ...
    if (wotlkSkin_) {
        wotlkSkin_->drawField(dl_, pa, pb, selected || over);
        if (selected) dl_->AddRectFilled(pa, pb, paperFade(accent, .18f), 2);
        textCentered(cx, textY, label, size, selected ? theme_.ink : theme_.inkSoft);
        return clicked;
    }
...
PaperUI::FieldResult PaperUI::field(const char* idStr, ImVec2 a, ImVec2 b, TextEdit& edit,
    ...
    if (focused && live) {
...
#ifdef WOWEE_PS4
        // The console's on-screen keyboard opens for this field; it needs to
        // know what to call it and what it holds. Noted every frame the field
        // has the focus; the ImGui backend takes it after the frame.
        {
            platform::ps4::TextFieldHint hint;
            hint.title = opts.placeholder ? opts.placeholder : idStr;
            hint.text = opts.password ? std::string() : edit.text();
            hint.password = opts.password;
            hint.numeric = opts.digitsOnly;
            platform::ps4::noteTextField(hint);
        }
#endif
        ...
        if (!tabHandledThisFrame_ && ImGui::IsKeyPressed(ImGuiKey_Tab, false)) {
            tabHandledThisFrame_ = true;
            if (shift) {
                if (drawnBefore != 0) {
                    focus_ = drawnBefore;
    ...
    if (wotlkSkin_) {
        wotlkSkin_->drawField(dl_, a, b, focused);
...
ImU32 PaperUI::onPaper(ImVec4 colour) const {
    if (wotlkSkin_) return ImGui::ColorConvertFloat4ToU32(colour);
...
void PaperUI::image(ImTextureID texture, ImVec2 a, ImVec2 b) {
    if (wotlkSkin_) { dl_->AddImage(texture, a, b); return; }
...
bool PaperUI::dropdown(const char* idStr, ImVec2 a, ImVec2 b, const std::string& preview,
    ...
    const uint32_t seed = seedFor(a, b);
    if (wotlkSkin_) wotlkSkin_->drawField(dl_, a, b, open || over);
    else dl_->AddRectFilled(a, b, paperFade(theme_.fieldFill, 0.75f), px(3));
...
bool PaperUI::sliderFloat(const char* idStr, ImVec2 a, ImVec2 b, float* value, float lo,
    ...
    if (wotlkSkin_) {
        // Reuse the MPQ field borders and button metalwork for a compact lever.
        wotlkSkin_->drawField(dl_, {x0, cy - px(4)}, {x1, cy + px(4)}, false);
        const float half = px(7);
        wotlkSkin_->drawButton(dl_, {kx - half, a.y}, {kx + half, b.y},
                              over, active_ == id, live);
