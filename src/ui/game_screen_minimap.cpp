...
void GameScreen::saveSettings() {
    ...
    // This setting belongs to the login scene. Preserve it when the in-game
    // panel rewrites its own values so world entry does not undo menu framing.
    std::string menuSceneSetting;
    {
        std::ifstream previous(path);
        std::string line;
        while (std::getline(previous, line)) {
            if (line.rfind("menu_scene_scale=", 0) == 0) menuSceneSetting = line;
        }
    }
    ...
    // Interface
    if (!menuSceneSetting.empty()) out << menuSceneSetting << "\n";
    ...
    // Shadows and water reflections. Both were added to the video panel and
    // neither was written here, so a player who turned shadows off got them
