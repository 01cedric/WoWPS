...
void GameScreen::syncEquipmentAnimation(const game::Inventory& inventory) {
        // Update renderer weapon type for animation selection
        auto* r = services_.renderer;
        if (r) {
            const auto& mh = inventory.getEquipSlot(game::EquipSlot::MAIN_HAND);
            const auto& oh = inventory.getEquipSlot(game::EquipSlot::OFF_HAND);
            if (mh.empty()) {
...
int GameScreen::getResolutionIndex() const {
    ...
    // The window is always the console's 1080p output, so asking it which mode
    // is selected would always answer "High Definition" however the world is
    // actually being rendered. The setting is the world's height, so read that.
    //
    // Through the shared lookup rather than an equality search here: the search
    // answered index 0 for any height that was on neither entry, and
    // WOWEE_PS4_SCENE_HEIGHT=900 is a supported way to be in exactly that
    // state. Both option screens then named a profile the renderer was not
    // running, and a player who picked the entry already displayed got no
    // change because there was nothing to change - the dropdown was reporting
    // the wrong row, not failing to apply the right one.
    return displayResolutionIndexForSceneHeight(
        static_cast<int>(rendering::configuredPs4SceneHeight()));
#else
...
#if defined(WOWEE_PS4)
    // B39: this used to reach Window::applyResolution, which is a documented
    // no-op on the console - the VideoOut surface is fixed - so choosing a
    // resolution stored a number and changed nothing a player could see.
    //
    // What it selects now is the size the world is rendered at before it is
    // sampled up to that fixed output. PostProcessPipeline re-reads this every
    // frame and prepares the new extent before the next frame is recorded, so
    // the change is visible immediately instead of at the next launch.
    rendering::setPs4SceneHeight(
        static_cast<uint32_t>(displayResolutionSceneHeight(index)));
    if (services_.window) {
        // Still recorded so the settings file and the FrameXML panel agree on
        // which entry is selected; the surface itself is unchanged.
...
void GameScreen::setServices(const UIServices& services) {
...
#if defined(WOWEE_PS4)
    // Put the client's own video panel on the entry the renderer is actually
    // running, before it first draws itself.
    //
    // That panel picks its row by matching pendingResolutionWidth/Height
    // against the list, and those two hold the *window* size on the console -
    // 1920x1080, always, whatever the world is drawn at. It therefore showed
    // "High Definition" over a 720p world, and a player selecting the entry it
    // was already showing changed nothing, which is exactly the report. The
    // renderer's scene height is the one true copy of this setting, so the
    // panel's fields are derived from it rather than from the surface.
    {
        const int current = getResolutionIndex();
        settingsPanel_.pendingResIndex = current;
        settingsPanel_.pendingResolutionWidth  = kDisplayResolutions[current][0];
        settingsPanel_.pendingResolutionHeight = kDisplayResolutions[current][1];
    }
#endif
    ...
    // The title-safe margin. Read from the settings file in the constructor,
    // long before the addon manager and its widget tree exist, so like the
    // camera values above it has to be handed over once they do. A margin that
    // only takes effect when the slider is moved is a margin that is wrong on
    // every launch.
    if (services_.addonManager) {
        if (auto* engine = services_.addonManager->getLuaEngine()) {
            engine->widgets().setSafeAreaInset(
                static_cast<float>(settingsPanel_.pendingSafeArea) / 100.0f);
        }
    }
...
void GameScreen::render(game::GameHandler& gameHandler) {
    ...
    // Shadows, sun rays and water reflections, applied once the renderer has
    // published its sinks. Loading them into the pending fields and stopping
    // there is what test_settings_apply_on_load calls "shown on the control and
    // never applied": a player who turned shadows off got them back at every
    // launch, and the control still said Off.
    //
    // Latched on the sinks rather than on the renderer, for the same reason the
    // flare below waits for the flare: the renderer exists before it fills
    // them, and marking the work done in between is how a setting comes to be
    ...
    if (!settingsPanel_.renderSettingsApplied_) {
        ...
        if (sinks.setShadowQuality) {
            ...
            // noteClientSettingChanged is for, and it is the same route the
            // panel uses when the player ticks the box.
            addons::noteClientSettingChanged(
                "waterreflections", settingsPanel_.pendingWaterReflections ? "1" : "0");
            settingsPanel_.renderSettingsApplied_ = true;
        }
    }
    ...
    if (inventoryScreen.consumeEquipmentDirty() || gameHandler.consumeOnlineEquipmentDirty()) {
        ...
        syncEquipmentAnimation(gameHandler.getInventory());
    }
