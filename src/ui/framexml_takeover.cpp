...
namespace { std::atomic<bool> gInterfaceActive{false}; }
void frameXmlSetActive(bool active) { gInterfaceActive.store(active, std::memory_order_relaxed); }
bool frameXmlActive() { return gInterfaceActive.load(std::memory_order_relaxed); }
static bool frameXmlLoaded() { return frameXmlActive(); }
...
bool frameXmlRequired() {
    const char* value = std::getenv("WOWPS_REQUIRE_FRAMEXML");
...
bool frameXmlOwns(UiElement element) {
    ...
    if (!frameXmlLoaded()) return frameXmlRequired();

    // The same for one element whose frames never arrived.
    if (frameXmlWasReleased(element) && !frameXmlRequired()) return false;
...
namespace {
/// Main thread only, like the cursor icon beside it: the pad navigation writes
/// it while laying out the frame and the renderer reads it while drawing one.
bool gPadAnchorActive = false;
float gPadAnchorX = 0.0f, gPadAnchorY = 0.0f;
std::function<std::string(uint32_t)> gItemIcon;
} // namespace

void frameXmlSetPadCursorAnchor(float x, float y) {
    gPadAnchorActive = true;
    gPadAnchorX = x;
    gPadAnchorY = y;
}

void frameXmlClearPadCursorAnchor() { gPadAnchorActive = false; }

bool frameXmlPadCursorAnchor(float& x, float& y) {
    if (!gPadAnchorActive) return false;
    x = gPadAnchorX;
    y = gPadAnchorY;
    return true;
}

void frameXmlSetItemIconResolver(std::function<std::string(uint32_t)> resolver) {
    gItemIcon = std::move(resolver);
}

std::string frameXmlItemIconPath(uint32_t displayId) {
    // Zero is "this item has no display entry", which every caller here already
    // means as "no icon" - asking the table about it would only cost a lookup
    // that cannot succeed.
    if (displayId == 0 || !gItemIcon) return {};
    return gItemIcon(displayId);
}

void frameXmlNoteMouseOwned(bool owned) {
    gMouseOwned.store(owned, std::memory_order_relaxed);
}
bool frameXmlOwnsMouse() { return frameXmlActive() && gMouseOwned.load(std::memory_order_relaxed); }
...
void frameXmlResetSession() {
    frameXmlSetActive(false);
    gReleased.store(0, std::memory_order_relaxed);
    gWorldEntered.store(false, std::memory_order_relaxed);
    gCheckRequested.store(false, std::memory_order_relaxed);
    gProbeRequested.store(false, std::memory_order_relaxed);
    gMouseOwned.store(false, std::memory_order_relaxed);
    gCombatTextAddOn.store(false, std::memory_order_relaxed);
    gCursorItem.clear(); gCursorHeldSlot = {}; gCursorPutDown = {};
    // The icon resolver closes over the state that registered it, so it goes
    // with the cursor bridge beside it rather than outliving the VM.
    gItemIcon = {}; gPadAnchorActive = false;
    std::lock_guard<std::mutex> guard(gLoadedAddOnsLock);
    gLoadedAddOns.clear();
}
