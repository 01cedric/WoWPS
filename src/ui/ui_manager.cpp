...
#else
// No SDL on the console: the pad-driven platform backend stands in for
// imgui_impl_sdl2. The Vulkan backend (imgui_impl_vulkan) stays; the real
// Vulkan ICD in ps4/third_party/ps4_vulkan runs it.
...
#ifdef WOWEE_PS4
    // ImGui's default "imgui.ini" is cwd-relative, which the console cannot
    // write; window layouts go to the config tree instead.
...
#ifdef WOWEE_PS4
    ImGui_ImplPS4_Init(window->getWidth(), window->getHeight());
#else
...
#ifdef WOWEE_PS4
        if (err < 0) throw std::runtime_error("ImGui Vulkan resource creation failed: " + std::to_string(err));
#endif
    };

    bool rendererBackendReady = false;
    try {
        rendererBackendReady = ImGui_ImplVulkan_Init(&initInfo);
    } catch (const std::exception& e) {
        LOG_ERROR("ImGui backend initialization: ", e.what());
...
void UIManager::loadInterfaceFont(const std::string& dataRoot,
    ...
    const bool archivesOpen = assets && assets->isInitialized();
    if (fontDir.empty() && !archivesOpen) {
    ...
    if (fontDir.empty()) {
        LOG_INFO("No font directory under ", dataRoot, "; taking the interface fonts from the archives");
    }
    ...
    auto resolve = [&](const char* name) {
        if (fontDir.empty()) return fs::path();
...
void UIManager::shutdown() {
    if (imguiInitialized) {
        ...
        // Screen destructors can release textures and stop menu audio. They
        // must run before ImGui and before Application releases renderer/assets.
        releaseFrontendScenes();
        gameScreen.reset(); characterScreen.reset(); characterCreateScreen.reset();
        realmScreen.reset(); authScreen.reset();
...
#ifdef WOWEE_PS4
    ImGui_ImplPS4_NewFrame(window ? window->getWidth() : 0, window ? window->getHeight() : 0);
#else
...
void UIManager::releaseFrontendScenes() {
    if (authScreen) authScreen->releaseSceneResources();
    if (characterScreen) characterScreen->releaseSceneResources();
    if (characterCreateScreen) characterCreateScreen->releaseSceneResources();
}

void UIManager::prepareScene(core::AppState appState, game::GameHandler* gameHandler, float deltaTime) {
    if (!imguiInitialized) return;
    // This stage runs before Renderer::beginFrame. Inactive previews otherwise
    // retain independent character renderers, textures and full-screen targets
    // all the way through world loading. Keep only the visible scene resident.
    if (appState != core::AppState::AUTHENTICATION && appState != core::AppState::REALM_SELECTION)
        authScreen->releaseSceneResources();
    if (appState != core::AppState::CHARACTER_SELECTION)
        characterScreen->releaseSceneResources();
    if (appState != core::AppState::CHARACTER_CREATION)
        characterCreateScreen->releaseSceneResources();
    switch (appState) {
        case core::AppState::AUTHENTICATION:
        case core::AppState::REALM_SELECTION:
            authScreen->updateFrontendMusic();
            authScreen->prepareScene(deltaTime);
            break;
        case core::AppState::CHARACTER_CREATION:
            authScreen->updateFrontendMusic();
            if (gameHandler) characterCreateScreen->prepareScene(*gameHandler, deltaTime);
            break;
        case core::AppState::CHARACTER_SELECTION:
            authScreen->updateFrontendMusic();
            if (gameHandler) characterScreen->prepareScene(*gameHandler, deltaTime);
            break;
...
void UIManager::render(core::AppState appState, auth::AuthHandler* authHandler, game::GameHandler* gameHandler) {
...
#ifdef WOWEE_PS4
    // The pad layer needs to know whether there is a world to look at: in
    // it the right stick turns the camera, on these screens it is the cursor.
    ImGui_ImplPS4_SetInWorld(appState == core::AppState::IN_GAME);
#endif
    ...
    switch (appState) {
        ...
        case core::AppState::CHARACTER_CREATION:
            authScreen->drawBackdrop(/*allowScene=*/false);
            if (gameHandler) {
...
#endif
#ifdef WOWEE_PS4
    // The console's equivalent: the system keyboard (sceImeDialog) opens when
    // a text field takes the focus. Both interfaces are asked, because
    // ImGui's flag is blind to a FrameXML chat box. Read after the frame is
    // built, so it reflects the box the player just pressed.
    ImGui_ImplPS4_EndFrame(interfaceTakingTypedInput());
#endif
...
void UIManager::processEvent(const SDL_Event& event) {
    if (imguiInitialized) {
#ifdef WOWEE_PS4
        ImGui_ImplPS4_ProcessEvent(&event);
#else
