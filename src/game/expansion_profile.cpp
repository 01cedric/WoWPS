...
bool ExpansionRegistry::loadProfile(const std::string& jsonPath, const std::string& dirPath) {
    ...
    // Optional client header fields (LOGON_CHALLENGE)
    {
        ...
        // UI and server-supplied names are requested in English. Asset locale
        // detection independently falls back when no English MPQs are installed.
        v = jsonValue(json, "locale"); p.locale = v == "enGB" ? "enGB" : "enUS";
