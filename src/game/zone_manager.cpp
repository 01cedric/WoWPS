...
// (the common case: this fork never ships the non-MIT Original Music
// tracks). Every filesystem call takes an error_code so a platform with no
// usable current-directory concept (PS4) fails this the same quiet way as
// the file simply being absent, rather than throwing.
static std::string resolveOriginalMusic(const char* filename) {
    namespace fs = std::filesystem;
    std::error_code ec;
    fs::path rel = fs::path("assets") / "Original Music" / filename;
#ifdef WOWEE_PS4
    rel = core::resolveRelativeAssetPath(rel.string());
    // Absolute paths require neither current_path nor canonical/realpath.
    return fs::is_regular_file(rel, ec) ? rel.string() : std::string{};
#else
    if (fs::exists(rel, ec)) {
        fs::path canon = fs::canonical(rel, ec);
...
bool ZoneManager::isMusicNight(float hours) {
    if(!std::isfinite(hours)) hours=12;
    hours=std::fmod(std::fmod(hours,24.0f)+24.0f,24.0f);
    return hours<6 || hours>=18;
}
uint32_t ZoneManager::resolveMusicZoneId(uint32_t areaId,float /*hours*/) const {
    uint32_t fallback=0;
    for(unsigned depth=0;areaId && depth<32;++depth) {
        const auto z=zones.find(areaId);
        if(z!=zones.end()) {
            if(z->second.hasDbcMusic) return areaId; // zero sound ID means intentional silence
            if(!fallback && !z->second.musicPaths.empty()) fallback=areaId;
        }
        const auto parent=areaParents_.find(areaId);
        if(parent==areaParents_.end() || parent->second==areaId) break;
        areaId=parent->second;
    }
    return fallback;
}
std::string ZoneManager::getRandomMusic(uint32_t areaId, float gameTimeHours) {
    auto it = zones.find(resolveMusicZoneId(areaId,gameTimeHours));
    if (it == zones.end()) return "";
    const auto& zone=it->second;
    // AreaTable -> ZoneMusic -> SoundEntries, retaining separate day/night lists.
    const auto& all = zone.hasDbcMusic ? zone.dbcMusicPaths[isMusicNight(gameTimeHours)?1:0] : zone.musicPaths;
...
std::vector<std::string> ZoneManager::getAllMusicPaths() const {
    ...
    for (const auto& [zoneId, zone] : zones) {
        (void)zoneId;
        auto add=[&](const auto& paths){for(const auto& path:paths)
            if(!path.empty() && seen.insert(path).second) out.push_back(path);};
        if(zone.hasDbcMusic) { add(zone.dbcMusicPaths[0]); add(zone.dbcMusicPaths[1]); }
        else add(zone.musicPaths);
    }
    return out;
}
...
void ZoneManager::enrichFromDBC(pipeline::AssetManager* assets) {
    ...
    enrichFromTables(areaDbc.get(),zoneMusicDbc.get(),soundDbc.get());
}
void ZoneManager::enrichFromTables(const pipeline::DBCFile* areaDbc,const pipeline::DBCFile* zoneMusicDbc,const pipeline::DBCFile* soundDbc) {
    // Reload without preserving stale day/night rows from an earlier expansion.
    for(auto& [id,zone]:zones){zone.hasDbcMusic=false;zone.dbcMusicPaths={};}
    ...
    for (uint32_t i = 0; i < numAreas; ++i) {
        ...
        zone.id = zoneId;
        zone.hasDbcMusic = true;
        zone.dbcMusicPaths[0] = getSoundPaths(daySoundId);
        zone.dbcMusicPaths[1] = getSoundPaths(nightSoundId);
        zonesEnriched += zone.dbcMusicPaths[0].size() + zone.dbcMusicPaths[1].size();
        if(zone.name.empty() && areaFields>11) zone.name=areaDbc->getString(i,11);

    }

    LOG_INFO("Zone music enriched from DBC: ", zones.size(), " zones, ", zonesEnriched, " authored day/night paths");
}
