...
#include "game/collision_height_packet.hpp"
...
void MovementHandler::registerOpcodes(DispatchTable& table) {
    ...
    table[Opcode::MSG_MOVE_SET_COLLISION_HGT] = [this](network::Packet& packet) {
        handleCollisionHeightBroadcast(packet);
    };
...
void MovementHandler::sendMovement(Opcode opcode) {
    ...
    // Local exploration uses the movement flags/physics above with no WoW
    // socket. The separate local-realm transport publishes its final position.
...
void MovementHandler::handleMoveSetCollisionHeight(network::Packet& packet) {
    // Later expansions use a different layout. Do not reinterpret it as 3.3.5.
    if (!isActiveExpansion("wotlk")) return;
    CollisionHeightChange change;
    if (!readCollisionHeightChange(packet, change, false)) {
        LOG_WARNING("SMSG_MOVE_SET_COLLISION_HGT: invalid or truncated payload");
        return;
    }
    if (change.guid != owner_.getPlayerGuid()) return;
    serverCollisionHeight_ = change.height;
    if (auto entity = owner_.getEntityManager().getEntity(change.guid)) {
        if (auto unit = std::dynamic_pointer_cast<Unit>(entity)) unit->setCollisionHeight(change.height);
    }
    if (!owner_.getSocket() || wireOpcode(Opcode::CMSG_MOVE_SET_COLLISION_HGT_ACK) == 0xFFFF) return;
    // GUID + counter + current MovementInfo + height. Shared force ACK builder
    // converts canonical and transport positions to server coordinates.
    auto ack = buildForceAck(Opcode::CMSG_MOVE_SET_COLLISION_HGT_ACK, change.counter);
    ack.writeFloat(change.height);
    owner_.getSocket()->send(ack);
}

void MovementHandler::handleCollisionHeightBroadcast(network::Packet& packet) {
    if (!isActiveExpansion("wotlk")) return;
    const size_t start = packet.getReadPos();
    CollisionHeightChange change;
    if (!readCollisionHeightChange(packet, change, true)) {
        LOG_WARNING("MSG_MOVE_SET_COLLISION_HGT: invalid or truncated payload");
        return;
    }
    if (auto entity = owner_.getEntityManager().getEntity(change.guid)) {
        if (auto unit = std::dynamic_pointer_cast<Unit>(entity)) unit->setCollisionHeight(change.height);
    }
    if (change.guid == owner_.getPlayerGuid()) serverCollisionHeight_ = change.height;
    // A validated broadcast also advances the remote mover and transport
    // attachment through the ordinary movement path. It must never be ACKed.
