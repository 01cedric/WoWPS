...
void TransportManager::removeTransport(uint64_t guid) {
...
void TransportManager::updateLocalTransportPose(uint64_t guid, const glm::vec3& position,
                                                float yaw, bool docked) {
    auto* t = getTransport(guid);
    if (!t) return;
    t->locallyScheduled = true;
    t->worldCoords = true;
    ...
    t->serverYaw = yaw;
    t->hasServerYaw = true;
    t->rotation = glm::angleAxis(yaw, glm::vec3(0, 0, 1));
    t->atDockDwell = docked;
    updateTransformMatrices(*t);
    pushTransform(*t);
    applyDoodadMotionState(*t, !docked);
}

void TransportManager::updateTransportMovement(ActiveTransport& transport, float deltaTime) {
    if (transport.locallyScheduled) return;
