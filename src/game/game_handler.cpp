...
void GameHandler::disconnect() {
    if (localExploration_) { spellNameCache_.clear(); spellNameCacheLoaded_ = false; }
    localExploration_ = false;
...
void GameHandler::resetDbcCaches() {
    ...
    if (movementHandler_) {
        movementHandler_->resetTaxiDbcCache();
        movementHandler_->resetServerCollisionHeight();
    }
...
bool GameHandler::hasRegisteredLocalTransport(uint64_t guid) const {
    return transportManager_ && transportManager_->getTransport(guid);
}
...
bool GameHandler::completePlayerTransportWorldTransfer(
    ...
    if (!transport || (localExploration_ && !transport->locallyScheduled)) return false;
    ...
    if (playerTransportGuid_ != transportGuid) return false;
...
void GameHandler::updateM2TransportBoarding(const glm::vec3& playerCanonical, float deltaTime) {
    auto* tm = getTransportManager();
    if (!tm) return;

    // Contact grace uses elapsed wall time, not the physics-clamped delta.
    const auto deckNow = std::chrono::steady_clock::now();
    const float contactElapsed = shipDeckLastSample_.time_since_epoch().count() == 0
        ? deltaTime : std::chrono::duration<float>(deckNow - shipDeckLastSample_).count();
    shipDeckLastSample_ = deckNow;
    ...
    if (!isOnTransport()) {
        ...
        for (auto& [guid, transport] : tm->getTransports()) {
            const bool isClientShip = !transport.isM2 && transport.worldCoords &&
                                      (transport.useClientAnimation || transport.locallyScheduled);
            ...
            if (isClientShip) {
                ...
                if (TransportDeckContact::within(local.x, local.y, local.z) &&
                    TransportDeckContact::canBoard(local.x, local.y, local.z,
        ...
        if (bestGuid != 0) {
            ...
            if (tr) {
                ...
                if (isDeeprunTram) {
                    ...
                    LOG_INFO("[TRANSPORT_RIDER] board guid=0x", std::hex, bestGuid, std::dec,
                                " entry=", tr->entry,
    ...
    if (!tr->isM2 && tr->worldCoords && (tr->useClientAnimation || tr->locallyScheduled)) {
        const glm::vec3 playerRenderPos = core::coords::canonicalToRender(playerCanonical);
        const glm::vec3 local(tr->invTransform * glm::vec4(playerRenderPos, 1.0f));
        const bool collisionReady = tm->isTransportCollisionReady(getPlayerTransportGuid());
        // A jumping passenger still has this hull beneath them. Retain the
        // attachment across the jump arc; walking off has no hull floor at all.
        const bool hasDeckSupport = collisionReady &&
            tm->isPointOnTransportDeck(getPlayerTransportGuid(), playerCanonical, 8.0f);
        if (shipDeckContact_.shouldLeave(local.x, local.y, local.z, hasDeckSupport,
                                        collisionReady, contactElapsed)) {
            LOG_INFO("[TRANSPORT_RIDER] disembark guid=", getPlayerTransportGuid(),
                     " entry=", tr->entry, " supported=", hasDeckSupport,
                     " missingSeconds=", shipDeckContact_.unsupportedSeconds,
...
void GameHandler::openAuctionHouse(uint64_t guid) {
    if (localExploration_) {
        ...
        const auto* p = realm ? realm->localPlayer() : nullptr;
        if (!p || p->dead) return;
        bool near = false;
        for (const auto& n : realm->npcs()) {
            const float dx=n.x-p->x,dy=n.y-p->y,dz=n.z-p->z;
            if (n.guid==guid && n.auctioneer && !n.dead && !n.hostile && n.mapId==p->mapId && n.instanceId==p->instanceId && dx*dx+dy*dy+dz*dz<=64) near=true;
        }
        if (!near) return;
        auctioneerGuid_=guid; auctionOpen_=true; auctionActiveTab_=0; localAuctionQuery_={}; localAuctionSort_.clear();
        refreshLocalAuctions(true);
...
void GameHandler::closeAuctionHouse() {
    if (localExploration_) {
        const bool wasOpen=auctionOpen_; auctionOpen_=false; auctioneerGuid_=0;
        if (wasOpen && addonEventCallback_) addonEventCallback_("AUCTION_HOUSE_CLOSED",{});
        auctionBrowseResults_={};auctionOwnerResults_={};auctionBidderResults_={};
        localAuctionNames_.clear();localAuctionSort_.clear();localAuctionFingerprint_=0;
        return;
    }
...
void GameHandler::auctionSearch(const std::string& name, uint8_t levelMin, uint8_t levelMax,
    ...
    if (localExploration_) {
        if (!auctionOpen_) return;
        localAuctionQuery_={name,levelMin,levelMax,quality,itemClass,itemSubClass,invTypeMask,usableOnly,offset};
        localAuctionSort_=sort;
        refreshLocalAuctions(true); return;
    }
...
void GameHandler::auctionSellItem(int backpackIndex, uint32_t bid,
    ...
    if (localExploration_) {
        if (const auto* item=localBagSlot(backpackIndex))
            submitLocalAuction(item->item.itemId, uint16_t(item->item.stackCount), bid, buyout, duration);
        return;
    }
...
void GameHandler::auctionSellItemByGuid(uint64_t itemGuid, uint32_t stackCount, uint32_t bid,
    ...
    if (localExploration_) {
        uint64_t current=0;
        const auto* item=localBagSlot(int(itemGuid & 0xffffu)-1, &current);
        if (item && current==itemGuid && stackCount>0 && stackCount<=65535 && stackCount<=item->item.stackCount)
            submitLocalAuction(item->item.itemId, uint16_t(stackCount), bid, buyout, duration);
        return;
    }
...
void GameHandler::auctionPlaceBid(uint32_t auctionId, uint32_t amount) {
    if (localExploration_) {
        if (auto* realm=localAuctionRealm_?localAuctionRealm_():nullptr; realm && auctionOpen_) {
            if (!realm->bidAuction(auctionId, amount, auctioneerGuid_) && addonEventCallback_) addonEventCallback_("UI_ERROR_MESSAGE", {realm->actionStatus()});
            refreshLocalAuctions(true);
        }
        return;
    }
...
void GameHandler::auctionBuyout(uint32_t auctionId, uint32_t buyoutPrice) {
    if (localExploration_) {
        if (auto* realm=localAuctionRealm_?localAuctionRealm_():nullptr; realm && auctionOpen_) {
            if (!realm->buyoutAuction(auctionId, auctioneerGuid_) && addonEventCallback_) addonEventCallback_("UI_ERROR_MESSAGE", {realm->actionStatus()});
            refreshLocalAuctions(true);
        }
        return;
    }
...
void GameHandler::auctionCancelItem(uint32_t auctionId) {
    if (localExploration_) {
        if (auto* realm=localAuctionRealm_?localAuctionRealm_():nullptr; realm && auctionOpen_) {
            if (!realm->cancelAuction(auctionId, auctioneerGuid_) && addonEventCallback_) addonEventCallback_("UI_ERROR_MESSAGE", {realm->actionStatus()});
            refreshLocalAuctions(true);
        }
        return;
    }
...
void GameHandler::auctionListOwnerItems(uint32_t offset) {
    if (localExploration_) { refreshLocalAuctions(true); return; }
...
void GameHandler::loadMapNameCache() const {
    ...
    // WotLK (66 fields): 4=battleground flag, 5..20=localized names.
    // Retain the pre-existing index for older expansion layouts.
    for (uint32_t i = 0; i < dbc->getRecordCount(); ++i) {
        uint32_t id = dbc->getUInt32(i, 0);
        mapInstanceTypeCache_[id] = dbc->getUInt32(i, 2);
        const uint32_t nameColumn = dbc->getFieldCount() == 66 ? 5 : 4;
        std::string name = dbc->getString(i, nameColumn);
        if (name.empty() && nameColumn == 5) {
            for (uint32_t locale = 6; locale <= 20 && name.empty(); ++locale) name = dbc->getString(i, locale);
        }
...
void GameHandler::rebuildCompanions() const {
    ...
    // A fresh/local character with no known spells cannot own a companion.
    // Do not open the large Spell.dbc (or build its index) just to return zero
    // from the original PetPaperDollFrame's OnLoad callback.
    if (known.empty()) {
        LOG_INFO("[COMPANION_CACHE] empty spellbook; Spell.dbc not opened");
        return;
    }
...
float GameHandler::getServerCollisionHeight() const {
    return movementHandler_ ? movementHandler_->getServerCollisionHeight() : core::DEFAULT_COLLISION_HEIGHT;
}
