...
bool GameHandler::isAuctionHouseOpen() const {
    return !localExploration_ && inventoryHandler_ ? inventoryHandler_->isAuctionHouseOpen() : auctionOpen_;
}
uint64_t GameHandler::getAuctioneerGuid() const {
    return !localExploration_ && inventoryHandler_ ? inventoryHandler_->getAuctioneerGuid() : auctioneerGuid_;
}
const AuctionListResult& GameHandler::getAuctionBrowseResults() const {
    if (!localExploration_ && inventoryHandler_) return inventoryHandler_->getAuctionBrowseResults();
    return auctionBrowseResults_;
}
AuctionListResult& GameHandler::auctionBrowseResultsRef() {
    if (!localExploration_ && inventoryHandler_) return inventoryHandler_->auctionBrowseResultsRef();
    return auctionBrowseResults_;
}
const AuctionListResult& GameHandler::getAuctionOwnerResults() const {
    if (!localExploration_ && inventoryHandler_) return inventoryHandler_->getAuctionOwnerResults();
    return auctionOwnerResults_;
}
AuctionListResult& GameHandler::auctionOwnerResultsRef() {
    if (!localExploration_ && inventoryHandler_) return inventoryHandler_->auctionOwnerResultsRef();
    return auctionOwnerResults_;
}
const AuctionListResult& GameHandler::getAuctionBidderResults() const {
    if (!localExploration_ && inventoryHandler_) return inventoryHandler_->getAuctionBidderResults();
    return auctionBidderResults_;
}
AuctionListResult& GameHandler::auctionBidderResultsRef() {
    if (!localExploration_ && inventoryHandler_) return inventoryHandler_->auctionBidderResultsRef();
    return auctionBidderResults_;
}
int GameHandler::getAuctionActiveTab() const {
    return !localExploration_ && inventoryHandler_ ? inventoryHandler_->getAuctionActiveTab() : auctionActiveTab_;
}
void GameHandler::setAuctionActiveTab(int tab) {
    if (!localExploration_ && inventoryHandler_) inventoryHandler_->setAuctionActiveTab(tab);
    else auctionActiveTab_ = tab;
}
float GameHandler::getAuctionSearchDelay() const {
    return !localExploration_ && inventoryHandler_ ? inventoryHandler_->getAuctionSearchDelay() : 0.0f;
}
...
void GameHandler::dismount() {
    if(localExploration_ && localAuctionRealm_) {
        if(auto* realm=localAuctionRealm_())realm->dismount();
        return;
    }
