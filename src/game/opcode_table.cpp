...
static bool loadOpcodeJsonRecursive(const std::filesystem::path& path,
    ...
    // Non-throwing: weakly_canonical resolves through realpath()/getcwd(),
    // which the PS4 sandbox refuses for the read-only package tree. The path
    // as given is a perfectly good cycle key and file name when that fails.
    std::error_code canonicalEc;
    std::filesystem::path canonicalPath = std::filesystem::weakly_canonical(path, canonicalEc);
    if (canonicalEc || canonicalPath.empty()) canonicalPath = path;
