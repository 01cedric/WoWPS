...
enum class TrackType { VEC3, QUAT_COMPRESSED, FLOAT, FIXED16, BYTE_BOOL, SPLINE_VEC3, SPLINE_FLOAT };
...
void parseAnimTrack(const std::vector<uint8_t>& data,
    ...
    for (uint32_t i = 0; i < numSubArrays; i++) {
        ...
        if (disk.globalSequence < 0 && i < sequenceFlags.size() &&
            !(sequenceFlags[i] & kM2SeqFlagEmbeddedData)) continue;
        ...
        if (static_cast<uint64_t>(tsOffset) + static_cast<uint64_t>(tsCount) * sizeof(uint32_t) > data.size()) continue;
        ...
        else if (type == TrackType::SPLINE_VEC3) keyElementSize = sizeof(float) * 9;
        else if (type == TrackType::SPLINE_FLOAT) keyElementSize = sizeof(float) * 3;
        ...
        } else if (type == TrackType::SPLINE_VEC3) {
            // Camera paths: value, in-tangent, out-tangent per key.
            struct SplineVec3Disk { float value[3]; float inTangent[3]; float outTangent[3]; };
            auto values = readArray<SplineVec3Disk>(data, keyOffset, keyCount);
            track.sequences[i].vec3Values.reserve(values.size());
            for (const auto& v : values) {
                track.sequences[i].vec3Values.emplace_back(v.value[0], v.value[1], v.value[2]);
                track.sequences[i].spline().vec3InTangents.emplace_back(v.inTangent[0], v.inTangent[1], v.inTangent[2]);
                track.sequences[i].spline().vec3OutTangents.emplace_back(v.outTangent[0], v.outTangent[1], v.outTangent[2]);
            }
        } else if (type == TrackType::SPLINE_FLOAT) {
            struct SplineFloatDisk { float value, inTangent, outTangent; };
            for (const auto& v : readArray<SplineFloatDisk>(data, keyOffset, keyCount)) {
                track.sequences[i].floatValues.push_back(v.value);
                track.sequences[i].spline().floatInTangents.push_back(v.inTangent);
                track.sequences[i].spline().floatOutTangents.push_back(v.outTangent);
            }
        } else {
...
M2Model M2Loader::load(const std::vector<uint8_t>& m2Data) {
    ...
    // Cameras include the authored motion and roll of the glue scenes.
    if (header.nCameras > 0 && header.ofsCameras > 0) {
        ...
        if (header.version < 264) {
            ...
            cameraSeqFlags.reserve(model.sequences.size());
            for (const auto& seq : model.sequences) cameraSeqFlags.push_back(seq.flags);
            for (const auto& dc : disk) {
                pushCamera(dc.type, dc.fov, dc.farClip, dc.nearClip, dc.positionBase, dc.targetBase);
                // The path the camera flies: the same array-of-arrays track
                // layout as a bone's, with spline keys.
                M2TrackDisk positionTrack{}, targetTrack{}, rollTrack{};
                std::memcpy(&positionTrack, dc.positionTrack, sizeof(positionTrack));
                std::memcpy(&targetTrack, dc.targetTrack, sizeof(targetTrack));
                std::memcpy(&rollTrack, dc.rollTrack, sizeof(rollTrack));
                M2Camera& cam = model.cameras.back();
                parseAnimTrack(m2Data, positionTrack, cam.positions, TrackType::SPLINE_VEC3, cameraSeqFlags);
                parseAnimTrack(m2Data, targetTrack, cam.targets, TrackType::SPLINE_VEC3, cameraSeqFlags);
                parseAnimTrack(m2Data, rollTrack, cam.roll, TrackType::SPLINE_FLOAT, cameraSeqFlags);
            }
        }
...
bool M2Loader::loadSkin(const std::vector<uint8_t>& skinData, M2Model& model) {
    ...
    if (outOfBounds > 0) {
        // A mismatched/truncated SKIN must not turn triangles into long spikes
        // tied to vertex zero. Keep the model invalid so all callers reject it.
        core::Logger::getInstance().error("SKIN rejected: ", outOfBounds,
            " indices outside vertex/lookup data for ", model.name);
        model.indices.clear();
        model.batches.clear();
        return false;
    }
    ...
    if (header.nBatches > 0 && header.ofsBatches > 0) {
        ...
        for (const auto& db : diskBatches) {
            ...
            if (db.skinSectionIndex < submeshes.size()) {
                const auto& sm = submeshes[db.skinSectionIndex];
                // WotLK SKIN sections split the first index across two words.
                // `level` is its high word, not a geometry LOD selector. Large
                // character skins and glue scenes cross 65535 indices; using
                // just indexStart joins triangles from an unrelated section.
                batch.indexStart = static_cast<uint32_t>(sm.indexStart) |
                                   (static_cast<uint32_t>(sm.level) << 16);
                batch.indexCount = sm.indexCount;
                batch.vertexStart = sm.vertexStart;
                batch.vertexCount = sm.vertexCount;
                batch.submeshId = sm.id;
                ...
                // An invalid section cannot supply this material's topology.
                // Preserve the batch slot but draw nothing for it.
                batch.indexStart = 0;
                batch.indexCount = 0;
                batch.vertexStart = 0;
                batch.vertexCount = 0;
