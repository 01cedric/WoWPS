...
std::vector<CustomZoneInfo> CustomZoneDiscovery::scanDirectory(const std::string& path) {
    ...
    // Non-throwing: these are cwd-relative optional directories, and on a
    // console with no usable working directory a stat() that answers with
    // anything but ENOENT would otherwise throw out of the first world load.
    std::error_code ec;
    if (!fs::exists(path, ec) || ec) return results;
    ...
    for (fs::directory_iterator it(path, ec), end; it != end && !ec; it.increment(ec)) {
        ...
        if (!fs::exists(zoneJson, ec) || ec) continue;
