...
void CombatFSM::cancelSpellCast() {
    if (state_ != State::SPELL_PRECAST && state_ != State::SPELL_CASTING &&
        state_ != State::SPELL_FINALIZE) return;
    clearSpellState();
    state_ = State::INACTIVE;
}
...
AnimOutput CombatFSM::resolve(const Input& in, const AnimCapabilitySet& caps,
    ...
    const bool newSwing = state_ != State::MELEE_SWING || in.meleeSwingTimer > lastMeleeTimer_;
    lastMeleeTimer_ = in.meleeSwingTimer;
    updateTransitions(in);
    if (state_ != State::MELEE_SWING) latchedMeleeAnim_ = 0;
    ...
    switch (state_) {
        ...
        case State::MELEE_SWING:
            ...
            } else if (in.meleeAnimId != 0) {
                animId = in.meleeAnimId;
            } else if (!newSwing && latchedMeleeAnim_ != 0) {
                animId = latchedMeleeAnim_;
            } else {
                // Resolve once per swing, never once per rendered frame.
