...
void PostProcessPipeline::initialize(VkContext* ctx) {
    vkCtx_ = ctx;
#ifdef WOWEE_PS4
    const auto output = ctx ? ctx->getSwapchainExtent() : VkExtent2D{};
    const auto scene = fitPs4SceneExtent(output, configuredPs4SceneHeight());
    LOG_INFO("PS4 scene profile: world=", scene.width, "x", scene.height,
             " output/UI=", output.width, "x", output.height,
             "; menu stays native; WOWEE_PS4_SCENE_HEIGHT=720/900/native");
#endif
}
...
VkExtent2D PostProcessPipeline::desiredSceneExtent() const {
    if (!vkCtx_) return {};
    const auto output = vkCtx_->getSwapchainExtent();
#ifdef WOWEE_PS4
    // The PS4 water continuation path for MSAA resolves into native images.
    // Keep that path native; the normal console profile is single sampled.
    if (worldRendering_ && !fxaaAllocationFailed_ &&
        vkCtx_->getMsaaSamples() == VK_SAMPLE_COUNT_1_BIT)
        return fitPs4SceneExtent(output, configuredPs4SceneHeight());
#endif
    return output;
}

bool PostProcessPipeline::needsFXAAPass() const {
#ifdef WOWEE_PS4
    if (fxaaAllocationFailed_) return false;
    if (vkCtx_) {
        const auto scene = desiredSceneExtent();
        const auto output = vkCtx_->getSwapchainExtent();
...
void PostProcessPipeline::setWorldRendering(bool active) {
    if (worldRendering_ == active) return;
    worldRendering_ = active;
#ifdef WOWEE_PS4
    // Retrying once on a real mode transition is bounded; never retry an
    // allocation failure every frame. Destruction happens in manageResources.
    fxaaAllocationFailed_ = false;
    const auto wanted = desiredSceneExtent();
    if (fxaa_.sceneFramebuffer && (wanted.width != fxaa_.sceneExtent.width ||
                                  wanted.height != fxaa_.sceneExtent.height))
        fxaa_.needsRecreate = true;
#endif
}

void PostProcessPipeline::manageResources() {
#ifdef WOWEE_PS4
    // A discarded recording can retain its acquired VideoOut image for a
    // retry. Do not replace attachments while that frame still owns them.
    if (vkCtx_ && vkCtx_->hasAcquiredFrameImage()) return;
#endif
...
#ifdef WOWEE_PS4
    // The scene height is a live setting, so the target it sizes has to be
    // rebuilt when it changes. The only place that compared the two was
    // setWorldRendering, which early-returns unless the IN_GAME state flips -
    // so a resolution chosen while already in the world moved the atomic and
    // nothing else, and the world went on being drawn at the extent it already
    // had. That is the whole of "I cannot change the resolution in game".
    if (fxaa_.sceneFramebuffer) {
        const auto wanted = desiredSceneExtent();
        if (wanted.width != fxaa_.sceneExtent.width ||
            wanted.height != fxaa_.sceneExtent.height) {
            fxaa_.needsRecreate = true;
        }
    }
    ...
    if ((fxaa_.needsRecreate || !useFXAA) && fxaa_.sceneFramebuffer) {
        if (!destroyFXAAResources()) return;
...
void PostProcessPipeline::destroyAllResources() {
    ...
    if (fxaa_.sceneFramebuffer && !destroyFXAAResources()) return;
}
...
#ifdef WOWEE_PS4
    if (enabled) {
        LOG_WARNING("PS4 FSR1 unavailable; the fixed world-resolution profile and FXAA remain available");
        enabled = false;
    }
#endif
...
MsaaChangeRequest PostProcessPipeline::setFSR2Enabled(bool enabled, Camera* camera) {
    MsaaChangeRequest req;
#ifdef WOWEE_PS4
    // FSR2's history handling issues a scaled vkCmdBlitImage, which the
    // ps4_vulkan ICD rejects (docs/ps4.md); a saved setting must not turn it on.
    if (enabled) {
        LOG_WARNING("FSR2 is not available on the PS4 renderer; staying off");
        enabled = false;
    }
#endif
...
bool PostProcessPipeline::initFXAAResources() {
    ...
    VkDevice device = vkCtx_->getDevice();
    VmaAllocator alloc = vkCtx_->getAllocator();
    VkExtent2D ext = desiredSceneExtent();
    if (!ext.width || !ext.height) return false;
    fxaa_.sceneExtent = ext;
    VkSampleCountFlagBits msaa = vkCtx_->getMsaaSamples();
    bool useMsaa = (msaa > VK_SAMPLE_COUNT_1_BIT);
    bool useDepthResolve = (vkCtx_->getDepthResolveImageView() != VK_NULL_HANDLE);
    ...
    layoutInfo.bindingCount = 1;
    layoutInfo.pBindings = &binding;
    if (vkCreateDescriptorSetLayout(device, &layoutInfo, nullptr, &fxaa_.descSetLayout) != VK_SUCCESS) {
        destroyFXAAResources();
        return false;
    }
    ...
    poolInfo.poolSizeCount = 1;
    poolInfo.pPoolSizes = &poolSize;
    if (vkCreateDescriptorPool(device, &poolInfo, nullptr, &fxaa_.descPool) != VK_SUCCESS) {
        destroyFXAAResources();
        return false;
    }
    ...
    if (vkAllocateDescriptorSets(device, &dsAllocInfo, fxaa_.descSet) != VK_SUCCESS) {
        destroyFXAAResources();
        return false;
    }
    ...
    plCI.pushConstantRangeCount = 1;
    plCI.pPushConstantRanges = &pc;
    if (vkCreatePipelineLayout(device, &plCI, nullptr, &fxaa_.pipelineLayout) != VK_SUCCESS) {
        destroyFXAAResources();
        return false;
    }
...
bool PostProcessPipeline::destroyFXAAResources() {
    if (!vkCtx_) return true;
    VkDevice device = vkCtx_->getDevice();
    VmaAllocator alloc = vkCtx_->getAllocator();
    if (!vkCtx_->waitIdleForResourceChange("post-process scene resources")) {
        return false;
    }
...
void PostProcessPipeline::renderFXAAPass() {
    ...
    VkExtent2D ext = vkCtx_->getSwapchainExtent();
#ifdef WOWEE_PS4
    // Pixel offsets address the source image, while the output viewport and
    // subsequent ImGui pass retain the native display size.
    ext = fxaa_.sceneExtent;
#endif
...
#ifdef WOWEE_PS4
    if (!fxaa_.enabled) sharpness = -1.0f; // scaling/effects without AA
#endif
    float pc[4] = {
