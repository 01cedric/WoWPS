    ...
    if (!enabled && vkCtx) {
        vkCtx->immediateSubmit([&](VkCommandBuffer cmd) {
            for (auto& sh : sceneHistory) {
              for (VkImage colorImage : {sh.colorImage, sh.tiledColorImage}) {
                ...
                vkCmdClearColorImage(cmd, colorImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &clearColor, 1, &range);
...
void WaterRenderer::destroySceneHistoryResources() {
    ...
    for (auto& sh : sceneHistory) {
        ...
        if (sh.tiledColorView) { vkDestroyImageView(device, sh.tiledColorView, nullptr); sh.tiledColorView = VK_NULL_HANDLE; }
        if (sh.tiledColorImage) { vmaDestroyImage(vkCtx->getAllocator(), sh.tiledColorImage, sh.tiledColorAlloc); sh.tiledColorImage = VK_NULL_HANDLE; sh.tiledColorAlloc = VK_NULL_HANDLE; }
...
void WaterRenderer::prepareSceneHistory(VkExtent2D extent) {
#ifdef WOWEE_PS4
    if (!vkCtx || !extent.width || !extent.height ||
        (extent.width == sceneHistoryExtent.width && extent.height == sceneHistoryExtent.height))
        return;
    // Never destroy descriptors/images while the current frame is being
    // recorded. Renderer invokes this before acquiring its frame image.
    if (vkCtx->hasAcquiredFrameImage()) return;
    if (!vkCtx->waitIdleForResourceChange("water history resize")) return;
    LOG_INFO("PS4 water capture extent: ", extent.width, "x", extent.height,
             "; direct color/depth copies, no scaled blits");
    createSceneHistoryResources(extent, vkCtx->getSwapchainFormat(), vkCtx->getDepthFormat());
#else
    (void)extent;
#endif
}
...
void WaterRenderer::createSceneHistoryResources(VkExtent2D extent, VkFormat colorFormat, VkFormat depthFormat) {
...
#ifdef WOWEE_PS4
    // A failed resize must neither publish a partial descriptor family nor
    // retry allocation on every frame. Keep the attempted extent after
    // cleanup; another real extent/mode change can attempt it again.
    struct HistoryRollback {
        WaterRenderer* renderer;
        VkExtent2D attempted;
        bool committed = false;
        ~HistoryRollback() {
            if (committed) return;
            renderer->destroySceneHistoryResources();
            renderer->sceneHistoryExtent = attempted;
        }
    } rollback{this, extent};
#endif
...
#ifdef WOWEE_PS4
    // Native VideoOut colour is linear. Depth history must retain DB tiling;
    // a generic sampled depth texture has a different physical surface layout.
    colorImgInfo.tiling = VK_IMAGE_TILING_LINEAR;
...
#ifdef WOWEE_PS4
        VkImageCreateInfo tiledColorInfo = colorImgInfo;
        tiledColorInfo.tiling = VK_IMAGE_TILING_OPTIMAL;
        tiledColorInfo.usage |= VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT;
        if (vmaCreateImage(vkCtx->getAllocator(), &tiledColorInfo, &allocCI,
                          &sh.tiledColorImage, &sh.tiledColorAlloc, nullptr) != VK_SUCCESS) {
            LOG_ERROR("WaterRenderer: tiled scene color history allocation failed [", f, "]");
            return;
        }
#endif
...
#ifdef WOWEE_PS4
        VkImageViewCreateInfo tiledViewInfo = colorViewInfo;
        tiledViewInfo.image = sh.tiledColorImage;
        if (vkCreateImageView(device, &tiledViewInfo, nullptr, &sh.tiledColorView) != VK_SUCCESS) {
            LOG_ERROR("WaterRenderer: tiled scene color history view failed [", f, "]");
            return;
        }
#endif
    ...
    // Initialize deterministic history before any water draw; a layout change
    // alone never initializes the GPU allocation's pixels.
    bool historyInitialized = false;
#ifdef WOWEE_PS4
    try {
#endif
    historyInitialized = vkCtx->immediateSubmit([&](VkCommandBuffer cmd) {
        for (auto& sh : sceneHistory) {
            if (!sh.colorImage || !sh.depthImage) continue;
            VkImageMemoryBarrier2 barriers[3]{};
            const uint32_t imageCount = sh.tiledColorImage ? 3 : 2;
            for (uint32_t i = 0; i < imageCount; ++i) {
                ...
                b.image = i == 0 ? sh.colorImage : (i == 1 ? sh.depthImage : sh.tiledColorImage);
                b.subresourceRange = {.aspectMask = i == 1 ? VK_IMAGE_ASPECT_DEPTH_BIT : VK_IMAGE_ASPECT_COLOR_BIT,
            ...
            const VkClearDepthStencilValue farDepth{1.0f, 0};
            vkCmdClearColorImage(cmd, sh.colorImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                &black, 1, &barriers[0].subresourceRange);
            vkCmdClearDepthStencilImage(cmd, sh.depthImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                &farDepth, 1, &barriers[1].subresourceRange);
            if (sh.tiledColorImage) vkCmdClearColorImage(cmd, sh.tiledColorImage,
                VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &black, 1, &barriers[2].subresourceRange);
            for (uint32_t i = 0; i < imageCount; ++i) {
                auto& b = barriers[i];
        ...
        // Even logging a submission failure can allocate and throw. Confirm
        // completion before unwinding through the image rollback guard.
        if (!vkCtx->waitIdleForResourceChange("water initialization exception")) {
    ...
    if (!historyInitialized) {
        if (!vkCtx->waitIdleForResourceChange("failed water initialization")) {
            // The failed initialization might still reference these images.
            // Keep them owned and stop; do not free them through rollback.
            rollback.committed = true;
            return;
        }
        LOG_ERROR("PS4 water initialization failed; incomplete history discarded");
        return;
    }
    rollback.committed = true;
#else
    (void)historyInitialized;
#endif
}
...
#ifdef WOWEE_PS4
    if (renderExtent_.width != sceneHistoryExtent.width || renderExtent_.height != sceneHistoryExtent.height)
        return; // a skipped resize/capture must not sample a stale-size history
#endif
...
void WaterRenderer::captureSceneHistory(VkCommandBuffer cmd,
...
#ifdef WOWEE_PS4
    // The ICD cannot scale tiled color/depth copies. A missed preparation
    // must not record unsupported blits or sample a partially copied image.
    // Preparation and any allocations are exclusively between frames.
    if (srcExtent.width != sceneHistoryExtent.width || srcExtent.height != sceneHistoryExtent.height) {
        sceneHistoryReady = false;
        if (renderDiagCounter_++ % 300 == 0)
            LOG_WARNING("PS4 water capture skipped: source/history extent mismatch");
        return;
    }
#endif

    VkImage historyColor = sh.colorImage;
#ifdef WOWEE_PS4
    VkImageView historyView = sh.colorView;
    if (!vkPs4ImageHasLinearStorage(srcColorImage)) {
        historyColor = sh.tiledColorImage;
        historyView = sh.tiledColorView;
    }
    if (!historyColor || !historyView) return;
    VkDescriptorImageInfo colorInfo{sceneColorSampler, historyView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL};
    VkWriteDescriptorSet write{.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET};
    write.dstSet = sh.sceneSet; write.dstBinding = 0; write.descriptorCount = 1;
    ...
    barrier2(historyColor, VK_IMAGE_ASPECT_COLOR_BIT,
    ...
    if (needsScaling) {
        ...
        vkCmdBlitImage(cmd, srcColorImage, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                       historyColor, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
        ...
        vkCmdCopyImage(cmd, srcColorImage, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                       historyColor, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &colorCopy);
    }
