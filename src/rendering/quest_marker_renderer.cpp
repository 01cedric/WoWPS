...
{
    if (!ctx || !assetManager || perFrameLayout == VK_NULL_HANDLE) {
        LOG_WARNING("QuestMarkerRenderer: Missing context, camera layout or assets");
        return false;
    }
    ...
    // --- Push constants: model (64), alpha (4), grayscale (4) ---
    ...
    if (!createQuad() || !loadTextures(assetManager)) {
        LOG_WARNING("QuestMarkerRenderer: No usable original marker artwork/resources");
        shutdown();
        return false;
    }
...
bool QuestMarkerRenderer::createDescriptorResources() {
    VkDevice device = vkCtx_->getDevice();
    ...
    if (materialSetLayout_ == VK_NULL_HANDLE) return false;
    ...
    if (vkAllocateDescriptorSets(device, &allocInfo, texDescSets_) != VK_SUCCESS) {
        ...
        for (auto& set : texDescSets_) set = VK_NULL_HANDLE;
        return false;
    }
    return true;
}

bool QuestMarkerRenderer::createQuad() {
...
bool QuestMarkerRenderer::loadTextures(pipeline::AssetManager* assetManager) {
    const char* paths[3] = {
    ...
    // Some clients omit the separate incomplete icon. Use the original active
    // question mark in that case; marker type 2 always desaturates it below.
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        if (assetManager->fileExists(paths[i])) blp = assetManager->loadTexture(paths[i]);
        if (!blp.isValid() && fallback[i]) {
            if (assetManager->fileExists(fallback[i])) blp = assetManager->loadTexture(fallback[i]);
            if (blp.isValid()) {
        ...
        if (!textures_[i].createSampler(device, VK_FILTER_LINEAR, VK_FILTER_LINEAR,
                VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE) || !textures_[i].isValid()) {
...
void QuestMarkerRenderer::setMarker(uint64_t guid, const glm::vec3& position, int markerType,
    ...
    if (markerType < 0 || markerType > 2 || !std::isfinite(position.x) ||
        !std::isfinite(position.y) || !std::isfinite(position.z)) {
        markers_.erase(guid);
        return;
    }
    if (!std::isfinite(boundingHeight) || boundingHeight < 0.0f) boundingHeight = 2.0f;
    if (!std::isfinite(grayscale)) grayscale = 0.0f;
    grayscale = markerType == 2 ? 1.0f : glm::clamp(grayscale, 0.0f, 1.0f);
...
void QuestMarkerRenderer::render(VkCommandBuffer cmd, VkDescriptorSet perFrameSet, const Camera& camera) {
    if (markers_.empty() || pipeline_ == VK_NULL_HANDLE || quadVB_ == VK_NULL_HANDLE ||
        cmd == VK_NULL_HANDLE || perFrameSet == VK_NULL_HANDLE) return;

    // WoW-style quest marker tuning parameters
    constexpr float BASE_SIZE = 0.95f;          // Legible at controller camera distance
    constexpr float HEAD_CLEARANCE = 0.25f;    // Bottom of icon above NPC bounds
    ...
    for (const auto& [guid, marker] : markers_) {
        ...
        markerPos.z += marker.boundingHeight + HEAD_CLEARANCE + size * 0.5f + bob;

        // Cull the actual floating icon, not the NPC's feet. A close camera
        // may show the head while the foot point is below the screen.
        if (!frustum.intersectsSphere(markerPos, size * 0.71f)) continue;
