...
#ifdef WOWEE_PS4
    // Closer third-person framing requested for the console. Keep the normal
    // zoom controls and collision sweep; this changes only the starting zoom.
    userTargetDistance = currentDistance = collisionDistance = 5.0f;
    LOG_INFO("PS4 camera: initial follow distance=5; D-pad up/down adjusts zoom");
#endif
    yaw = defaultYaw;
...
void CameraController::startIntroPan(float durationSec, float orbitDegrees) {
    if (!camera || scriptedView_) return;
    introActive = true;
    introTimer = 0.0f;
    idleTimer_ = 0.0f;
...
void CameraController::cancelIntroPan() {
    introActive = false;
    introTimer = 0.0f;
    idleOrbit_ = false;
    idleTimer_ = 0.0f;
...
void CameraController::clearScriptedViewInputs() {
    // The normal update cannot send its stop edges while a cinematic owns the
    // frame. Publish them here before clearing the corresponding latches.
    if (movementCallback) {
        if (wasMovingForward || wasMovingBackward || autoRunning)
            movementCallback(static_cast<uint32_t>(game::Opcode::MSG_MOVE_STOP));
        if (wasStrafingLeft || wasStrafingRight)
            movementCallback(static_cast<uint32_t>(game::Opcode::MSG_MOVE_STOP_STRAFE));
        if (wasTurningLeft || wasTurningRight)
            movementCallback(static_cast<uint32_t>(game::Opcode::MSG_MOVE_STOP_TURN));
        if (wasAscending_ || wasDescending_)
    ...
    releaseMouseCapture();
    scriptedInputReleasePending_ = true;
}

void CameraController::updateScriptedInputRelease() {
    if (!scriptedInputReleasePending_) return;
    const auto& input = core::Input::getInstance();
    // Include toggle keys as well as movement: a held autorun or jump button
    // used to dismiss a cinematic must not become a fresh press on return.
    constexpr SDL_Scancode controls[] = {
        ...
        SDL_SCANCODE_GRAVE, SDL_SCANCODE_NUMLOCKCLEAR, SDL_SCANCODE_X,
        SDL_SCANCODE_R
    };
    for (SDL_Scancode key : controls) {
        if (input.isKeyPressed(key)) return;
    }
    if (input.isMouseButtonPressed(SDL_BUTTON_LEFT) ||
        input.isMouseButtonPressed(SDL_BUTTON_RIGHT)) return;
    scriptedInputReleasePending_ = false;
}

void CameraController::setScriptedView(bool active) {
    if (scriptedView_ == active) return;
    scriptedView_ = active;
    cancelIntroPan();
    clearScriptedViewInputs();
    if (!camera) return;
    if (active) {
        scriptedSavedCameraPos_ = camera->getPosition();
        return;
    }

    // Never integrate a cinematic's position into the follow camera or write
    // its position/orientation back to the player. Snap to the preserved view;
    // ordinary collision/zoom smoothing resumes on the following frame.
    camera->setRotation(yaw, pitch);
    smoothedCamPos = scriptedSavedCameraPos_;
    if (thirdPerson && followTarget) {
        const float mountedOffset = mounted_ ? mountHeightOffset_ : 0.0f;
        const glm::vec3 pivot = *followTarget +
            glm::vec3(0.0f, 0.0f, pivotHeight_ + mountedOffset);
        smoothedCamPos = pivot - camera->getForward() * currentDistance;
    }
    smoothedCollisionDist_ = smoothedTerrainDist_ = -1.0f;
    cameraDeflectDeg_ = waterNudgeZ_ = 0.0f;
...
glm::vec3 CameraController::sweepAgainstWalls(const glm::vec3& from, const glm::vec3& to,
    ...
    for (int i = 0; i < steps; i++) {
        ...
        if (wmoRenderer) {
            glm::vec3 adjusted;
            if (wmoRenderer->checkWallCollision(stepPos, candidate, adjusted, cachedInsideWMO, collisionHeight_)) {
        ...
        if (includeDoodads && m2Renderer && !externalFollow_) {
            glm::vec3 adjusted;
            if (m2Renderer->checkCollision(stepPos, candidate, adjusted, 0.5f, collisionHeight_)) {
                candidate.x = adjusted.x;
                candidate.y = adjusted.y;
            }
        }
...
CameraController::FloorSample CameraController::sampleFloorUnderFeet(const glm::vec3& targetPos,
    ...
    if (useCached) {
        ...
        if (wmoH && !ignoreSlopeLimit_ && wmoNormalZ < minWalkableWmo) {
            const float rejectedHeight = *wmoH;
            const float rejectedNormal = wmoNormalZ;
            wmoH = std::nullopt;
            // A nearby steep decoration must not hide the walkable floor
            // under it. Retry only this rejected query, with the same probe,
            // feet reference and already-decided interior/tunnel slope limit.
            // Ordinary walkable hits still perform just one WMO query.
            if (wmoRenderer) {
                const float feetRef = grounded ? targetPos.z : lastGroundZ;
                try {
                    wmoH = wmoRenderer->getFloorHeight(px, py, wmoProbeZ,
                        &wmoNormalZ, feetRef, minWalkableWmo);
                } catch (const std::exception& e) {
                    LOG_ERROR("WMO walkable floor retry: ", e.what());
                }
            }
            static std::chrono::steady_clock::time_point lastSlopeRetryLog{};
            const auto now = std::chrono::steady_clock::now();
            if (now - lastSlopeRetryLog > std::chrono::seconds(1)) {
                lastSlopeRetryLog = now;
                LOG_WARNING("[WMO_FLOOR_RETRY] feet=", targetPos.z, " probe=", wmoProbeZ,
                    " rejected=", rejectedHeight, " normalZ=", rejectedNormal,
                    " minNormalZ=", minWalkableWmo, " replacement=", wmoH ? *wmoH : -99999.0f);
            }
        }
...
void CameraController::update(float deltaTime) {
    if (!enabled || !camera || scriptedView_) {
        return;
    }
    updateScriptedInputRelease();
    ...
    bool uiWantsKeyboard = scriptedInputReleasePending_ || ImGui::GetIO().WantTextInput ||
    ...
    // Pressing L3 while the stick is still held forward must survive its
    // release. A new forward press or any backward input cancels autorun.
    if ((keyW && !manualForwardWasDown_ && !tildeDown) || keyS) {
        autoRunning = false;
        doCancelAutoFollow();
    }

    manualForwardWasDown_ = keyW;
...
void CameraController::processMouseMotion(const SDL_MouseMotionEvent& event) {
    if (!enabled || !camera || scriptedView_ || scriptedInputReleasePending_) {
        return;
    }
    if (introActive) {
        return;
...
void CameraController::processMouseButton(const SDL_MouseButtonEvent& event) {
    if (!enabled || scriptedView_ || scriptedInputReleasePending_) {
        return;
    }
...
void CameraController::applyLookDelta(float dxPixels, float dyPixels) {
    if (!enabled || !camera || introActive || scriptedView_ || scriptedInputReleasePending_) return;
...
void CameraController::processMouseWheel(float delta) {
    if (!enabled || scriptedView_ || scriptedInputReleasePending_) return;
...
void CameraController::triggerMountJump() {
    if (scriptedView_ || scriptedInputReleasePending_) return;
