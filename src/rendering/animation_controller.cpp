...
void AnimationController::startSpellCast(uint32_t precastAnimId, uint32_t castAnimId, bool castLoop,
    ...
    // A previous attack's timer is a forced CombatFSM override. Leaving it
    // active would replace the new wind-up on the next animation update.
    meleeSwingTimer_ = 0.0f;
    meleeSwingFresh_ = meleeSwingRestart_ = false;
    specialAttackAnimId_ = 0;
    rangedShootTimer_ = 0.0f;
    rangedAnimId_ = 0;
    characterAnimator_.setRangedWeaponActive(false);
    if (restoreWeaponAfterRangedShot_) {
...
void AnimationController::cancelSpellCast() {
    characterAnimator_.cancelSpellCast();
}
...
void AnimationController::setDead(bool dead) {
    if (dead_ == dead) return;
    dead_ = dead;
    resetCombatVisualState();
    if (!dead_ && renderer_ && renderer_->getCharacterRenderer()) {
        const auto instance = renderer_->getCharacterInstanceId();
        if (instance) renderer_->getCharacterRenderer()->playAnimation(instance, anim::STAND, true);
...
void AnimationController::triggerMeleeSwing() {
    ...
    meleeSwingFresh_ = meleeSwingRestart_ = true;
    LOG_INFO("[MELEE_ANIM] queued instance=", characterInstanceId, " inventoryType=", weaponLoadout_.inventoryType,
             " animation=", meleeAnimId_, " durationMs=", meleeAnimDurationMs_);
...
void AnimationController::updateCharacterAnimation() {
    ...
    if (dead_) {
        if (characterRenderer && characterInstanceId) {
            uint32_t current = 0; float time = 0, duration = 0;
            if (!characterRenderer->getAnimationState(characterInstanceId, current, time, duration) ||
                current != anim::DEATH)
                characterRenderer->playAnimation(characterInstanceId, anim::DEATH, false);
        }
        return;
    }
    ...
    const bool freshMelee = meleeSwingRestart_ && meleeSwingTimer_ > 0.0f && !loop &&
        animId == (specialAttackAnimId_ ? specialAttackAnimId_ : meleeAnimId_);
    const bool shouldPlay = requestChanged || drifted || freshMelee;
    ...
    if (shouldPlay) {
        if (freshMelee) {
            meleeSwingRestart_ = false;
            LOG_INFO("[MELEE_ANIM] playing instance=", characterInstanceId, " animation=", animId,
                     " grounded=", fi.grounded, " moving=", fi.moving);
        }
        characterRenderer->playAnimation(characterInstanceId, animId, loop);
        lastPlayerAnimRequest_ = animId;
        lastPlayerAnimLoopRequest_ = loop;
