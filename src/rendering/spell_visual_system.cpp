...
void SpellVisualSystem::shutdown() {
    ...
    acherusModelsScanned_=false;acherusPortalPath_.clear();acherusTransitionPath_.clear();
}
...
void SpellVisualSystem::resolveAcherusModels() {
    if(acherusModelsScanned_)return;
    if(!cachedAssetManager_)cachedAssetManager_=core::Application::getInstance().getAssetManager();
    if(!cachedAssetManager_)return;
    acherusModelsScanned_=true;
    const auto dbc=cachedAssetManager_->loadDBC("SpellVisualEffectName.dbc");
    if(!dbc || !dbc->isLoaded() || dbc->getFieldCount()<3)return;
    int portalScore=0,transitionScore=0;
    for(uint32_t row=0;row<dbc->getRecordCount();++row) {
        auto path=dbc->getString(row,2);
        std::string key=path;
        for(auto& c:key)c=char(std::tolower(static_cast<unsigned char>(c)));
        if(key.size()<4)continue;
        if(key.ends_with(".mdx") || key.ends_with(".mdl"))path.replace(path.size()-4,4,".m2");
        else if(!key.ends_with(".m2"))continue;
        const bool deathGate=key.find("deathgate")!=std::string::npos || key.find("death_gate")!=std::string::npos;
        const bool acherus=key.find("acherus")!=std::string::npos;
        const bool portal=key.find("portal")!=std::string::npos || key.find("teleport")!=std::string::npos;
        int score=deathGate?100:acherus&&portal?120:0;
        if(score && (key.find("state")!=std::string::npos || key.find("loop")!=std::string::npos))score+=20;
        if(key.find("impact")!=std::string::npos || key.find("cast")!=std::string::npos)score=0;
        int impact=(deathGate || acherus)?100:0;
        if(key.find("impact")==std::string::npos && key.find("teleport")==std::string::npos)impact=0;
        if(score<=portalScore && impact<=transitionScore)continue;
        if(!cachedAssetManager_->fileExists(path))continue;
        if(score>portalScore) {portalScore=score;acherusPortalPath_=path;}
        if(impact>transitionScore) {transitionScore=impact;acherusTransitionPath_=path;}
    }
    if(acherusTransitionPath_.empty())acherusTransitionPath_=acherusPortalPath_;
    LOG_INFO("[LOCAL_ACHERUS_FX] persistent=",acherusPortalPath_," arrival=",acherusTransitionPath_,
        " found=",!acherusPortalPath_.empty());
}

uint32_t SpellVisualSystem::loadWorldEffectModel(const std::string& path) {
    if(!m2Renderer_ || !cachedAssetManager_ || path.empty())return 0;
    uint32_t modelId=0;
    if(auto found=spellVisualModelIds_.find(path);found!=spellVisualModelIds_.end())modelId=found->second;
    else {
        if(nextSpellVisualModelId_>=999800)return 0;
        modelId=nextSpellVisualModelId_++;spellVisualModelIds_[path]=modelId;
    }
    if(m2Renderer_->hasModel(modelId))return modelId;
    if(spellVisualFailedModels_.count(modelId))return 0;
    const auto bytes=cachedAssetManager_->readFile(path);
    if(bytes.empty()) {spellVisualFailedModels_.insert(modelId);return 0;}
    auto model=pipeline::M2Loader::load(bytes);model.name=path;
    if(model.vertices.empty() && model.particleEmitters.empty() && model.ribbonEmitters.empty()) {
        spellVisualFailedModels_.insert(modelId);return 0;
    }
    if(model.version>=264) {
        const auto skin=cachedAssetManager_->readFile(pipeline::skinPathForM2(path));
        if(!skin.empty())pipeline::M2Loader::loadSkin(skin,model);
    }
    if(!m2Renderer_->loadModel(model,modelId)) {spellVisualFailedModels_.insert(modelId);return 0;}
    m2Renderer_->markModelAsSpellEffect(modelId);
    return modelId;
}

void SpellVisualSystem::setAcherusPortals(const std::vector<std::pair<uint64_t,glm::vec3>>& pads) {
    if(!m2Renderer_)return;
    for(auto it=acherusInstances_.begin();it!=acherusInstances_.end();) {
        if(std::none_of(pads.begin(),pads.end(),[&](const auto& p){return p.first==it->first;})) {
            m2Renderer_->removeInstance(it->second);it=acherusInstances_.erase(it);
        } else ++it;
    }
    if(pads.empty())return;
    resolveAcherusModels();
    const auto modelId=loadWorldEffectModel(acherusPortalPath_);if(!modelId)return;
    for(const auto& [key,pos]:pads) {
        if(!std::isfinite(pos.x)||!std::isfinite(pos.y)||!std::isfinite(pos.z))continue;
        if(auto found=acherusInstances_.find(key);found!=acherusInstances_.end()) {
            m2Renderer_->setInstancePosition(found->second,pos);continue;
        }
        if(acherusInstances_.size()>=4)break;
        const auto id=m2Renderer_->createInstance(modelId,pos,glm::vec3(0.f),1.f,false);
        if(id) {acherusInstances_[key]=id;m2Renderer_->setInstanceAnimation(id,0,true);}
    }
}

void SpellVisualSystem::playAcherusTransition(const glm::vec3& position) {
    resolveAcherusModels();
    const auto model=loadWorldEffectModel(acherusTransitionPath_);
    if(!model || activeSpellVisuals_.size()>=128)return;
    const auto id=m2Renderer_->createInstance(model,position,glm::vec3(0.f),1.f,false);
    if(id)activeSpellVisuals_.push_back({id,0.f,2.f,false,0,0});
}

void SpellVisualSystem::reset() {
    for(const auto& [key,id]:acherusInstances_) if(m2Renderer_)m2Renderer_->removeInstance(id);
    acherusInstances_.clear();
