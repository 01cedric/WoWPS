...
bool M2Renderer::buildMainPassPipelines(VkDescriptorSetLayout perFrameLayout) {
    ...
    // loc 5: vec2 uv1 (32), pad (40), loc 3: vec4 boneWeights (48), loc 4: vec4 boneIndices (64)
    VkVertexInputBindingDescription m2Binding{};
    m2Binding.binding = 0;
    m2Binding.stride = 20 * sizeof(float);
    ...
    std::vector<VkVertexInputAttributeDescription> m2Attrs = {
        ...
        {.location = 3, .binding = 0, .format = VK_FORMAT_R32G32B32A32_SFLOAT, .offset = 12 * sizeof(float)}, // boneWeights
        {.location = 4, .binding = 0, .format = VK_FORMAT_R32G32B32A32_SFLOAT, .offset = 16 * sizeof(float)}, // boneIndices (float)
    };
...
bool M2Renderer::initialize(VkContext* ctx, VkDescriptorSetLayout perFrameLayout,
    ...
    LOG_INFO("M2 render diagnostics available (NO_PARTICLES/NO_RIBBONS/NO_SKINNING): ",
             ...
             // The budget, on the same line, because "particles=on" alone does
             // not say how many. A log from a busy scene is read against these
             // two numbers and the periodic "frame budget full" line.
             " particleFrameBudget=", PARTICLE_FRAME_BUDGET,
             " particleEmitterSoftLimit=", PARTICLE_EMITTER_SOFT_LIMIT);
...
#ifdef WOWEE_PS4
    // The world and sky each construct an M2 renderer. Reserving 65,536
    // large CPU instances in both wastes flexible memory before any spawn.
    // Grow with actual population; this is a reservation, not an instance cap.
    instances.reserve(256);
    LOG_INFO("M2 CPU instance storage: stride=", sizeof(M2Instance),
             " capacity=", instances.capacity(), " bytes=", instances.capacity() * sizeof(M2Instance));
#else
    ...
    instanceCapacity_[0] = instanceCapacity_[1] = MAX_INSTANCE_DATA;
...
#else
    // Current-camera CPU tests already run while classifying draws. A GPU
    // readback adds a full frame-slot round trip (hundreds of milliseconds at
    // low frame rates), hides newly visible trees, and duplicates that work.
    // Keep GPU skinning/rendering; only visibility classification stays on CPU.
    LOG_INFO("PS4 M2 visibility: current-camera CPU frustum, no delayed readback");
#endif
    ...
    // --- Create dynamic particle buffers (mapped for CPU writes) ---
    {
        ...
        // M2 particle buffer.
        //
        // One frame's whole budget, once per frame in flight. It was one
        // instance's cap for the entire buffer, which forced the draw path to
        // rewind to offset zero for every texture group and left every group
        // drawing the last one's vertices; and with a single region the CPU
        // write for frame N+1 lands in what frame N's queued draws still read.
        // Both go away by giving each frame its own half and each group its
        // own run within it.
        particleVBVertices_ = PARTICLE_FRAME_BUDGET * MAX_FRAMES_IN_FLIGHT;
        bci.size = particleVBVertices_ * m2particles::kFloatsPerParticle * sizeof(float);
        ...
        if (!m2ParticleVBMapped_) particleVBVertices_ = 0;
...
void M2Renderer::shutdown() {
    // A failed frame can leave outstanding bone jobs reading this renderer.
...
bool M2Renderer::loadModel(const pipeline::M2Model& model, uint32_t modelId) {
    ...
    if (hasGeometry) {
        ...
        // Format: position (3), normal (3), texcoord0 (2), texcoord1 (2), pad (2),
        // boneWeights (4), boneIndices (4 as float). The two pad floats put the
        ...
        for (const auto& v : model.vertices) {
            ...
            vertexData.push_back(0.0f);   // pad: vec4s below start 16-byte aligned
    ...
    if (!model.batches.empty()) {
        for (const auto& batch : model.batches) {
            ...
            // WotLK section levels are first-index high words. They belong to
            // the same selected skin in both color and shadow passes.
            bgpu.submeshLevel = m2SkinSectionLod(model.version, batch.submeshLevel);
