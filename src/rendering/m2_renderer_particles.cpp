...
#include "rendering/glue_emitter_motion.hpp"
...
void M2Renderer::emitParticles(M2Instance& inst, const M2ModelGPU& gpu, float dt) {
    // Instance portals used to be excluded here, and in updateRibbons, and in
    // both render paths, leaving appendInstancePortalGlow's two glow sprites as
    // the whole of a dungeon entrance: the "small static light point" a player
    // sees where the swirl should be. The exclusion was only ever standing in
    // for a particle system that was switched off on the console; a portal is
    // an ordinary particle-and-ribbon effect and is fed like one. The glow card
    // stays - it is what the portal reads as from across a zone, past the
    // distance the particles themselves are culled at.
    ...
    for (size_t ei = 0; ei < gpu.particleEmitters.size(); ei++) {
        ...
        float life = interpFloat(em.lifespan, inst.animTime, inst.globalSequenceTime,
                                 inst.currentSequenceIndex, gpu.globalSequenceDurations);
        {
            if (!std::isfinite(rate) || !std::isfinite(life)) continue;
            rate = std::clamp(rate, 0.0f, 20000.0f);
            life = std::clamp(life, 0.0f, 300.0f);
        }
        ...
        inst.emitterAccumulators[ei] = std::min(inst.emitterAccumulators[ei]+rate*dt,128.f);
        size_t emitterPopulation=0;
        const size_t emitterLimit=gpu.isInstancePortal
            ? std::max<size_t>(1,particleInstanceShare_/std::max<size_t>(1,gpu.particleEmitters.size()))
            : particleInstanceShare_;
        if(gpu.isInstancePortal) for(const auto& particle:inst.particles)
            if(particle.emitterIndex==int(ei))++emitterPopulation;

        // particleInstanceShare_ is this instance's slice of the frame budget,
        // set once per update() from the number of emitters that survived the
        // cull. MAX_M2_PARTICLES was a per-instance cap being read as a frame
        // one: a few hundred emitters in range each held up to 4000 particles,
        // which is what an unbounded per-frame cost looks like on a console
        // that has died of std::bad_alloc.
        while (inst.emitterAccumulators[ei] >= 1.0f &&
               inst.particles.size() < particleInstanceShare_ && emitterPopulation < emitterLimit) {
            ++emitterPopulation;
            ...
            if (gpu.isGlueScene && em.emitterType == 1 &&
                !glue::planeParticleMotion(em, inst.currentSequenceIndex,
                    inst.animTime, inst.globalSequenceTime, gpu.globalSequenceDurations,
                    inst.modelMatrix * boneXform, particleRng_, p.position, p.velocity)) {
                continue;
            }
            ...
            if (!gpu.isGlueScene && std::abs(speed) < 0.01f) {
...
void M2Renderer::updateParticles(M2Instance& inst, float dt) {
    ...
    // The share can fall between frames - the scene got busier, or memory
    // pressure cut the budget - and emission alone only stops adding. Drop the
    // excess from the back so the memory goes back now rather than over the
    // next lifespan, which is the whole point of a budget that degrades.
    if (inst.particles.size() > particleInstanceShare_) {
        inst.particles.resize(particleInstanceShare_);
    }
    ...
    if (numEm > 0) {
        ...
        for (size_t e = 0; e < numEm; ++e) {
            ...
            float grav = interpFloat(pem.gravity,
                                      ...
                                      inst.currentSequenceIndex, gpu.globalSequenceDurations);
            if (grav == 0.0f && !gpu.isFireflyEffect && !gpu.isGlueScene && !gpu.isInstancePortal && !gpu.isSpellEffect) {
                float emSpeed = interpFloat(pem.emissionSpeed,
                                             inst.animTime, inst.globalSequenceTime,
                                             inst.currentSequenceIndex, gpu.globalSequenceDurations);
                grav = (std::abs(emSpeed) > 0.1f) ? 4.0f : 1.5f;
            }
            emitterGrav[e] = std::isfinite(grav) ? grav : 0.0f;
        }
    }
    ...
    for (size_t i = 0; i < inst.particles.size(); ) {
        ...
        p.life += dt;
        if (!std::isfinite(p.life) || !std::isfinite(p.maxLife) ||
            !std::isfinite(p.position.x) || !std::isfinite(p.position.y) ||
            !std::isfinite(p.position.z) || p.life >= p.maxLife) {
...
void M2Renderer::updateRibbons(M2Instance& inst, const M2ModelGPU& gpu, float dt) {
    // Portals are simulated like any other ribbon emitter now; see the note in
    // emitParticles for why they were excluded and why that has stopped.
    ...
    for (size_t ri = 0; ri < emitters.size(); ri++) {
        ...
        auto getFloatVal = [&](const pipeline::M2AnimationTrack& track,float fallback) {
            return m2_track::sampleFloat(track,inst.currentSequenceIndex,inst.animTime,
                inst.globalSequenceTime,gpu.globalSequenceDurations,fallback);
        };
        auto getVec3Val = [&](const pipeline::M2AnimationTrack& track,glm::vec3 fallback) {
            return m2_track::sampleVec3(track,inst.currentSequenceIndex,inst.animTime,
                inst.globalSequenceTime,gpu.globalSequenceDurations,fallback);
        };
        ...
        if (visibility > 0.5f && std::isfinite(em.edgesPerSecond) &&
            std::isfinite(heightAbove) && std::isfinite(heightBelow) && std::isfinite(alpha)) {
            // Malformed or extreme authored rates must not spin forever at menu startup.
            accum += std::clamp(em.edgesPerSecond, 0.0f, 4096.0f) * dt;
            accum = std::min(accum, 128.0f);
            while (accum >= 1.0f) {
                accum -= 1.0f;
...
// This records the M2 secondary command buffer, so it makes Vulkan calls, and
// which thread it is on depends on the platform: desktop records it on a frame
// worker (Renderer::render, the parallelRecordingEnabled_ branch) and PS4 on
// the main thread, because ps4_vulkan implements no render-pass inheritance
// for secondaries and Renderer::initialize therefore never enables parallel
// recording there. Everything above - emit and simulate - runs on the frame
// worker that owns M2Renderer::update on both, and the future joined in
// Renderer::render is what orders the two.
//
// The console had particles forced off because "descriptors are allocated on
// worker threads". That was true of exactly one branch: when a texture group
// had no pre-allocated set, this function called vkAllocateDescriptorSets
// while recording, which on desktop is a worker thread, against an ICD whose
// descriptor pool had no lock. The console never reached it - see above, it
// records on the main thread - so nothing about particles was ever unsafe
// here; the switch was a precaution written against the desktop shape.
//
// It is gone either way now: a group with no pre-allocated set is skipped. The
// sets themselves are made once per emitter in loadModel, on the main thread,
// beside the ribbon sets that have been shipping enabled all along - and a
// per-frame allocation out of a fixed pool is what exhausts it, which is the
// failure the fallback was written to work around rather than cause.
void M2Renderer::renderM2Particles(VkCommandBuffer cmd, VkDescriptorSet perFrameSet) {
    if (!particlePipeline_ || !m2ParticleVB_ || !m2ParticleVBMapped_) return;
    ...
    // What this frame may write, and where. Each frame in flight owns its own
    // half of the buffer; see the buffer's creation in m2_renderer.cpp.
    const size_t frameCapacity = m2particles::frameVertexCapacity(
        particleVBVertices_, MAX_FRAMES_IN_FLIGHT);
    if (frameCapacity == 0) return;
    const size_t frameBase = m2particles::frameVertexBase(
        vkCtx_->getCurrentFrame(), MAX_FRAMES_IN_FLIGHT, particleVBVertices_);
    ...
    //
    // The map is keyed partly on a VkTexture*, so it accumulates a bucket for
    // every texture that has ever emitted, including the ones whose models
    // were unloaded a zone ago; the reuse that makes it cheap is also what
    // stops it ever shrinking. Nothing dereferences a stale key - the pointer
    // is only compared, never followed - but the buckets and the vertexData
    // capacity each one holds are real memory on a heap that has run out
    // before. The sweep at the end of this function drops the ones that went
    // unused; see there for why it is periodic rather than every frame.
    ...
    // Only instances that have emitters, not all of them: a city log shows
    // m2Instances=16382, and the empty-particles test on every one of those
    // was the whole of this loop's cost in the common case.
    for (size_t instIdx : particleInstanceIndices_) {
        if (instIdx >= instances.size()) continue;
        auto& inst = instances[instIdx];
        if (inst.particles.empty()) continue;
        if (!inst.cachedModel) continue;
        const auto& gpu = *inst.cachedModel;
        // The frame's whole vertex region is the ceiling on collection too,
        // so a scene that overruns the budget stops building vertex data
        // rather than building megabytes of it for draws that cannot fit.
        if (totalParticles >= frameCapacity) break;
    ...
    // Drop the buckets nothing filled. Every four seconds at the console's
    // ~20 FPS, not every frame: a group that goes quiet for a moment is worth
    // keeping (rebuilding it costs a hash insert and a vector regrowth), a
    // group whose model was unloaded a zone ago is not, and the difference is
    // only visible over time. Done here rather than before collection because
    // this is where "unused" means anything - the clear at the top empties
    // every group unconditionally.
    if (++particleGroupSweepFrame_ >= kParticleGroupSweepInterval) {
        particleGroupSweepFrame_ = 0;
        for (auto it = particleGroups_.begin(); it != particleGroups_.end(); ) {
            it = it->second.vertexData.empty() ? particleGroups_.erase(it)
                                               : std::next(it);
        }
    }
    ...
    float* const vb = static_cast<float*>(m2ParticleVBMapped_);
    // Vertices written into this frame's region so far. Every group draws from
    // its own run: the draws all execute after recording ends, so a group that
    // rewound to the start of the region would be drawn with whatever the last
    // group left there rather than its own particles.
    size_t written = 0;
    particleVBOverflowGroups_ = 0;

    for (auto& [key, group] : groups) {
        if (group.vertexData.empty()) continue;

        // A group with no pre-allocated set is skipped rather than allocating
        // one here. See the note above this function: this is the branch that
        // put descriptor allocation on a recording thread, and allocating from
        // a fixed pool every frame is what exhausts the pool it draws from.
        const VkDescriptorSet texSet = group.preAllocSet;
        if (texSet == VK_NULL_HANDLE) {
            static bool reportedMissingSet = false;
            if (!reportedMissingSet) {
                reportedMissingSet = true;
                LOG_WARNING("M2 particles: a texture group has no pre-allocated "
                            "descriptor set (materialDescPool_ exhausted at load "
                            "time); its particles are not drawn");
            }
            continue;
        }

        const size_t want = group.vertexData.size() / m2particles::kFloatsPerParticle;
        const size_t count = m2particles::admitVertices(written, want, frameCapacity);
        if (count == 0) {
            ++particleVBOverflowGroups_;
            continue;
        }

        uint8_t blendType = group.blendType;
        // The additive pipeline is built separately and can be absent on its
        // own if that build failed, which the entry test above does not catch.
        // An additive group then draws blended rather than binding null.
        VkPipeline desiredPipeline = ((blendType == 3 || blendType == 4) &&
                                      particleAdditivePipeline_)
        ...
        const size_t firstVertex = frameBase + written;
        memcpy(vb + firstVertex * m2particles::kFloatsPerParticle,
               group.vertexData.data(),
               count * m2particles::kFloatsPerParticle * sizeof(float));
        vkCmdDraw(cmd, static_cast<uint32_t>(count), 1,
                  static_cast<uint32_t>(firstVertex), 0);
        written += count;
    }

    if (particleVBOverflowGroups_ > 0) {
        static uint32_t overflowDiagFrame = 0;
        if (++overflowDiagFrame % 300 == 1) {
            LOG_INFO("M2 particles: frame budget full - ", written, "/", frameCapacity,
                     " vertices drawn, ", particleVBOverflowGroups_,
                     " group(s) dropped, share=", particleInstanceShare_,
                     " emitters=", particleEmittersLastFrame_);
        }
    }
}
