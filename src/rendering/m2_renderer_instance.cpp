...
void M2Renderer::setInstanceTransform(uint32_t instanceId, const glm::mat4& transform) {
    ...
    if (inst.participatesInPositionDedup && !inst.cachedIsGroundDetail) {
...
void M2Renderer::removeInstance(uint32_t instanceId) {
    ...
    if (inst.participatesInPositionDedup && !inst.cachedIsGroundDetail) {
        ...
        if (dit != instanceDedupMap_.end() && dit->second == instanceId)
            instanceDedupMap_.erase(dit);
    }
...
void M2Renderer::clear() {
    // Normally update() consumes every future. If a frame exits via an
    // allocation/animation exception, remaining jobs still read models and
    // instances. Join them before resetting either CPU or GPU ownership.
    for (auto& future : animFutures_) if (future.valid()) future.wait();
    if (vkCtx_) {
        if (!vkCtx_->waitIdleAndDrainCleanup("M2 reset")) return;
    ...
    // They must not outlive the model cache, even if no particles draw next frame.
    particleGroups_.clear();
    // Frame-slot shadow descriptors are retained; their contents refresh on use.
    modelUnusedSince_.clear();
    reapedModelIds_.clear();
    predecodedBLPCache_ = nullptr;
#ifdef WOWEE_PS4
    // Release session CPU storage: clear() destroys elements but retains the
    // capacity. 32,768 M2 instances at the measured 488-byte stride leave
    // 15.25 MiB behind in the menu, plus visibility/particle working arrays.
    // Only full scene reset releases capacity; streaming keeps its reusable
    // arrays to avoid reallocating them on every tile boundary.
    const size_t releasedInstanceBytes = instances.capacity() * sizeof(M2Instance);
    decltype(instances){}.swap(instances);
    decltype(sortedVisible_){}.swap(sortedVisible_);
    decltype(transparentVisible_){}.swap(transparentVisible_);
    decltype(glowSprites_){}.swap(glowSprites_);
    decltype(ribbonDraws_){}.swap(ribbonDraws_);
    decltype(boneWorkIndices_){}.swap(boneWorkIndices_);
    decltype(animFutures_){}.swap(animFutures_);
    decltype(smokeParticles){}.swap(smokeParticles);
    decltype(smokeInstanceIndices_){}.swap(smokeInstanceIndices_);
    decltype(portalInstanceIndices_){}.swap(portalInstanceIndices_);
    ...
    decltype(instanceDedupMap_){}.swap(instanceDedupMap_);
    decltype(spatialGrid){}.swap(spatialGrid);
    decltype(particleGroups_){}.swap(particleGroups_);
    LOG_INFO("[SESSION_CPU_RELEASE] M2 instance capacity bytes=", releasedInstanceBytes,
             "; visibility, particle and index storage released");
    // End session CPU storage release.
#endif
    ...
    // VkTexture deliberately requires explicit destruction; dropping its
    // shared_ptr alone leaked images/allocations on every character switch.
    if (vkCtx_) {
...
void M2Renderer::rebuildSpatialIndex() {
    ...
    // per rebuild, and the index is marked dirty on every instance added or
    // removed, so it happens continuously while a city streams.
    //
    // That churn is what leaves an arena with forty megabytes free and no four
    // contiguous megabytes in it, which is precisely the allocation a character
    // atlas needs and precisely the one that was throwing. Emptying the vectors
    // in place keeps their capacity and asks the allocator for nothing.
    for (auto& cell : spatialGrid) cell.second.clear();
    instanceIndexById.clear();
    boneSeedInstanceByModel_.clear();
    instanceDedupMap_.clear();
    ...
    // Cells that ended up empty are dropped, so a zone change does not leave
    // the grid carrying the previous zone's geometry as capacity forever.
    for (auto it = spatialGrid.begin(); it != spatialGrid.end(); ) {
        it = it->second.empty() ? spatialGrid.erase(it) : std::next(it);
    }
...
void M2Renderer::cleanupUnusedModels(const std::unordered_set<uint32_t>& preparing) {
...
#ifdef WOWEE_PS4
    // Tile residency already supplies spatial hysteresis. Do not retain every
    // abandoned tile's CPU/GPU model payload for another minute on the console.
    constexpr auto kGracePeriod = std::chrono::seconds(5);
#else
    ...
    for (const auto& [id, model] : models) {
        if (usedModelIds.find(id) != usedModelIds.end() ||
            pinnedModelIds_.find(id) != pinnedModelIds_.end() || preparing.count(id)) {
    ...
    // Retire behind all frame fences, without stopping rendering for every
    // streamed-out model. Descriptor pools stay alive until the drain at reset.
    for (uint32_t id : toRemove) {
        auto it = models.find(id);
        if (it != models.end()) {
            auto retired = std::make_shared<M2ModelGPU>();
            vkCtx_->deferAfterAllFrameFences([this, retired]() { destroyModelGPU(*retired); });
    ...
    // Textures are non-owning pointers in every model. Keep all live models,
    // including pinned/temporarily instanceless ones, and retire only true
    // orphans. The cache otherwise grows for the entire journey.
    std::unordered_set<VkTexture*> referenced;
    for (const auto& [id, model] : models) {
        for (const auto& batch : model.batches) referenced.insert(batch.texture);
        for (auto* texture : model.particleTextures) referenced.insert(texture);
        for (auto* texture : model.ribbonTextures) referenced.insert(texture);
    }
    size_t released = 0;
    for (auto it = textureCache.begin(); it != textureCache.end();) {
        if (referenced.count(it->second.texture.get())) { ++it; continue; }
        auto retired = std::make_shared<TextureCacheEntry>();
        vkCtx_->deferAfterAllFrameFences([retired]() { retired->texture.reset(); });
        texturePropsByPtr_.erase(it->second.texture.get());
        // Particle draw groups hold raw texture keys between frames.
        for (auto group = particleGroups_.begin(); group != particleGroups_.end();) {
            if (group->first.texture == it->second.texture.get()) group = particleGroups_.erase(group);
            else ++group;
        }
    ...
    if (released) LOG_INFO("[STREAM_RECLAIM] M2 orphan textures bytes=", released,
...
VkTexture* M2Renderer::loadTexture(const std::string& path, uint32_t texFlags) {
    ...
    if (!tex->uploadBLP(*vkCtx_, blp)) {
        LOG_WARNING("M2: texture upload failed: ", path);
        return whiteTexture_.get();
    }
...
bool M2Renderer::checkCollision(const glm::vec3& from, const glm::vec3& to,
                                 glm::vec3& adjustedPos, float playerRadius, float playerHeight) const {
    QueryTimer timer(&queryTimeMs, &queryCallCount);
    adjustedPos = to;
    bool collided = false;
    playerHeight = core::collisionHeightOrDefault(playerHeight);

    glm::vec3 queryMin = glm::min(from, to) - glm::vec3(7.0f, 7.0f, 5.0f);
    glm::vec3 queryMax = glm::max(from, to) + glm::vec3(7.0f, 7.0f, std::max(5.0f, playerHeight));
    gatherCandidates(queryMin, queryMax, tl_m2_candidateScratch);
    ...
    for (size_t idx : tl_m2_candidateScratch) {
        ...
        if (from.z + playerHeight < instance.worldBoundsMin.z && adjustedPos.z + playerHeight < instance.worldBoundsMin.z) continue;
        ...
        if (model.collision.valid()) {
            ...
            const float PLAYER_HEIGHT = playerHeight / instance.scale;
