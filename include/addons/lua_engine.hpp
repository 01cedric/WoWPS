...
/// Visit every widget that is running or drawn, and nothing beneath one that is
/// neither.
///
/// Two properties of the tree are what make this complete rather than merely
/// cheap, and both are load-bearing. A widget's `visible` and `visibleChain`
/// are each derived from its parent's, so neither can be set on a widget whose
/// parent has it clear - the live part of the tree is therefore one connected
/// region hanging off the root. And hideSubtree clears a subtree whole and
/// stops on one already cleared, so a node with both flags down has nothing
/// live below it. Descending from the root while either flag holds reaches
/// every live widget and no others.
///
/// The children are taken before the visitor runs. A handler is free to create
/// frames and OnShow very often does, and a newly created widget has neither
/// flag set until the next layout - so nothing that could matter is missed by
/// not descending into what a handler just built. Reparenting mid-walk can show
/// the same widget twice; every visitor here is written to be idempotent for
/// that reason.
///
/// Here rather than on the tree because the passes that need it are here, and a
/// template rather than a callback so the benchmark that times it exercises the
/// same walk the engine runs rather than a second copy of it.
template <typename Fn>
void forEachLiveWidget(ui::WidgetTree& tree, std::vector<uint32_t>& stack, Fn&& fn) {
    stack.clear();
    const uint32_t root = tree.rootId();
    if (root == 0) return;
    stack.push_back(root);
    while (!stack.empty()) {
        const uint32_t id = stack.back();
        stack.pop_back();
        ui::Widget* w = tree.get(id);
        if (!w || w->id == 0) continue;
        if (!w->visibleChain && !w->visible) continue;
        // Before the visitor, so a handler that adds a child cannot reallocate
        // the vector this is reading. Reversed onto the stack so that popping
        // hands the children back in the order they were created: the sweep
        // this replaces went by widget id, and a frame's id is its creation
        // order, so keeping siblings in that order leaves the handlers running
        // in very nearly the sequence they always did. What is guaranteed
        // rather than merely usual is that a parent is visited before its
        // children, which is the half anything depends on.
        stack.insert(stack.end(), w->children.rbegin(), w->children.rend());
        fn(*w);
    }
}
...
public:
    ...
    bool initialize();
    // Low-memory recovery must not allocate a missing-API report before it
    // can release the Lua state and widget tree.
    void shutdown(bool reportErrors = true);
    ...
    /// Remember that Show or Hide toggled this frame, so the visibility pass
    /// can find it without sweeping the tree for the flag.
    ///
    /// A frame the interface hid and showed again before anything looked is
    /// owed both handlers, and the counter that records that pair sits on a
    /// widget the pass would otherwise have to walk 24,000 nodes to reach. The
    /// bindings that raise the counter say so here instead.
    void noteShownToggle(uint32_t wid);
    /// Remember that this frame has taken a message, so the ageing pass knows
    /// where the message frames are. There are a dozen of them in the whole
    /// interface and the pass used to look at every widget to find them.
    void noteMessageFrame(uint32_t wid);

    // What the per-frame interface passes cost, as last-frame samples in
    // milliseconds - the same shape as the renderer's lastXxxMs figures so
    // they can sit on one log line.
    //
    // These exist because the three of them were sweeping all 24,191 widgets
    // once each per frame and nothing measured any of it: updateMeanMs=17-25
    // was the only evidence, and it covers everything the update half does.
    [[nodiscard]] double lastVisibilityMs() const { return lastVisibilityMs_; }
    [[nodiscard]] double lastSizeChangeMs() const { return lastSizeChangeMs_; }
    [[nodiscard]] double lastMessageAgeMs() const { return lastMessageAgeMs_; }
    [[nodiscard]] double lastOnUpdateMs() const { return lastOnUpdateMs_; }
    /// The scroll pass, which is small in frames and large in what it walks:
    /// each scroll frame's whole content subtree, with a stack allocated for
    /// it. The count beside it is how many it had to walk.
    [[nodiscard]] double lastScrollRangeMs() const { return lastScrollRangeMs_; }
    [[nodiscard]] size_t lastScrollRangeWalks() const { return lastScrollRangeWalks_; }
    /// Widgets the visibility pass actually looked at. Against widgets().size()
    /// this is the whole point: a tree of 24,191 with the panels closed should
    /// report a few hundred here.
    [[nodiscard]] size_t lastVisibilityVisits() const { return lastVisibilityVisits_; }
    /// The same for the size pass, and for how many frames ran an OnUpdate.
    [[nodiscard]] size_t lastSizeChangeVisits() const { return lastSizeChangeVisits_; }
    [[nodiscard]] size_t lastOnUpdateCalls() const { return lastOnUpdateCalls_; }
...
private:
    ...
    // ── What the once-a-frame interface passes have to look at ──────────────
    //
    // All three used to be a loop from 1 to widgets_.size(), which on the
    // original interface is 24,191 iterations each, every frame, to find the
    // handful of widgets anything had happened to. What replaces the sweep is
    // the live walk above plus these three lists, and between them they cover
    // every widget the old loop could have done anything for. See
    // updateVisibility for why that is exhaustive rather than merely likely.

    /// Widgets whose last reported state was "shown". The only widgets that
    /// can still owe an OnHide once they have dropped out of the live part of
    /// the tree, and the only writer of reportedVisible is the pass that keeps
    /// this list - so it cannot drift.
    std::vector<uint32_t> visReported_;
    /// Widgets Show or Hide touched since the last pass. Hidden ones fire
    /// nothing, but their toggle counter still has to be cleared or it would
    /// be read as a fresh hide-and-show whenever the panel around them opens.
    std::vector<uint32_t> visToggled_;
    /// Every frame that has ever been handed a message. A dozen frames in the
    /// interface, against a whole-tree sweep to find them.
    std::vector<uint32_t> messageFrames_;
    /// Scratch for forEachLiveWidget, kept so the walk does not allocate. One
    /// per pass rather than one shared: the visitor runs Lua, and a walk that
    /// found its own stack emptied under it would stop early and silently.
    std::vector<uint32_t> visWalkStack_;
    std::vector<uint32_t> sizeWalkStack_;
    /// Widgets whose size has been recorded at least once. Below this every id
    /// is new, and a new widget is measured without being announced - the same
    /// rule the old sweep applied on the first frame it saw one.
    uint32_t sizeSeedHi_ = 1;

    double lastVisibilityMs_ = 0.0;
    double lastSizeChangeMs_ = 0.0;
    double lastMessageAgeMs_ = 0.0;
    double lastOnUpdateMs_ = 0.0;
    double lastScrollRangeMs_ = 0.0;
    size_t lastScrollRangeWalks_ = 0;
    size_t lastVisibilityVisits_ = 0;
    size_t lastSizeChangeVisits_ = 0;
    size_t lastOnUpdateCalls_ = 0;
    ...
    /// Fire whatever one widget owes for having appeared or gone. The body of
    /// updateVisibility, so that the three sets it covers can share it.
    void reconcileVisibility(uint32_t wid);
