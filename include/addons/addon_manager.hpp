...
public:
    ...
    InterfaceSource& interfaceSource() { return interfaceSource_; }
    void scanAddons(const std::string& addonsPath);
    void loadAllAddons();
    /// Drop a failed UI without saving partially initialized SavedVariables.
    bool discardPartialInterface();
    ...
    bool originalInterfaceLoaded() const { return originalInterfaceLoaded_; }
    bool beginOriginalInterfaceAttempt();
    bool completeOriginalInterfaceAttempt();
    void failOriginalInterface(const char* reason);
    const char* missingOriginalInterfaceRoot() const;
...
private:
    ...
    void finishOriginalInterfaceSession();
    InterfaceSource interfaceSource_;
    bool executeSourceFile(const std::string& path);
    ...
    bool originalInterfaceLoaded_ = false;
