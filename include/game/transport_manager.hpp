...
public:
    ...
    void updateLocalTransportPose(uint64_t guid, const glm::vec3& position, float yaw, bool docked);
    ...
    // Retire one map-local presentation; the realm's route clock is untouched.
    void removeTransport(uint64_t guid);
