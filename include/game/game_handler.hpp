...
#include "game/local_presentation.hpp"
...
public:
    ...
    // Local exploration deliberately keeps isInWorld()/isConnected() false:
    // those predicates also authorize socket-based server commands.
    void beginLocalExploration(const Character& character, float serverOrientation, bool transportTransfer = false);
    /// The character screen's list when it shows this console's saved local
    /// characters: no realm answered, nothing else about the handler changes.
    void setLocalCharacterList(std::vector<Character> list);
    ...
    uint64_t previousLocalAttackTarget(uint64_t guid) const {
        ...
        return it == localPresentationStates_.end() ? 0 : it->second.target;
    }
    ...
    bool isCasting() const { return localExploration_ ? localCastPresentation_.activeSpell != 0 : spellHandler_ ? spellHandler_->isCasting() : false; }
    ...
    uint32_t getCurrentCastSpellId() const { return localExploration_ ? (localCastCallbackSpellId_ ? localCastCallbackSpellId_ : localCastPresentation_.activeSpell) : spellHandler_ ? spellHandler_->getCurrentCastSpellId() : 0; }
    float getCastProgress() const { return localExploration_ ? (localCastPresentation_.totalMs ? 1.0f - float(localCastPresentation_.remainingMs) / float(localCastPresentation_.totalMs) : 0.0f) : spellHandler_ ? spellHandler_->getCastProgress() : 0.0f; }
    float getCastTimeRemaining() const { return localExploration_ ? localCastPresentation_.remainingMs / 1000.0f : spellHandler_ ? spellHandler_->getCastTimeRemaining() : 0.0f; }
    float getCastTimeTotal() const { return localExploration_ ? localCastPresentation_.totalMs / 1000.0f : spellHandler_ ? spellHandler_->getCastTimeTotal() : 0.0f; }
    ...
    void setLocalWorldTimeHours(float hours) { gameTime_ = hours; }
    ...
    bool hasRegisteredLocalTransport(uint64_t guid) const;
    void resumeLocalTransport(uint32_t entry, uint32_t map, const glm::vec3& headingOffset);
    ...
    void updateM2TransportBoarding(const glm::vec3& playerCanonical, float deltaTime = 1.0f / 60.0f);
    void syncLocalTransportPassengers() { if (localExploration_) updateAttachedTransportChildren(0.0f); }
    void setPlayerOnTransport(uint64_t transportGuid, const glm::vec3& localOffset) {
        ...
        if (transportGuid != 0 && !isTransportGuid(transportGuid) &&
            !hasRegisteredLocalTransport(transportGuid)) {
            return;  // Transport not yet registered; skip attachment
        }
        if (playerTransportGuid_ != transportGuid) { shipDeckContact_.reset(); shipDeckLastSample_ = {}; }
    ...
    const std::string& lookupName(uint64_t guid) const {
        if (localExploration_) {
            const auto it=localAuctionNames_.find(guid);
            if (it!=localAuctionNames_.end()) return it->second;
        }
    ...
    bool submitLocalAuction(uint32_t itemId, uint16_t count, uint32_t bid, uint32_t buyout,
                            uint32_t minutes, uint16_t stacks = 1);
    bool submitLocalAuctionStacks(uint64_t itemGuid, uint16_t count, uint16_t stacks,
...
private:
    ...
    TransportDeckContact shipDeckContact_;
    std::chrono::steady_clock::time_point shipDeckLastSample_{};
