...
// Where a markLayoutDirty() call came from, filled in by the compiler at the
// call site rather than by the caller.
//
// The gate in WidgetRenderer::layout only pays for itself if nothing raises the
// generation on an idle frame, and the first console reading of it said
// fxLayoutSkipped=0 on every sample - something was dirtying the tree sixty
// times a second and there was no way to ask what. Thirty call sites raise it,
// nine of them in files this half does not own, so a hand-written tag on each
// would have been thirty edits and the thirty-first would be the one that
// mattered. A default argument evaluated at the call site names every one of
// them, including the ones in lua_engine.cpp, without touching a line there.
//
// Guarded rather than assumed: the console toolchain's compiler is older than
// this one, and a build that cannot name the site must still build.
#if defined(__has_builtin)
#  if __has_builtin(__builtin_FILE) && __has_builtin(__builtin_LINE)
#    define WOWEE_DIRTY_SITE_FILE __builtin_FILE()
#    define WOWEE_DIRTY_SITE_LINE __builtin_LINE()
#  endif
#endif
#ifndef WOWEE_DIRTY_SITE_FILE
#  define WOWEE_DIRTY_SITE_FILE nullptr
#  define WOWEE_DIRTY_SITE_LINE 0
#endif
...
struct Widget {
    ...
    TextureContentBounds textureContent; // decoded small button art, for pad focus
...
public:
    ...
    /// Whether a complete layout pass is required. Most frames only change
    /// draw state (colours, cooldowns, textures); walking several thousand
    /// widgets for those frames is pure CPU cost.
    [[nodiscard]] bool needsLayout(float pixelW, float pixelH) const {
        return lastPixelW_ != pixelW || lastPixelH_ != pixelH ||
               hitGeneration_ != layoutGeneration_ ||
               hitWidgetCount_ != widgets_.size();
    }

    /// Every widget a sizing pass could have work for, by id, in id order.
    ///
    /// The renderer settles three things before the solve runs - a tooltip's
    /// size from its lines, a label's from its text, a texture's from its
    /// file - and each of those used to be its own sweep of the whole tree.
    /// Three walks of 26250 widgets measured 10.8 ms of a 44.8 ms frame on
    /// PS4, against 0.5 ms to draw what came out of them: the cost was
    /// reaching every widget to ask, not the answering.
    ///
    /// What can possibly answer yes is a small, stable set - regions and
    /// tooltip frames - and the great majority of a FrameXML tree is neither.
    /// This is that set, kept between frames and rebuilt only when the tree
    /// grows, so the passes walk what could need sizing rather than
    /// everything.
    ///
    /// Deliberately not filtered by visibility, and that is not an oversight.
    /// FrameXML builds a panel while it is hidden, measures what it built and
    /// only then shows it - UIDropDownMenu_Initialize does exactly that, and a
    /// dropdown whose entries were never measured opens at nothing by nothing.
    /// Hidden is what the *solve* may skip; it is not what a measurement may
    /// skip.
    [[nodiscard]] const std::vector<uint32_t>& sizingCandidates() const;

    // What the last layout cost, in milliseconds, and how much of the tree it
    // had to touch.
    //
    // A PS4 log reported frameMeanMs=84 with renderMeanMs=67, and the timed
    // world passes - shadow, reflection, camera, terrain, WMO, M2, godray,
    // post - added up to 4.7 of it. The other 62 were this, and nothing
    // measured it, so the only evidence that the interface was the cost was
    // that turning it off tripled the frame rate. These exist so the next log
    // says which half of the layout the time went into rather than leaving it
    // to be inferred.
    //
    // Last-frame samples rather than window means, matching the renderer's own
    // lastXxxMs figures so they can sit on the same line.
    [[nodiscard]] double lastLayoutMs() const { return lastLayoutMs_; }
    /// The anchor solve alone: the recursive walk that gives every widget its
    /// rect, visibility, strata, level and scale.
    [[nodiscard]] double lastSolveMs() const { return lastSolveMs_; }
    /// Building the draw order and sorting it. Separate from the solve because
    /// the two scale with different things - the solve with how much of the
    /// tree is on screen, the sort with how much of it is actually drawn - and
    /// a single total cannot say which grew.
    [[nodiscard]] double lastDrawOrderMs() const { return lastDrawOrderMs_; }
    /// Widgets the solve actually resolved. Against size() this is the whole
    /// point of the hidden-subtree prune: a tree of 24191 widgets with the
    /// panels closed should report a few thousand here, not 24191.
    [[nodiscard]] size_t lastSolvedWidgets() const { return lastSolvedWidgets_; }
    /// Widgets this pass had to mark unseen because something above them
    /// stopped being shown. Falls to zero on the frame after a panel closes -
    /// the subtree is already clear by then and the prune stops at its root,
    /// which is the saving rather than a gap in the counting.
    [[nodiscard]] size_t lastPrunedWidgets() const { return lastPrunedWidgets_; }
    ...
    ///
    /// `why` is a short literal for the sites in this file and the renderer's,
    /// which know what they are; everywhere else the two defaults name the
    /// file and line by themselves. Nothing is copied and nothing is formatted
    /// - three pointer stores on a call that already exists.
    void markLayoutDirty(const char* why = nullptr,
                         const char* file = WOWEE_DIRTY_SITE_FILE,
                         int line = WOWEE_DIRTY_SITE_LINE) {
        ++layoutGeneration_;
        // The first raise since the last completed pass is the one that broke
        // the gate; the ones after it were owed a solve either way. Both are
        // kept because they answer different questions - which site to fix,
        // and whether it is one site or a storm of them.
        if (dirtyMarks_ == 0) {
            firstDirtyWhy_ = why;
            firstDirtyFile_ = file;
            firstDirtyLine_ = line;
        }
        ++dirtyMarks_;
        lastDirtyWhy_ = why;
        lastDirtyFile_ = file;
    ...
    /// How many times the generation was raised since the last layout() -
    /// zero on a frame nothing has asked for a pass.
    [[nodiscard]] size_t dirtyMarksSinceLayout() const { return dirtyMarks_; }
    /// The call site that broke the layout gate, and the one that raised it
    /// last, as "file:line why". Empty when nothing has. Built on demand
    /// because the only caller is a log line.
    [[nodiscard]] std::string dirtySiteReport() const;
    ...
    [[nodiscard]] float safeAreaInset() const { return safeAreaInset_; }
...
private:
    ...
    std::map<std::string, uint32_t, std::less<>> nameIndex_;
    ...
    /// Mark a subtree unseen, without solving any of it.
    ///
    /// A widget whose chain is not shown cannot be drawn and cannot run an
    /// OnUpdate, so the only thing anything asks of it is that it says so.
    /// Solving its geometry as well is what the closed half of FrameXML was
    /// costing every frame: the interface builds the character sheet, the
    /// spellbook, the talent frame, the quest log, the world map, the auction
    /// house, the mail and the merchant at load and the player has one of them
    /// open, so the great majority of 24191 widgets were being placed for
    /// nobody.
    ///
    /// The stop is what makes it cheap over a run of frames: a subtree already
    /// marked was marked whole, because this only ever descends after clearing
    /// the node above - so the second frame of a closed panel costs one test
    /// per top-level child rather than a walk. A widget created under a hidden
    /// frame arrives already clear, since both flags default to false, and
    /// setParent clears a subtree it moves under something unseen.
    ///
    /// Deliberately does not touch resolvedGen. Leaving it stale is what keeps
    /// resolveWidget honest: FrameXML measures frames while they are hidden -
    /// it builds a panel, sizes it, and only then shows it - and every one of
    /// those getters resolves on demand. Claiming the rect was resolved here
    /// would hand back whatever the last visible layout left, which is the
    /// quiet kind of wrong.
    void hideSubtree(uint32_t id);
    /// Add a widget to the draw order and the hit-candidate list if it belongs
    /// in either. Called from the solve, so both lists cost what is on screen
    /// rather than what is in the tree.
    void considerForDraw(const Widget& w);
    ...
    /// Who raised it since the last completed pass, and how often. See
    /// markLayoutDirty. Every one of these is a pointer to a string literal or
    /// to __FILE__, so nothing here owns anything.
    size_t dirtyMarks_ = 0;
    const char* firstDirtyWhy_ = nullptr;
    const char* firstDirtyFile_ = nullptr;
    int firstDirtyLine_ = 0;
    const char* lastDirtyWhy_ = nullptr;
    const char* lastDirtyFile_ = nullptr;
    int lastDirtyLine_ = 0;

    /// See sizingCandidates(). Mutable because it is a cache of what the tree
    /// already says, answered from a const accessor.
    ///
    /// Rebuilt when the widget count changes, which is the only way a widget
    /// can join the set. kind is fixed at create(). objectType is written by
    /// the CreateFrame binding in the same statement run as the create that
    /// invalidated this, so the first rebuild after a GameTooltip is made
    /// always sees it as one - nothing lays out in between. isTooltip is the
    /// one flag a tooltip setter raises later on an existing frame, and it is
    /// in the predicate for symmetry rather than for safety: the tooltip pass
    /// throws out anything whose objectType is not GameTooltip regardless, so
    /// a frame that raises the flag and is not one has never had work here.
    mutable std::vector<uint32_t> sizingCandidates_;
    mutable size_t sizingIndexCount_ = static_cast<size_t>(-1);
    ...
    /// Sort the draw order the solve gathered. The gathering itself happens in
    /// considerForDraw as each widget is resolved; only the ordering is left,
    /// and the comparison is a total one - creationOrder breaks every tie and
    /// is unique per widget - so the order the entries arrived in cannot change
    /// the result.
    void sortDrawOrder();
    ...
    size_t lastSolvedWidgets_ = 0;
    size_t lastPrunedWidgets_ = 0;
    ...
    /// Zero by default: a monitor loses nothing to overscan, and a margin
    /// nobody asked for is just a smaller interface. The console build sets
    /// it at startup, and the interface options expose it.
    float safeAreaInset_ = 0.0f;
    /// Where the interface's coordinate space starts, in units. Zero without a
    /// title-safe margin; the inset otherwise. Anchor resolution reads it for
    /// the "anchored to the screen with no parent" case, which is the one
    /// place the root's own rect is not already carrying the offset.
    float layoutOriginX_ = 0.0f;
    float layoutOriginY_ = 0.0f;
    std::vector<uint32_t> scrollFrames_;
    std::vector<uint32_t> hitCandidates_;
    uint64_t hitGeneration_=0;
    size_t hitWidgetCount_=0;
