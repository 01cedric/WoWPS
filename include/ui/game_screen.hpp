...
class GameScreen {
public:
    // Visual-only path for local authority equipment; no online UI or requests.
    void refreshLocalEquipment(game::Inventory& inventory) {
        updateCharacterGeosets(inventory);
        updateCharacterTextures(inventory);
        syncEquipmentAnimation(inventory);
    }
    void syncEquipmentAnimation(const game::Inventory& inventory);
