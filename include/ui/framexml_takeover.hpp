...
/// Mandatory original-interface mode. When true, the native duplicate UI is
/// never used as a recovery path.
bool frameXmlRequired();
/// Runtime ownership, independent of the startup request. Loading may claim
/// ownership for FrameXML's bootstrap; a discarded/closed VM owns nothing.
void frameXmlSetActive(bool active);
bool frameXmlActive();
...
/// Clear ownership, deferred-addon and cursor state before another Lua session.
...
/// Where the carried icon is drawn while the pad is steering the interface.
///
/// The icon rides the pointer, and a pad has no pointer: the hardware mouse
/// sits wherever it was last put, which on a console is one place forever. A
/// player picking an item up with the D-pad could therefore not see what they
/// were carrying, only that a slot had emptied. This is the centre of the
/// control the pad has focused, in screen pixels, published by the pad
/// navigation and read by the widget renderer; with no pad focus there is none
/// and the mouse position stands.
void frameXmlSetPadCursorAnchor(float x, float y);
void frameXmlClearPadCursorAnchor();
bool frameXmlPadCursorAnchor(float& x, float& y);

/// An item display id turned into "Interface\Icons\<name>", or empty.
///
/// The local realm's bags carry a display id and no artwork, and the table that
/// turns one into an icon name is ItemDisplayInfo.dbc - which lives behind the
/// asset layer, on the far side of the bridge from the code that publishes
/// those bags. Registered from the Lua bindings, which already hold the handler
/// that owns that table, and taken back down with the state that registered it.
/// Empty until then, and empty for a display id the table does not name - which
/// is the only case a question mark is the honest answer to.
