...
//
// Views rather than strings on the way in. Resolving a face is asked once per
// visible label per frame, and every one of these used to oblige the caller to
// hold a std::string before normalising it into another one. The normalisation
// now goes into a stack buffer and the map takes a view, so nothing on the path
// constructs a string. The renderer's one-entry memo in front of it stays: it
// answers in half a nanosecond against forty for the lookup, which is a great
// deal more than the string ever cost.
