...
public:
    ...
    bool settingsWasVisible_ = false;
    bool settingsPopupOpen_ = false;
    bool settingsEditing_ = false;
    ...
    // The three graphics settings this client added itself. Defaults are read
...
#else
    bool pendingWaterReflections = true;
#endif
    ...
    /// Whether the stored shadow and sun-ray levels have reached the renderer
    /// this session. Their sinks are filled after the renderer is built, so
    /// this waits for them rather than for it.
    bool renderSettingsApplied_ = false;
