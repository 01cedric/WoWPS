...
public:
    ...
    /// Prepare the active front-end scene and its uploads before acquiring
    /// the main Vulkan frame. Rendering the controls only consumes resources.
    void prepareScene(core::AppState appState, game::GameHandler* gameHandler, float deltaTime);

    /// Free front-end 3D targets/models before a world load. Call only before
    /// acquiring/recording a Vulkan frame; controls and selections are retained.
    void releaseFrontendScenes();
