...
// CPU staging format and GPU residency can differ on PS4. Cache admission
// must charge the decoded image when uploadBLP uses the RGBA fallback.
inline size_t textureUploadBudgetBytes(const BLPImage& image) {
    return image.isBlockCompressed() && !blockCompressionSupported()
        ? image.approxDecodedUploadBytes() : image.approxUploadBytes();
}
