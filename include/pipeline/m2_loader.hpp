...
struct M2AnimationTrack {
    ...
    struct SequenceKeys {
        ...
        // Only camera splines use tangents. Keeping four empty vectors in
        // every bone/animation slot added 96 bytes per slot, including all
        // external sequences not yet read. Allocate that storage only for
        // actual splines; their authored handles remain unchanged.
        struct SplineTangents {
            std::vector<glm::vec3> vec3InTangents, vec3OutTangents;
            std::vector<float> floatInTangents, floatOutTangents;
        };
        SequenceKeys() = default;
        SequenceKeys(SequenceKeys&&) noexcept = default;
        SequenceKeys& operator=(SequenceKeys&&) noexcept = default;
        SequenceKeys(const SequenceKeys& other)
            : timestamps(other.timestamps), vec3Values(other.vec3Values),
              quatValues(other.quatValues), floatValues(other.floatValues),
              spline_(other.spline_ ? std::make_unique<SplineTangents>(*other.spline_) : nullptr) {}
        SequenceKeys& operator=(const SequenceKeys& other) {
            if (this != &other) *this = SequenceKeys(other);
            return *this;
        }
        SplineTangents& spline() {
            if (!spline_) spline_ = std::make_unique<SplineTangents>();
            return *spline_;
        }
        [[nodiscard]] const SplineTangents& spline() const {
            static const SplineTangents empty;
            return spline_ ? *spline_ : empty;
        }
        [[nodiscard]] bool hasSplineStorage() const noexcept { return bool(spline_); }
    private:
        std::unique_ptr<SplineTangents> spline_;
    };
...
struct M2Batch {
    ...
    uint16_t submeshLevel = 0;      // WotLK first-index high word (already included in indexStart)
};
...
struct M2Camera {
    ...
    float fov = 0.0f;      // radians (diagonal)
    float farClip = 0.0f;
    float nearClip = 0.0f;
    glm::vec3 positionBase{0.0f};
    glm::vec3 targetBase{0.0f};  // the point the camera looks at
    // The authored camera path (WotLK models): spline keys per sequence,
    // relative to the bases above. A glue scene's fly-in lives here; the
    // login and character screens are wrong without it.
    M2AnimationTrack positions;
    M2AnimationTrack targets;
    M2AnimationTrack roll;
};
