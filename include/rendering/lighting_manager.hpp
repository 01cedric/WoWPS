...
public:
    ...
    void update(const glm::vec3& playerPos, uint32_t mapId, uint32_t zoneId,
                float gameTime = -1.0f,
                bool isRaining = false, bool isUnderwater = false, float deltaTime = 1.0f/60.0f);
    ...
    [[nodiscard]] uint32_t getActiveSkyboxFlags() const { return activeSkyboxFlags_; }
...
private:
    ...
    uint32_t activeSkyboxFlags_ = 0;
    bool firstLightingSample_ = true;
