...
public:
     ...
     */
    bool addDoodadToInstance(uint32_t instanceId, uint32_t m2InstanceId, const glm::mat4& localTransform);
    ...
    uint32_t getLastFrustumCulledGroups() const { return lastFrustumCulledGroups; }
    ...
    /// Positive `minNormalZ` keeps only hits whose world-space slope is
    /// walkable before choosing the best floor. Zero preserves the unfiltered
    /// query used by camera/ceiling checks.
    std::optional<float> getFloorHeight(float glX, float glY, float glZ,
                                        float* outNormalZ = nullptr,
                                        float referenceZ =
                                            std::numeric_limits<float>::quiet_NaN(),
                                        float minNormalZ = 0.0f) const;
     ...
     */
    bool checkWallCollision(const glm::vec3& from, const glm::vec3& to, glm::vec3& adjustedPos, bool insideWMO = false, float playerHeight = 2.0f) const;
...
private:
    ...
    bool frustumCulling = true;  // conservative vertex-derived group AABBs
    ...
    // Portal rejection remains opt-in. Geometric frustum culling is
    // independent. PS4 also enforces the profile's view distance against each
    // visual AABB; desktop retains optional distance rejection. Culled groups
    // stay loaded with collision data and contribute no graphics commands.
    ...
    uint32_t lastFrustumCulledGroups = 0;
