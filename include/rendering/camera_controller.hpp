...
public:
    ...
    void setSteering(bool steering) { steering_ = steering && !scriptedView_ && !scriptedInputReleasePending_; }
    ...
    void cancelIntroPan();
    /// Give a cinematic exclusive access to Camera without moving the player
    /// or changing their ordinary yaw, pitch or zoom. Ending the view restores
    /// the player-relative camera and waits for held controls to be released.
    /// reset()/teleportTo() explicitly end this ownership before relocating.
    /// The cinematic owner restores any FOV/roll overrides itself.
    void setScriptedView(bool active);
    [[nodiscard]] bool isScriptedView() const { return scriptedView_; }
    ...
    // a hard landing.
    [[nodiscard]] bool isFalling() const { return !scriptedView_ && !grounded && !swimming && verticalVelocity <= 0.0f; }
    ...
    void setCollisionHeight(float height) {
        if (core::validCollisionHeight(height)) collisionHeight_ = height;
    }
    [[nodiscard]] float getCollisionHeight() const { return collisionHeight_; }
    ...
    void setFacingYaw(float yaw) { if (!scriptedView_) facingYaw = yaw; }  // For taxi movement
...
private:
    ...
    bool manualForwardWasDown_ = false;
    ...
    float collisionHeight_ = core::DEFAULT_COLLISION_HEIGHT;
    ...
    bool scriptedView_ = false;
    bool scriptedInputReleasePending_ = false;
    glm::vec3 scriptedSavedCameraPos_{0.0f};
    void clearScriptedViewInputs();
    void updateScriptedInputRelease();
