...
public:
    ...
    bool loadSharedModel(std::shared_ptr<const pipeline::M2Model> model, uint32_t id);
    ...
    /// drawFilter >= 0 records only that draw (counting every draw this call
    /// would issue, in order) - the console's first-frame diagnostics submit
    /// the scene one draw at a time to learn which one the GPU refuses.
    ...
    /// to leave out, and degradations applied to every batch this renderer
    /// draws, so a scene that faults the GPU can be bisected across launches
    /// - by pipeline state (forceOpaque), by texture (whiteTexture) and by
    /// shader path (simpleMaterial) - while the rest of it stays visible.
    struct DrawOverrides {
        std::vector<uint32_t> skipDraws;   // sorted draw indices (render() numbering)
        bool forceOpaque = false;          // every batch through the opaque pipeline
        bool whiteTexture = false;         // every batch samples the 1x1 white texture
        bool simpleMaterial = false;       // unlit, no normal map, no POM, no alpha test
    };
    void setDrawOverrides(DrawOverrides overrides);
    [[nodiscard]] const DrawOverrides& drawOverrides() const { return drawOverrides_; }
    ...
    struct SceneFrame {
        ...
        glm::mat4 model{1.0f};
        int sequence = 0;
        float timeMs = 0, globalTimeMs = 0;
    };
    // A view of the current CPU pose, valid until instances are changed or
    // update() runs again. Glue cameras and emitters must share this clock.
    bool getSceneFrame(uint32_t instanceId, SceneFrame& frame) const;
    ...
    // Main-thread streaming collection. Callers protect uploaded models that
    // still await an instance; live instances and all their texture overrides
    // are retained automatically. GPU destruction follows frame fences.
...
#ifdef WOWEE_PS4
        (void)enabled;
        normalMappingEnabled_ = false;
#else
        normalMappingEnabled_ = enabled;
#endif
        
    }
    void setNormalMapStrength(float strength) { normalMapStrength_ = strength; }
    void setPOMEnabled(bool enabled) {
#ifdef WOWEE_PS4
        (void)enabled;
        pomEnabled_ = false;
#else
...
private:
    ...
    struct M2ModelGPU {
        ...
        std::shared_ptr<const pipeline::M2Model> data; // Immutable parsed asset shared by NPC displays
        BoneHierarchy boneHierarchy; // Parent-before-child evaluation, cached per model
        ...
        bool bodyTextureRequired = false;
        ...
        // Per-batch facts that depend only on the model, computed once at load.
        //
        // The draw loop runs this for every batch of every visible instance,
        // and again for the shadow pass. Everything here was being recomputed
        // there: a texture-type scan walking up to eight texture-combo entries
        // with bounds checks, three integer divisions of the same submesh id,
        // and several group comparisons - none of which can change while the
        // model is loaded. M2Instance already caches its per-instance facts
        // this way (see m2_renderer.hpp); this is the same idea one level
        // down, for the batch.
        //
        // Only model-static facts belong here. Anything an instance can
        // change - texture overrides, geoset filters, animated colour and
        // alpha - stays in the loop, because it genuinely varies per frame.
        struct BatchStatic {
            uint16_t blendMode = 0;
            uint16_t materialFlags = 0;
            uint16_t submeshGroup = 0;
            uint32_t firstTextureType = 0;  // type of the batch's first lookup slot
            bool usesHairTexture = false;   // any combo entry is a type-6 texture
            bool hairGeoset = false;        // submesh id falls in the hair groups
            bool skinGroup = false;         // body group eligible for skin composite
            bool skippedWithoutGeosetFilter = false;  // cloak / eye-glow groups
        };
        std::vector<BatchStatic> batchStatics;
    ...
    struct CharacterInstance {
        ...
        float nextSceneUVDiagnosticMs = 0.0f;
...
public:
    ...
    /// The widest composite atlas to build, or 0 for no limit.
    ///
    /// Set per frame on the console from a probe allocation, because the size
    /// of this one buffer decides the whole cost: a 1024 atlas is four
    /// megabytes and the path holds roughly five times that live at once - its
    /// own buffer, the magenta-bleed copy, the CPU mip chain and the staging
    /// buffer - which is half a crowded city's entire measured headroom, spent
    /// on one face. It only ever declines to *grow*; a composite whose art is
    /// already at this size is untouched, and a limit below the art's own size
    /// is not a reduction to make here (see compositeWithRegions).
    void setCompositeEdgeLimit(int edge) { compositeEdgeLimit_ = edge; }
    [[nodiscard]] int getCompositeEdgeLimit() const { return compositeEdgeLimit_; }
    ...
    void requireModelBodyTexture(uint32_t modelId, bool required);
    [[nodiscard]] bool isUsableTexture(const VkTexture* texture) const;
...
private:
    ...
    /// See setCompositeEdgeLimit. Zero on every platform that never sets it.
    int compositeEdgeLimit_ = 0;
...
private:
    ...
    // Normal mapping / POM settings
#ifdef WOWEE_PS4
    bool normalMappingEnabled_ = false; // No procedural normal/height scratch on console.
#else
    bool normalMappingEnabled_ = true;
#endif
    float normalMapStrength_ = 0.8f;
#ifdef WOWEE_PS4
    bool pomEnabled_ = false; // No procedural normal/height scratch on console.
#else
    ...
    // Glue scenes are M2s too, and bigger ones than any character: the
    // Wrath login scene has 288 bones (B4 test 13). Vertices that name a
    // bone beyond this read the identity slot at the end.
