...
#include "rendering/m2_renderer_particle_budget.hpp"
...
struct M2ModelGPU {
    ...
    bool isGlueScene = false;    // Preserve authored menu effects; no world flame/gravity heuristics.
...
struct M2Instance {
    ...
    bool participatesInPositionDedup = true;
 ...
 */
class M2Renderer {
    // Reuse the existing CPU emitter simulation without allocating a world renderer.
    friend class GlueSceneEffects;
public:
    M2Renderer();
    ~M2Renderer();
...
private:
    ...
    VkDescriptorSetLayout shadowInstanceSetLayout_ = VK_NULL_HANDLE;
    VkDescriptorPool shadowInstancePool_ = VK_NULL_HANDLE;
    // Fixed, mapped GPU storage, owned separately from the main-pass SSBO.
    // This frame slot is written only after its existing submission fence.
    static constexpr uint32_t MAX_SHADOW_INSTANCE_DATA = 32768;
    ...
    bool initializeShadowInstancing(VkRenderPass renderPass,
                                   const VkVertexInputBindingDescription& binding,
                                   const std::vector<VkVertexInputAttributeDescription>& attributes);
    void destroyShadowInstancing();
    ...
    struct M2InstanceGPU {
        ...
        int32_t _pad[2] = {};      //  8 bytes @ offset 88
        glm::vec4 uvLinear{1,0,0,1}; // 16 bytes @ offset 96; total 112 (std430)
    };
    ...
    // buffers of 112 bytes a slot, so this costs about 7 MiB.
    ...
    uint32_t instanceCapacity_[2] = {MAX_INSTANCE_DATA, MAX_INSTANCE_DATA};
    bool ensureInstanceCapacity(uint32_t frameIndex, uint64_t required);
    ...
    // M2 particle emitter system. MAX_M2_PARTICLES used to live here and be
    // read as three different limits; it is m2particles::kMaxParticlesPerInstance
    // now, and only the one it always was.
    std::mt19937 particleRng_{123};

    // The frame's particle budget, and what it is shared out to. See
    // m2_renderer_particle_budget.hpp for why one per-instance constant was
    // not a budget.
    //
    // Written once per update() and read again while recording. Those are two
    // different threads: Renderer::render submits update() to the frame worker
    // pool and renderM2Particles records later on whichever thread owns the M2
    // secondary. It joins that future before any of it (renderer.cpp, "finish
    // structural M2 work before any main-thread collision query"), and the
    // join is what orders these - so do not move the emit/simulate step out
    // from behind it. Nothing here is atomic and nothing here should need to
    // be; if these ever have to be read while update() is in flight, that is
    // a signal the ordering has been broken rather than a reason to add one.
#ifdef WOWEE_PS4
    static constexpr size_t PARTICLE_FRAME_BUDGET = m2particles::kConsoleFrameBudget;
#else
    static constexpr size_t PARTICLE_FRAME_BUDGET = m2particles::kDesktopFrameBudget;
#endif
    /// Live particles one instance may hold this frame.
    size_t particleInstanceShare_ = m2particles::kMaxParticlesPerInstance;
    /// The whole frame's budget after memory pressure, and the counter that
    /// decides when to ask for it again. Sampling it is two kernel calls on
    /// the console, and headroom moves on the scale of a tile load.
    size_t particleFrameBudget_ = PARTICLE_FRAME_BUDGET;
    uint32_t particleBudgetSample_ = 0;
    static constexpr uint32_t kParticleBudgetSampleInterval = 20;
    /// Emitting instances that survived the cull last frame. Lagging by one
    /// frame is what keeps it free: the count falls out of the loop that is
    /// already running, with no pass of its own. Reported, not acted on.
    size_t particleEmittersLastFrame_ = 0;
    /// ...and the smoothed version the cull and the share are actually driven
    /// by. Feeding the raw count back would make this a one-frame loop that
    /// oscillates - tighter cull, fewer emitters, looser cull, more emitters -
    /// and the fires on the boundary would blink at frame rate.
    size_t particleEmittersSmoothed_ = 0;
    /// Above this many admitted emitters the particle cull distance starts
    /// closing in. Sized so an ordinary scene never reaches it and a city
    /// square full of braziers and torches does.
    static constexpr size_t PARTICLE_EMITTER_SOFT_LIMIT = 96;
    /// Vertices the particle VB holds in total, across all frames in flight.
    size_t particleVBVertices_ = 0;
    /// Groups dropped this frame because the frame's vertex region filled,
    /// reported periodically rather than per frame.
    uint32_t particleVBOverflowGroups_ = 0;
    /// particleGroups_ is keyed partly on a VkTexture*, so it gains a bucket
    /// per emitting texture and never loses one. Swept on this counter rather
    /// than every frame; see renderM2Particles.
    uint32_t particleGroupSweepFrame_ = 0;
    static constexpr uint32_t kParticleGroupSweepInterval = 80;
    bool skyMode_ = false;
    ...
    uint32_t commitInstance(M2Instance&& instance);
