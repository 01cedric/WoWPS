...
public:
    ...
    /// PS4: match refraction history to the actual scene target, including
    /// native fallback. Must run between frames, after post-process setup.
    void prepareSceneHistory(VkExtent2D extent);
...
private:
    ...
    struct PerFrameSceneHistory {
        ...
        // PS4 maintains matching destinations for native linear VideoOut and
        // tiled offscreen FXAA color; both share the same depth history.
        VkImage tiledColorImage = VK_NULL_HANDLE;
        VmaAllocation tiledColorAlloc = VK_NULL_HANDLE;
        VkImageView tiledColorView = VK_NULL_HANDLE;
