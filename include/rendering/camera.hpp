...
public:
    ...
    void setClipPlanes(float nearValue, float farValue) {
        if (!std::isfinite(nearValue) || !std::isfinite(farValue) ||
            nearValue <= 0.0f || farValue <= nearValue) return;
        nearPlane = nearValue; farPlane = farValue; updateProjectionMatrix();
    }
    void setRoll(float radians) {
        if (!std::isfinite(radians)) return;
        roll = radians; updateViewMatrix();
    }
