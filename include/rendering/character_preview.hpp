...
public:
    ...
    void shutdown();
    // Retire the displayed unit and its attachments before preparing another
    // one. The render target and any racial backdrop remain valid.
    void releaseCharacterModel();
    ...
    /// A glue scene as the whole picture - the login screen's main-menu scene
    /// with its own camera, animation and lighting baked in - and no character.
    /// The model's first camera frames it exactly as the client does.
    bool loadGlueScene(const std::string& m2Path);

    /// See the racial scene through the camera the scene carries (position,
    /// target and field of view), as the original character screen does,
    /// rather than through the portrait rig. Set before loadCharacter; it
    /// applies to the backdrop loaded with the character.
    void setSceneCameraMode(bool enabled);
    void setSceneScale(float scale);
    [[nodiscard]] bool sceneCameraMode() const { return sceneCameraMode_; }
    ...
    [[nodiscard]] bool hasSceneBackdrop() const { return backdropInstanceId_ != 0 && hasSceneCamera_; }
    [[nodiscard]] bool isSceneReady() const { return modelLoaded_ && hasSceneCamera_ && lastError_.empty(); }
...
private:
    ...
    // Returns the display id only when its model is attached to the head bone.
    uint32_t attachHelm(const std::vector<game::EquipmentItem>& equipment);
    ...
    bool portraitView_ = false;
    bool hasAuthoredPortrait_ = false;
    glm::vec3 portraitCameraPosition_{0.f}, portraitCameraTarget_{0.f};
    float portraitFocusZ_ = 1.6f, portraitDistance_ = 1.4f, portraitFov_ = 30.f;
    // The scene camera onto the render target: the M2 field of view is the
    // diagonal one, converted for this target's aspect.
    void applySceneCamera();
    bool failPreview(const std::string& message);
    void loadSceneEffects(const pipeline::M2Model& scene);
    std::string lastError_;

    pipeline::AssetManager* assetManager_ = nullptr;
    VkContext* vkCtx_ = nullptr;
    std::unique_ptr<CharacterRenderer> charRenderer_;
    std::unique_ptr<GlueSceneEffects> sceneEffects_;
    ...
    // detachWeapon retires this model before a replacement is uploaded.
    static constexpr uint32_t PREVIEW_HELM_MODEL_ID = 9995;
    ...
    glm::vec3 modelBoundMin_{-.5f,-.5f,0}, modelBoundMax_{.5f,.5f,2};
    glm::vec3 previewFitEye_{0}, previewFitTarget_{0};
    float previewFitScale_ = -1;
    glm::vec3 previewStandPosition_{0.0f};
    glm::vec3 previewViewDirection_{0.0f, 1.0f, 0.0f};
    bool transparentBackground_ = false;

    // The authored camera of the scene behind the character (or of the glue
    // scene that is the whole picture), used when sceneCameraMode_ is on.
    bool sceneCameraMode_ = false;
    bool hasSceneCamera_ = false;
    glm::vec3 sceneCameraPosition_{0.0f};       // where the camera is this frame
    glm::vec3 sceneCameraTarget_{0.0f};         // what it looks at this frame
    glm::vec3 sceneCameraPositionBase_{0.0f};
    glm::vec3 sceneCameraTargetBase_{0.0f};
    float sceneCameraFov_ = 0.0f;   // diagonal, radians, as the M2 stores it
    float sceneScale_ = 1.0f;       // login framing; racial previews keep 1:1
    float sceneCameraNear_ = 0.5f, sceneCameraFar_ = 30000.0f;
    float sceneCameraRoll_ = 0.0f;
    // The authored camera path, relative to the bases, on the scene's own
    // clock: the Northrend login flies in over the citadel, the racial scenes
    // settle on the character. Sequence 0, looping with the scene.
    pipeline::M2AnimationTrack sceneCameraPositions_;
    pipeline::M2AnimationTrack sceneCameraTargets_;
    pipeline::M2AnimationTrack sceneCameraRolls_;
    std::vector<uint32_t> sceneGlobalDurations_;
    float sceneCameraClockMs_ = 0.0f;
    uint32_t sceneCameraLoopMs_ = 0;
    bool takeSceneCamera(const pipeline::M2Model& scene);
    void animateSceneCamera(float deltaTime);

    // Console diagnostics: the first composite of a preview goes out one
    // draw per submit, each announced in boot.log before it is sent, so a
    // GPU fault names the draw. WOWEE_PS4_SCENE_LEVEL in the env file picks
    // 0 (no scene draws), 1 (normal, default) or 2 (draw isolation).
    bool firstCompositeDiagnosed_ = false;
    void diagnoseFirstComposite(uint32_t frameSlot, const VkClearColorValue& clearColor);
    // Per-scene state kept across launches in config/scene_diag.txt: which
    // draw was being attempted when the process died (the GPU fault leaves
    // no other trace), the degradations forced so far, the draws given up
    // on. Each launch then goes one step further than the last.
public:
    struct SceneDiagState {
        std::vector<uint32_t> bad;
        unsigned forced = 0;            // 1 = opaque pipeline, 2 = white texture, 4 = simple material
        int attempting = -1;
        unsigned attemptingForced = 0;
        std::string attemptingDesc;
        std::vector<std::string> notes;
    };
private:
    std::string sceneDiagKey_ = "<portrait>";
    std::string sceneDiagLoadedKey_;
    SceneDiagState sceneDiag_;
    void loadSceneDiagState();
    void saveSceneDiagState() const;
    void applySceneDiagOverrides();
    // The console samples a white 1x1 colour texture through the comparison
    // sampler where the desktop binds a depth image.
    std::unique_ptr<VkTexture> ps4ShadowPlaceholder_;
