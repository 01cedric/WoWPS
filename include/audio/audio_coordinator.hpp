...
public:
    ...
    // Call after world jobs stop, before restarting glue music. No bank reload.
    void stopWorldAudio();
    void beginWorldAudio();
    void setTransportMusic(uint32_t entry, bool zeppelin, pipeline::AssetManager* assets);
...
private:
    ...
    uint32_t musicTransport_ = 0;
    std::string transportMusicPath_;
    std::string transportTracks_[2];
    bool transportTrackScanned_[2] = {};
    std::chrono::steady_clock::time_point transportMusicRetryAt_{};
    bool audioAvailable_ = false;
    bool worldAudioActive_ = true;

    // Zone/music state - moved from Renderer
    uint32_t playingMusicArea_ = 0;
    int playingMusicPeriod_ = -1;
    float musicTimeHours_ = 12;
