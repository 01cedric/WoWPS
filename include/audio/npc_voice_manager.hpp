...
public:
    ...
    void playCombatWound(uint64_t npcGuid, uint32_t displayId, const glm::vec3& position);
    void playCombatDeath(uint64_t npcGuid, uint32_t displayId, const glm::vec3& position);
...
private:
    ...
    std::unordered_map<uint32_t, uint32_t> creatureWoundSoundByDisplay_;
    std::unordered_map<uint32_t, uint32_t> creatureDeathSoundByDisplay_;
    std::unordered_set<uint64_t> missingCombatMappings_;
    void playCombatEvent(uint64_t npcGuid, uint32_t displayId, const glm::vec3& position,
                         const std::unordered_map<uint32_t, uint32_t>& mappings,
    ...
    std::unordered_map<uint64_t, std::chrono::steady_clock::time_point> lastWoundTime_;
    std::unordered_map<uint64_t, std::chrono::steady_clock::time_point> lastDeathTime_;
