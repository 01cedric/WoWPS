...
vec3 filteredRadiance(ivec2 source, ivec2 sceneSize, float depth) {
    ...
    // Evaluate every original depth gate, then combine each accepted pair
    // with one linear radiance sample. The two weights remain independent:
    // rejecting either depth endpoint cannot borrow illumination from it.
    // High uses 15 radiance reads instead of25; Low retains its9-tap kernel.
    for (int y=-2;y<=2;++y) {
        ...
        for (int x=-2;x<=2;++x) {
            ...
            if (abs(x)<=radius) {
                ...
                if (abs(predictedW-guideW)<=max(0.0005*guideW*predictedW,0.0002*guideW))
                    weight = axisWeights[abs(x)]*axisWeights[abs(y)];
            }
            if (x==-2 || x==1) {
                previousWeight=weight;
            } else {
                if (x==0) previousWeight=0.0;
                float pairWeight=previousWeight+weight;
                if (pairWeight>0.0) {
                    int previousX=clamp(source.x+x-1,0,sceneSize.x-1);
                    vec2 coordinate=vec2(samplePixel)+0.5;
                    coordinate.x-=float(samplePixel.x-previousX)*(previousWeight/pairWeight);
                    sum+=textureLod(scattering,coordinate*inverseSize,0.0).r*pairWeight;
                    validWeight+=pairWeight;
                }
            }
        }
    }
    ...
    float peak = validWeight > 0.000001 ? sum/validWeight : texelFetch(scattering,source,0).r;
